	...
	public static Field getAccessibleField(final Object obj, final String fieldName) {
		...
		for (Class<?> superClass = obj.getClass(); superClass != Object.class; superClass = superClass.getSuperclass()) {
			try {
				...
				// Field不在当前类定义,继续向上转型
				logger.error("{}--->>>{}===>>>反射赋值操作异常：", obj.getClass().getSimpleName(), fieldName, e);
				// logger.error("反射赋值操作异常：",e);
			}
		}
		return null;
	}
			...
			if (face[i].getName().equals(szInterface)) {
				...
				for (int x = 0; x < face1.length; x++) {
					if (face1[x].getName().equals(szInterface)) {
						return true;
					} else if (isInterface(face1[x], szInterface)) { return true; }
				}
			}
		}
		if (null != c.getSuperclass()) { return isInterface(c.getSuperclass(), szInterface); }
		return false;
	}
}
