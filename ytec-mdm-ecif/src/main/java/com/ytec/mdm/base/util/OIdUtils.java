...
public class OIdUtils {
	...
	/**
	 *
	 * @函数名称:Object createId(Object obj)
	 * @函数描述:为对象生成id.
	 * @参数与返回说明: Object createId(Object obj)
	 * @算法描述:
	 */
	public static Object createId(Object obj) throws Exception{
		Class clazz = obj.getClass();
		IMCIdentifying util = (IMCIdentifying) SpringContextUtils.getBean(MdmConstants.MCIDENTIFYING);
		String key=GetKeyNameUtil.getInstance().getKeyName(obj);
		if(key!=null&&!key.equals(MdmConstants.CUSTID)){
			Field field=clazz.getDeclaredField(key);
			Class typeClass = field.getType();
			field.setAccessible(true);
			String keyV=util.getPriIdByAttrName(NameUtil.getJavaToColum(key));
			if(keyV!=null){
				if(typeClass.equals(Long.class)||typeClass.equals(long.class)){
					//LONG 类型
					field.set(obj, Long.parseLong(keyV));
				}else if(typeClass.equals(String.class)){
					//字符类型
					field.set(obj, keyV);
				}else if(typeClass.equals(BigInteger.class)){
					//BigInteger 类型
					field.set(obj, BigInteger.valueOf(Long.parseLong(keyV)));
				}else if(typeClass.equals(BigDecimal.class)){
					//BigDecimal 类型
					field.set(obj, BigDecimal.valueOf(Long.parseLong(keyV)));
				}else{
					//其他类型
					field.set(obj, keyV);
				}
