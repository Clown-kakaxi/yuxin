...
public class QueryEcifDealEngine extends AbstractEcifDealEngine {
	...
	public void execute(EcifData data) {
		try {
			...
			buildNodeXML(responseEle, rootTxMsgNode, txMsgNodeMap, reqParamMap, parentNodeValueMap);
	 ...
	 * @算法描述:
	 */
	private void buildNodeXML(Element pointer, TxMsgNode currentxMsgNode, Map<Long, List<TxMsgNode>> txMsgNodeMap,
			Map<String, String> reqParamMap, Map<Long, Object> parentNodeValueMap) {
		if (MdmConstants.NODE_TP_T.equals(currentxMsgNode.getNodeTp())) {
			buildTableNodeXML(pointer, currentxMsgNode, txMsgNodeMap, reqParamMap, parentNodeValueMap);
			...
			buildChildNodeXML(pointer, currentxMsgNode, txMsgNodeMap, reqParamMap, parentNodeValueMap);
		}
	}
	 ...
	 *        父节点的属性值 ,用于外键查询
	 */
	private void buildChildNodeXML(Element pointer, TxMsgNode currentxMsgNode, Map<Long, List<TxMsgNode>> txMsgNodeMap,
			Map<String, String> reqParamMap, Map<Long, Object> parentNodeValueMap) {
		// 当前节点下的所有子节点
		List<TxMsgNode> txMsgNodeList = txMsgNodeMap.get(currentxMsgNode.getNodeId());
		if (txMsgNodeList != null) {
			for (TxMsgNode txMsgNode : txMsgNodeList) {
				buildNodeXML(pointer, txMsgNode, txMsgNodeMap, reqParamMap, parentNodeValueMap);
				if (!this.ecifData.isSuccess()) { return; }
			}
		}
	}
	 ...
	 * @算法描述:
	 */
	private void buildCommonNodeXML(Element pointer, TxMsgNode currentxMsgNode, Map<String, String> reqParamMap) {
	 ...
	 * @算法描述:
	 */
	private void buildTableNodeXML(Element pointer, TxMsgNode currentxMsgNode, Map<Long, List<TxMsgNode>> txMsgNodeMap,
			Map<String, String> reqParamMap, Map<Long, Object> parentNodeValueMap) {
		// 基于数据库表的节点
		Long nodeId = currentxMsgNode.getNodeId();
			...
			if (txMsgNodeAttr.getFkAttrId() != null && txMsgNodeAttr.getFkAttrId() != 0) {
				...
				if (StringUtil.isEmpty(value)) {
					...
					if (MdmConstants.NODE_TYPE_FIX_STR.equals(dataType)
						...
						this.ecifData.setStatus(ErrorCode.ERR_XML_CFG_NO_FK.getCode(), msg);
						log.error("结点{}外键参数类型错误,外键类型配置为:{}", nodeCode, dataType);
						return;
					}
					requestMap.put(MdmConstants.QUERY_SQL_FILTER_FK_FLAG + txMsgNodeAttr.getAttrCode(), value);
				}
			}
		...
		if (this.ecifData.getQueryModelObj().getResultSize() > 1) {
			if (!MdmConstants.NODE_GROUP_M.equals(currentxMsgNode.getNodeGroup())
					&& !MdmConstants.NODE_NOGROUP_LIST_SUFFIX) {
		...
		for (Map<String, Object> rowMap : queryResultList) {
			...
			if (!noResult || !nodeGroupFlag) {
				...
				for (TxMsgNodeAttr txMsgNodeAttr : txMsgNodeAttrList) {
					...
					if (!StringUtil.isEmpty(value)) {
						/*** 信息漂白 ****/
						if (!EcifPubDataUtils.infoLevelRead(authType, authCode, MdmConstants.CTRLTYPE_QUERY,
								txMsgNodeAttr.getTabId(), txMsgNodeAttr.getColId())) {
							if (StringUtil.isEmpty(MdmConstants.txInfoCtrlformat)) {// 如果屏蔽占位符为空，标签不要
								continue;
							}
							if (txMsgNodeAttr.getDataLen() != null
									&& txMsgNodeAttr.getDataLen() < MdmConstants.txInfoCtrlformat.length()) {
								newValue = MdmConstants.txInfoCtrlformat.substring(MdmConstants.txInfoCtrlformat
										.length() - txMsgNodeAttr.getDataLen().intValue());
							} else {
								newValue = MdmConstants.txInfoCtrlformat;
							}
						} else {
							// 转换成字符串
							newValue = this.convertObjectToString(nodeId, txMsgNodeAttr, value);
							if (!this.ecifData.isSuccess()) { return; }
							// 转换
					...
					if (txMsgNodeAttr.getDataLen() != null && txMsgNodeAttr.getDataLen() != 0) {
						...
						//将<node></node>形式的表示方法修改为<node/>形式，以节省报文长度， wangtb@yuchengtech.com
						if (newValue == null || "null".equals(newValue) || StringUtil.isEmpty(newValue)) {
							hand.addElement(txMsgNodeAttr.getAttrCode());
						} else {
			...
			// }
			if (!noResult) {
				buildChildNodeXML(hand, currentxMsgNode, txMsgNodeMap, reqParamMap, parentNodeValueMap);
			}
		}
