...
public abstract class AbstractEcifDealEngine extends TransVerif implements
	...
	protected Object convertStringToObject(Class typeClass, String value,
		...
		if (java.lang.String.class.equals(typeClass)) {// 字符型
			...
			if(value!=null&&value.equals("sysdate")){
				value = StringUtil.date2Str(new java.util.Date(),TIMESTAMP_FORMAT);
			}
	...
	public void setTxModel(TxModel txModel) {
	 ...
	 * @函数描述:获取结点过滤条件
	 * @参数与返回说明:
	 * 		@param currentxMsgNode 结点对象
	 * 		@param requestMap      条件存放MAP
	 * 		@param reqParamMap     原始条件数据MAP
	 * @算法描述:
	 */
	protected void getFilterParam(TxMsgNode currentxMsgNode,
			Map<String, Object> requestMap, Map<String, String> reqParamMap) {
		/**
		 * 查询条件参数
		 */
		List<TxMsgNodeFilter> txMsgNodeFilterList = this.txModel
				.getTxMsgNodeFilterMap().get(currentxMsgNode.getNodeId());
		if (txMsgNodeFilterList != null && !txMsgNodeFilterList.isEmpty()) {
			String value = null;
			String attrCode = null;
			Object newValue = null;
			/****
			 * 从参数过滤配置中获取参数名称，从报文中获取参数
			 */
			for (TxMsgNodeFilter txMsgNodeFilter : txMsgNodeFilterList) {// ------------------------------
				attrCode = txMsgNodeFilter.getReqAttrCode();
				if (txMsgNodeFilter.getAttrId() != null
						&& txMsgNodeFilter.getAttrId() != 0) {
					value = reqParamMap.get(attrCode);
					if (StringUtil.isEmpty(value)) {
						value = txMsgNodeFilter.getDefaultVal();
						/***
						 *  默认值规则 **
						 *  */
						if (!StringUtil.isEmpty(value)) {
							value = parseDefaultVal(value,
									ecifData.getBodyNode());
					...
					value = txMsgNodeFilter.getFilterConditions();
					/*** 
					 * 默认值规则 **
					 * */
					if (!StringUtil.isEmpty(value)) {
						value = parseDefaultVal(value, ecifData.getBodyNode());
						if (!this.ecifData.isSuccess()) {
							return;
						}
					}
				...
				if (StringUtil.isEmpty(value)) {
					if ("N".equals(txMsgNodeFilter.getReqAttrNullAble())) {
						log.warn("报文节点:{},查询属性:{}找不到查询条件",
								currentxMsgNode.getNodeCode(), attrCode);
						this.ecifData
								.setStatus(
										ErrorCode.ERR_XML_CFG_NO_FK.getCode(),
										"查询条件为空");
						return;
					} else {
						log.warn("报文节点:{},查询属性:{}找不到查询条件,配置允许为空",
								currentxMsgNode.getNodeCode(), attrCode);
						requestMap.put(attrCode + "_1", value);
					}
				} else {
					try {
						/**
						 * 数据转换
						 */
						newValue = convertStringToObject(
								currentxMsgNode.getNodeCode(), txMsgNodeFilter,
								value);
						if (!this.ecifData.isSuccess()) {
							return;
						}
					} catch (Exception e) {
						log.error("条件转换为对象失败", e);
						this.ecifData.setStatus(
								ErrorCode.ERR_XML_FORMAT_ERROR.getCode(),
								"条件转换失败");
						return;
					}
					requestMap.put(attrCode + "_1", newValue);
				}
			}
		} else {
			/**
			...
			if(requestMap.isEmpty()){
				log.warn("报文节点:{}没有配置过滤查询条件", currentxMsgNode.getNodeCode());
				this.ecifData.setStatus(
						ErrorCode.ERR_SERVER_CFG_FILE_ERROR.getCode(), "未配置查询条件");
				return;
			}
		}
	}
