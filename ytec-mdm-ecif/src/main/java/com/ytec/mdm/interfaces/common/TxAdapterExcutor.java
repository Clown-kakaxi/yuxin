...
public abstract class TxAdapterExcutor implements Runnable {
	...
	public TxAdapterExcutor() {
		 ...
		 * 为每个服务工作线程分配一个ID号
		 */
		long id = (long) (Math.random() * 10000000) + System.currentTimeMillis() + this.hashCode();
	...
	protected void endAdapter() {
		...
		if (this.recvmsg != null) {
			if (data.getTxCode() != null) {
				log.info("交易码[{}],请求系统[{}],交易返回[{}],处理耗时[{}]ms", data.getTxCode(), data.getOpChnlNo(), data.getDetailDes(), data.getStopWatch().getElapsedTime());
			}
			if (!data.isSuccess()) {
	...
	protected void resolvingXml() throws RequestIOException {
		try {
			 ...
			 * 解析XML报文
			 ****/
			doc = XMLUtils.stringToXml(recvmsg.trim());
			/***
			 * 保存原始报文供以后客户化开发使用
			 */
