		...
		try {
			...
			if (read < 0) {
				...
				*/
				...
				/**
				 * added by wangtb@yuchengtech.com for nginx
				 */
				log.info("@@@服务器read:" + read);
				...
				do {
					...
					if (request) {
						...
						return;			//added by sjf:如果读完则退出本次连接
				...
				while(true);					//added by sjf:如果未读完则一直读取，直到达到最大次数（100）退出，receivebuffer.hasRemaining()判断有误
...
//				}
			...
			try {
				...
				data.setStatus(ErrorCode.ERR_CLIENT_BAD_REQUEST.getCode(), ErrorCode.ERR_CLIENT_BAD_REQUEST.getChDesc());
				afterExecutor();
				send(decoder.encode(resXml));
			} catch (Exception e1) {
			...
			log.error("IO错误", ie);
			try {
				data.setStatus(ErrorCode.ERR_OTHER_ERROR.getCode(), ErrorCode.ERR_OTHER_ERROR.getChDesc() + ":" + ie.getMessage());
				selector.removeOpenSocket(client);
			} catch (Exception e2) {
			...
			try {
				...
				data.setStatus(ErrorCode.ERR_SERVER_INTERNAL_ERROR.getCode(), ErrorCode.ERR_SERVER_INTERNAL_ERROR.getChDesc() + ":" + e.getStackTrace());
				afterExecutor();
				send(decoder.encode(resXml));
			} catch (Exception e3) {
			...
			try {
				...
				data.setStatus(ErrorCode.ERR_SERVER_INTERNAL_ERROR.getCode(), ErrorCode.ERR_SERVER_INTERNAL_ERROR.getChDesc() + ":" + ex.getStackTrace());
				afterExecutor();
				send(decoder.encode(resXml));
			} catch (Exception e3) {
