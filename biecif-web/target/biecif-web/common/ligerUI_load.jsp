<link rel="stylesheet" type="text/css"
	href="${ctx}/css/classics/ligerui-custom.css" />
<link rel="stylesheet" type="text/css"
	href="${ctx}/css/classics/template/common.css" />
<script type="text/javascript" src="${ctx}/js/ligerUI/ligerui.all.js"></script>
<script type="text/javascript" src="${ctx}/js/ligerUI/ligerui.expand.min.js"></script>
<script type="text/javascript" src="${ctx}/js/bione/BIONE.js"></script>
<script type="text/javascript" src="${ctx}/js/bione/json2.min.js"></script>
<script type="text/javascript" src="${ctx}/js/bione/Common.js"></script>