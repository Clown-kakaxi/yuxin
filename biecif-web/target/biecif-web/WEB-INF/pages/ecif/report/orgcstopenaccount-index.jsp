...
<script type="text/javascript">	
	...
	$(init);
	function init() {
		url = "${ctx}/ecif/report/openaccount/list.json";
		initGrid();
		searchForm();
		initButtons();
	...
	function searchForm() {
		$("#search").ligerForm({
			fields : [{
				display : "<font color='red'>*</font>开户开始日期",
				name : "srptMonth",
				newline : true,
				type : "date",
				cssClass : "field",
				...
				display : "<font color='red'>*</font>开户结束日期",
				name : "erptMonth",
				newline : false,
				type : "date",
				cssClass : "field",
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/report/openaccount/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			...
			url: '${ctx}/ecif/report/openaccount/getExportFile.json?' + timestamp,
			type: 'post',
			beforeSend : function() {
				BIONE.loading = true;
