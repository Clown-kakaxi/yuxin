...
<script type="text/javascript">	
	...
	$(init);
	function init() {
		url = "${ctx}/ecif/report/riskgrade/list.json";
		initGrid();
		searchForm();
		initButtons();
	...
	function initForm() {
		...
		//-- 初始化机构部门联动
		if($.ligerui.get("createBranchNo")){
						
				$.ligerui.get("createBranchNo").openSelect({
					url : "${ctx}/ecif/report/orgtreelist",
				...
				$.ligerui.get("createBranchNo").bind('beforeOpen', function() {
					firstopen();
					return false;
				});
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/report/riskgrade/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			...
			url: '${ctx}/ecif/report/riskgrade/getExportFile.json?' + timestamp,
			type: 'post',
			beforeSend : function() {
				BIONE.loading = true;
