<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="/common/taglibs.jsp"%>
<html>
<head>
<meta name="decorator" content="/template/template0.jsp">
<script type="text/javascript">
	var grid, btns, url, ids = [];
	
	var nodeId = "${nodeId}";
	
	$(function() {
		url = "${ctx}/ecif/transaction/txmsgnodeattr/list.json?nodeId=" + nodeId;
		searchForm();
		initGrid();
		initButtons();
		BIONE.addSearchButtons("#search", grid, "#searchbtn");
	});
	
	function RenderFlag(rowdata){
		if(rowdata.flag == '1'){
			return '有效';
		}else if(rowdata.flag == '0'){
			return '无效';
		}else{
			return rowdata.flag;
		}
	}

	//搜索表单应用ligerui样式
	function searchForm() {
		$("#search").ligerForm({

			fields : [ {
				name:"nodeId",
				type:'hidden'
			} ]
		});
		var e = $("[name=nodeId]");
		
		e.val(nodeId);
	}

	function initGrid() {

		grid = $("#maingrid").ligerGrid(
				{
					height : '100%',
					width : '100%',
					columns : [
							{
								display : '属性代码',
								name : 'attrCode',
								align : 'center',
								width : '20%',
								minWidth : '10%',
								hide:1 
							},
							{
								display : '属性名称',
								name : 'attrName',
								align : 'center',
								width : '20%',
								minWidth : '10%'
							},
							{
								display : '所属表',
								name : 'tabId',
								align : 'center',
								width : '20%',
								minWidth : '10%',
								hide:1 
							},
							{
								display : '所属字段',
								name : 'colId',
								align : 'center',
								width : '20%',
								minWidth : '10%'
							},
							{
								display : '非空标志',
								name : 'nulls',
								align : 'center',
								width : '20%',
								minWidth : '10%'
							},
							{
								display : '数据类型',
								name : 'dataType',
								align : 'center',
								width : '20%',
								minWidth : '10%'
							},
							{
								display : '长度',
								name : 'dataLen',
								align : 'center',
								width : '20%',
								minWidth : '10%'
							},
							{
								display : '格式',
								name : 'dataFmt',
								align : 'center',
								width : '20%',
								minWidth : '10%'
							},
							{
								display : '数据过滤、转换',
								name : 'checkRule',
								align : 'center',
								width : '10%',
								minWidth : '10%'
							},
							{
								display : '标准代码分类',
								name : 'cateId',
								align : 'center',
								width : '10%',
								minWidth : '10%'
							},
							{
								display : '备注',
								name : 'defaultVal',
								align : 'center',
								width : '10%',
								minWidth : '10%'
							} ],
					checkbox : true,
					usePager : true,
					isScroll : false,
					rownumbers : true,
					alternatingRow : true,//附加奇偶行效果行
					colDraggable : true,
					dataAction : 'server',//从后台获取数据
					method : 'post',
					url : url,
					sortName : 'serviceAuthId', //第一次默认排序的字段
					sortOrder : 'asc',
					toolbar : {}
				});
	}

	function initButtons() {
		btns = [ {
			text : '增加',
			click : resDef_add,
			icon : 'add',
			operNo : 'resDef_add'
		}, {
			text : '修改',
			click : resDef_modify,
			icon : 'modify',
			operNo : 'resDef_modify'
		}, {
			text : '删除',
			click : resDef_delete,
			icon : 'delete',
			operNo : 'resDef_delete'
		} ];
		BIONE.loadToolbar(grid, btns, function() {
		});

	}

	function resDef_add(item) {
		BIONE.commonOpenLargeDialog('添加资源', 'resDefManage',
				'${ctx}/ecif/transaction/txmsgnodeattr/new?nodeId=' + nodeId);
	}
	function resDef_modify(item) {
		achieveIds();
		if (ids.length == 1) {
			var buttons = [
					{
						text : '保存',
						onclick : function(item, dialog) {
							BIONE.submitForm($("#mainform",dialog.frame.window.document), function() {
								dialog.close();
								initGrid();
								parent.BIONE.tip('修改成功');
							}, function() {
								BIONE.tip('保存失败');
							});
						}
					}, {
						text : '取消',
						onclick : function(item, dialog) {
							dialog.close();
						}
					} ];
			BIONE.commonOpenLargeDialog('修改资源', 'resDefManage',
					'${ctx}/ecif/transaction/txmsgnodeattr/' + ids[0] + '/edit', buttons);
		} else if (ids.length > 1) {
			BIONE.tip('只能选择一条记录');
		} else {
			BIONE.tip('请选择记录');
		}
	}
	function resDef_delete(item) {
		achieveIds();
		if (ids.length > 0) {
			$.ligerDialog.confirm('确实要删除这些记录吗?', function(yes) {
				if(yes) {
					var flag = false;
					$.ajax({
						async : false,
						type : "DELETE",
						url : "${ctx}/ecif/transaction/txmsgnodeattr/" + ids.join(','),
						dataType : "script",
						success : function() {
							flag = true;
						}
					});
					if (flag == true) {
						BIONE.tip('删除成功');
						initGrid();
					} else {
						BIONE.tip('删除失败');
					}
				}
			});
		} else {
			BIONE.tip('请选择记录');
		}
	}
	
	// 获取选中的行
	function achieveIds() {
		ids = [];
		var rows = grid.getSelectedRows();
		for(var i in rows) {
			ids.push(rows[i].attrId);
		}
	}
</script>
</head>
<body>
</body>
</html>