...
<script type="text/javascript">
	...
	var testId;
	var custNoTemp = identNoTemp = lastUpdateSystTemp = sdateTemp = edateTemp = "";
	$(init);

	function init() {
		url = "${ctx}/ecif/customer/speciallistapproval/list.json";
		initGrid();
		searchForm();
		initButtons();
			...
			BIONE.createButton({
				...
				click : function() {
					...
					if(flag){
						...
						lastUpdateSystTemp = $('#specialListKind').val();
	...
	function test_delete_dynamic(item) {
		...
		if (ids.length > 0) {
			...
			BIONE.commonOpenLargeDialog('审批信息', 'testManage',
					'${ctx}/ecif/customer/speciallistapproval/'+testId+'/edit');
		} else {
			BIONE.tip('请选择记录');
		}
	}
	function file_add(item) {
		...
		BIONE.commonOpenDialog('上传模板', 'upload',550,335,
				'${ctx}/ecif/customer/speciallistapproval/importresult');
	}
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/customer/speciallistapproval/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url: '${ctx}/ecif/customer/speciallistapproval/getReportFile?'+timestamp,
			type: 'get',
			data: {
				repo: "2",
				//others: others
				custNo: custNoTemp,
				identNo: identNoTemp,
				lastUpdateSys: lastUpdateSystTemp,
				srptMonth : sdateTemp,
				erptMonth : edateTemp
			},
			//success: doDownload
	...
	function achieveIds() {
		...
		$(rows).each(function() {
			ids.push(this.specialListApprovalId);
		});
	}
