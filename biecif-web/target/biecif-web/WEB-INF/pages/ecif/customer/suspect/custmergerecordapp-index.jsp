...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/customer/custmergerecordapp/list.json";
		initGrid();
		searchForm();
		initButtons();
		...
		$("#search").ligerForm({
			fields : [{
				...
				options : {
					...
					url : "${ctx}/ecif/customer/custmergerecordapp/getComBoBox.json",
					ajaxType : "get"
				},
				attr : {
					op : "=",
	...
	function test_delete_dynamic(item) {
		...
		if (ids.length > 0) {
			...
			BIONE.commonOpenLargeDialog('审批信息', 'testManage',
					'${ctx}/ecif/customer/custmergerecordapp/'+testId+'/edit');
		} else {
			BIONE.tip('请选择记录');
		}
	}
	...
	function file_load(item) {
		// 导入
		BIONE.commonOpenDialog('导入客户合并已审批信息', 'upload', 562, 334,
					'${ctx}/ecif/customer/custmergerecordapp/importresult');
	}
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/customer/custmergerecordapp/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url: '${ctx}/ecif/customer/custmergerecordapp/getReportFile?'+timestamp,
			type: 'get',
			data: {
				repo: "7",
				//others: others
	...
	function achieveIds() {
		...
		$(rows).each(function() {
			ids.push(this.mergeRecordApprovalId);
		});
	}
