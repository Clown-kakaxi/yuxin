...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/customer/custsplitrecord/list.json";
		initGrid();
		searchForm();
		initButtons();
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					method : 'post',
					url : url,
					sortName : 'splitOperTimeDate', //第一次默认排序的字段
					sortOrder : 'desc',
					toolbar : {}
				});
	}
	...
	function RenderMergeType(rowdata){
		return BIONE.paramTransformer(rowdata.splitStat, 
				'${ctx}/ecif/customer/suspectgroup/getCodeMapMergeType.json', '');
	}
	
	function file_load(item) {
		// 导入
		BIONE.commonOpenDialog('导入客户拆分待审批信息', 'upload', 562, 334,
					'${ctx}/ecif/customer/custsplitrecord/importresult');
	}
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/customer/custsplitrecord/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url: '${ctx}/ecif/customer/custsplitrecord/getReportFile?'+timestamp,
			type: 'get',
			data: {
				repo: "5",
				//others: others
