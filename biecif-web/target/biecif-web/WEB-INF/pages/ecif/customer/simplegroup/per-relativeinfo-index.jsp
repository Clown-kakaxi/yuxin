...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/perinfo/perrelative/list.json";
		initGrid();
		initTab();
		searchForm();
		initButtons();
	...
	function searchForm() {
		...
		$("#form_styles").ligerForm({
			fields : [ {
				...
				options : {
					valueFieldID : 'id',
					url : "${ctx}/ecif/orgctzt/getComBoBox.json?codeType="+"CD010030",
					ajaxType : "post"
				},
				attr : {
					tabFlag : "styles",
	...
	function submitFrom(custId, queryName){
		...
		var pageName = "per-relativeinfo-index";
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
			action : '${ctx}/ecif/perinfo/perrelative/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url : '${ctx}/ecif/perinfo/perrelative/getReportFile?' + timestamp,
			type : 'get',
			data : {
				param0 : param0,
				param1 : param1,
