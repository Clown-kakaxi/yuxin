...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/orginfo/orgbasic/list.json";
		initGrid();
		initTab();
		searchForm();
		initButtons();
	...
	function submitFrom(custId, queryName){
		...
		var pageName = "org-basicinfo-index";
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
			action : '${ctx}/ecif/orginfo/orgbasic/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url : '${ctx}/ecif/orginfo/orgbasic/getReportFile?' + timestamp,
			type : 'get',
			data : {
				param0 : param0,
				param1 : param1,
