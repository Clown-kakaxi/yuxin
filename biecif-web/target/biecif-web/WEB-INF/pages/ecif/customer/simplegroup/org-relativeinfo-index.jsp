...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/orginfo/orgrelative/list.json";
		initGrid();
		initTab();
		searchForm();
		initButtons();
				...
				.each(
						function() {
							...
							if (selectedTab == null) {
								...
								if ($(this).attr("tabFlag") != selectedTab)
									return;
							}
	...
	function searchForm() {
		...
		$("#form_styles").ligerForm({
			fields : [ {
				...
				options : {
					valueFieldID : 'id',
					url : "${ctx}/ecif/orgctzt/getComBoBox.json?codeType="+"CD010004",
					ajaxType : "post"
				},
				attr : {
					tabFlag : "styles",
	...
	function renderData(data) {
		...
		return value;
		--%>
		var value = "<a href='#' onclick='submitFrom(\""+custId+"\", \""+queryName+"\");'><font color='blue'>" + data.srcCustNo + "</font></a>";
		return value;
	}
	...
	function submitFrom(custId, queryName){
		...
		var pageName = "org-relativeinfo-index";
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
			action : '${ctx}/ecif/orginfo/orgdetailed'
		}).css('display', 'none');
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
			action : '${ctx}/ecif/orginfo/orgrelative/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url : '${ctx}/ecif/orginfo/orgrelative/getReportFile?' + timestamp,
			type : 'get',
			data : {
				param0 : param0,
				param1 : param1,
