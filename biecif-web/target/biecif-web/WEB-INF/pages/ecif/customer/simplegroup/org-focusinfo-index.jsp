...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/orginfo/orgfocus/list.json";
		initGrid();
		initTab();
		searchForm();
		initButtons();
	...
	function submitFrom(custId, queryName){
		...
		var pageName = "org-focusinfo-index";
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target : '',
			method : 'post',
			action : '${ctx}/ecif/orginfo/orgfocus/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url : '${ctx}/ecif/orginfo/orgfocus/getReportFile?' + timestamp,
			type : 'get',
			data : {
				param0 : param0,
				param1 : param1,
