(function ($)
{
    ...
    window.liger = $.ligerui = {
        ...
        // 错误提示
        error: {
            managerIsExist: 'manager id is existed'
        },
        getId: function (prev)
        {
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        ajaxType:'post',		// 异步加载提交方式，默认是POST提交(edit by caiqy in
								// 2012-07-06)
        onSuccess: null,
        onError: null,
...
//            })
            ...
            if (!p.selectBoxHeight && itemsleng < 8) p.selectBoxHeight = itemsleng * 25;
...
//            if (value)
 ...
 //       },
        ...
        {
            var g = this, p = this.options;
            // edit by caiqy in 2012-07-06,add ajaxType
            var ajaxType = p.ajaxType;
            if(ajaxType == null){
            	ajaxType = "post";
            }
            if(ajaxType.toLowerCase() != "post" 
            	&& ajaxType.toLowerCase() != "get"){
            	ajaxType = "post";
            }
            this.clearContent();
            if (g.select)
        ...
        setData: function (data)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                if (!p.columns)
                {
                    $("table.l-table-checkbox", g.selectBox).append("<tr value='" + val + "'><td style='width:18px;'  index='" + i + "' value='" + val + "' text='" + txt + "' ><input type='checkbox' /></td><td index='" + i + "' value='" + val + "' align='left' class='l-noselected'>" + txt + "</td>");
                    $("table.l-table-nocheckbox", g.selectBox).append("<tr value='" + val + "'><td index='" + i + "' value='" + val + "' align='left' class='l-noselected'>" + txt + "</td>");
                } else
                {
        ...
        setTree: function (tree)
        {
            ...
            {
            	// modify by xugy 2012-09-19
                tree.onSelect = function (node)
                {
                	// modify begin
                	if (p.onBeforeSelect) {
        				if(p.onBeforeSelect(node, null) == false) {
        					return;
        				}
        			}
                	// modify end
   ...
   //                 return;
   //             }
                $(".l-selected", g.selectBox).removeClass("l-selected").addClass("l-noselected");
                $(this).addClass("l-selected");
                if (g.select)
                {
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        //2013-04-01,change the default value of 'isHidden' to 'false',this is not a joke!
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            {
                // 位置初始化
            	// edit by caiqy , $(window) change to $(document)
                var left = 0;
                var top = 0;
                var width = p.width || g.dialog.width();
                if (p.slide == true) p.slide = 'fast';
                if (p.left != null) left = p.left;
                else p.left = left = 0.5 * ($(document).width() - width);
                if (p.top != null) top = p.top;
                else p.top = top = 0.5 * ($(document).height() - g.dialog.height()) + $(document).scrollTop() - 10;
...
(function ($)
{
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput)
    {
        ...
        var g = this, p = this.options;
        var inputOptions = {};
        // 2012-4-15修改支持comboBox下拉框功能 by songxuefeng
        var ltype ="";
        $(p.fields).each(function (index, field)
                {
                if(jinput[0].type!="hidden"){
                	if (field.comboboxName&&field.comboboxName==jinput[0].id)
                     	{
                			inputOptions= field.options;
							ltype="select";
                  
                     }else if(field.name&&field.name==jinput[0].id){
                     		inputOptions= field.options;
                     		ltype=field.type;
                     }
                }else{
        ...
        else if(jinput.is(":hidden") && jinput.attr("ltype") == "radioGroup")
        {
        	jinput.ligerRadioGroup(inputOptions);
        } 
        else if(jinput.is(":hidden") && jinput.attr("ltype") == "checkboxGroup")
        {
        	jinput.ligerCheckBoxGroup(inputOptions);
        } 
        else if (ltype=="spinner")
        {
            jinput.ligerSpinner(inputOptions);
        }
		else if (ltype=="textarea")
        {
        	if(inputOptions&&inputOptions.disabled)
        	jinput.attr('readOnly','true');
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if (p.fields && p.fields.length)
            {
                ...
                $(p.fields).each(function (index, field)
                {
                    ...
                    {	
                    	//modify by caiqy at 2012-09-25
                    	//-if hidden type input has the 'attr' param,add the 'attr' into input content  
                    	var inputContent = '<input type="hidden" id="' + name + '" name="' + name + '" ';
                    	if (field.attr)
                        {
                            for (var attrp in field.attr)
                            {
                            	var attrpTmp = attrp + '="' + field.attr[attrp] + '" ';
                            	inputContent += attrpTmp;
                            }
                        }
                    	inputContent += ' />';
                        out.push(inputContent);
                        return;
                    }
                    ...
                    if (newLine)
                    {
                        ...
                        if (field.group)
                        {
                            ...
                            out.push('">');
                            if (field.groupicon){}
                                out.push('<div class="l-icon-div"><img src="' + field.groupicon + '" /></div>');
                            out.push('<div><span>' + field.group + '</span></div></div>');
                        }
                        out.push('<ul>');
        ...
        _buliderControl: function (field)
        {
        	...
        	if( $.browser.msie && 
        	    ...
        	    out.push('<textarea style="margin-left:-6px" ');
        	}else{
        	    out.push('<textarea ');
        	}
            }
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        // edit in 2012-04-23 by caiqy
        form:null								// 以指定form作为默认参数输入
    };
 ...
 */
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm)
        {
            ...
            var clause = null;
            var loadServer = true;
            // 作为参数输入的form,仅当loadSearver为true时有效
            ...
            if (p.dataAction == "server")
            {
                ...
                if (form)
                {
                	var realForm = form[0];
                	if(realForm){
                		for(var n = 0 ; n < realForm.length ; n++){
                			// 将form中的field放入param,readOnly为true(combobox中的text域)的不要
                			if(!realForm[n].readOnly && realForm[n].value != null
                					&& realForm[n].value != ''){
                				param.push({ name: realForm[n].name,value: realForm[n].value});
                			}
                		}
                	}
                }
        ...
        _createEditor: function (editor, container, editParm, width, height)
        {
            ...
            var editorInput = editor.create(container, editParm , columnWidth);
...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _calMaxNodeWidth: function () {
        	...
        	g._calNodeWidth(g.getData(), strObj, 1);
        	strObj.remove();
        },
        _calNodeWidth: function (data, strObj, level) {
        	var g = this, p = this.options;
            $(data).each(function (i, item) {
                var width = g._getCurrentStrWidth(strObj, item.text);
                ...
                if (this.children) {
                    g._calNodeWidth(this.children, strObj, level + 1);
                }
            });
        },
        _getFont: function(){
        ...
        _setTreeEven: function ()
        {
            ...
            g.tree.click(function (e)
            {
                ...
                if($(obj).attr("tagName") == "SPAN" && $(obj).parent().hasClass("l-body")){
            ...
            if ($.fn.ligerResizable)
            {
                g.resizeable = g.window.resizable = g.window.ligerResizable({
                    ...
                    onStopResize: function (current, e)
                    {
                        ...
                        if (current.newHeight)
                            g.window.content.height(current.newHeight - 26);
