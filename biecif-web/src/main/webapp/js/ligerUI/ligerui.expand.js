...
$.ligerui.controls.ComboBox.prototype.openSelect = function(options) {
	...
	g.bind('beforeOpen', function() {
		if(options.dialogType&&options.dialogType=='icon'){
			showIconDialog();
		}else{
			show();
		}
	...
	function show() {
		BIONE.commonOpenDialog(options.title, options.dialogname,
				options.width, options.height, options.url, options.comboxName);
	}
};