...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/custrelapprovalhistory/list.json";
		initGrid();
		searchForm();
		initButtons();
	...
	function greaterThan(){
		var value1 = $('#approvalTimeBegin').val();
		var value2 = $('#approvalTimeEnd').val();
		if(value1 == "" || value2 == ""){
			return true;
		}
		var tdate;
...
// 					field : "custrel.DEST_CUST_ID"
				...
				attr : {
					op : ">=",
					field : "custrelApproval.APPROVAL_TIME"
				}
			},{
				display : "审批结束日期",
				name : "approvalTimeEnd",
				newline : false,
				type : "date",
				cssClass : "field",
				width : 180,
				...
				attr : {
					op : "=",
					field : "custrelApproval.APPROVAL_STAT"
				}
			}]
		});
	...
	function initGrid() {
		...
		grid = $("#maingrid").ligerGrid({
			...
			method : 'post',
			url : url,
			sortName : 'APPROVAL_TIME', //第一次默认排序的字段
			sortOrder : 'desc',
			toolbar : {}
		});
	}
...
// 			alert(this.custrelApprovalId);
	...
	function file_down() {
		...
		$.ajax({
			url: '${ctx}/ecif/custrelapprovalhistory/getReportFile.json?' + timestamp,
			type: 'post',
			data: {
				reportNo: '3',
				rule : ruleJson
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/custrelapprovalhistory/export.xls'
		}).css('display', 'none');
