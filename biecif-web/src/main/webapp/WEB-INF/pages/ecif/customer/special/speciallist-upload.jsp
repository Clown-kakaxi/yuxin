...
<script type="text/javascript">
	...
	$(function() {
		$("#uploader").pluploadQueue({
			...
			url : '${ctx}/ecif/customer/speciallist/startUpload',
			max_file_size : '100MB',
			unique_names : true,
			chunk_size : '10MB',
		...
		uploader.bind('FileUploaded', function(result, file, resp) {
			...
			if (resp) {
				...
				if(temp.substring(0,3) == "err"){
					//alert(temp.substring(3));
					alert("错误信息下载");
					//parent.BIONE.tip("错误信息下载");
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/customer/speciallist/export.xls?'+new Date().getTime()
		}).css('display', 'none');
