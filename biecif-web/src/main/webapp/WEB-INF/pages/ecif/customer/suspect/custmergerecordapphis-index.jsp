...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/customer/custmergerecordapphis/list.json";
		initGrid();
		searchForm();
		initButtons();
		...
		$("#search").ligerForm({
			fields : [{
				...
				options : {
					...
					url : "${ctx}/ecif/customer/custmergerecordapphis/getComBoBox.json",
					ajaxType : "get"
				},
				attr : {
					op : "=",
	...
	function RenderApprovalStatus(rowdata){
		return BIONE.paramTransformer(rowdata.approvalStat, 
				'${ctx}/ecif/customer/custmergerecordapphis/getCodeMapApprovalStatus.json', '');
	}
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/customer/custmergerecordapphis/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url: '${ctx}/ecif/customer/custmergerecordapphis/getReportFile?'+timestamp,
			type: 'get',
			data: {
				repo: "8",
				//others: others
