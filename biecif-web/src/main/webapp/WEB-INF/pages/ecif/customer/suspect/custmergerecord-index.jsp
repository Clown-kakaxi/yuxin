...
<script type="text/javascript">
	...
	function init() {
		url = "${ctx}/ecif/customer/custmergerecord/list.json";
		initGrid();
		searchForm();
		initButtons();
	...
	function RenderSuspectType(rowdata){
		return BIONE.paramTransformer(rowdata.suspectFlag, 
				'${ctx}/ecif/customer/suspectgroup/getCodeMapSuspectType.json', '');
	}
	
	function RenderMergeType(rowdata){
		return BIONE.paramTransformer(rowdata.mergeStat, 
				'${ctx}/ecif/customer/suspectgroup/getCodeMapMergeType.json', '');
	}
	
	function file_load(item) {
		// 导入
		BIONE.commonOpenDialog('导入客户合并待审批信息', 'upload', 562, 334,
					'${ctx}/ecif/customer/custmergerecord/importresult');
	}
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/customer/custmergerecord/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			url: '${ctx}/ecif/customer/custmergerecord/getReportFile?'+timestamp,
			type: 'get',
			data: {
				repo: "4",
				//others: others
