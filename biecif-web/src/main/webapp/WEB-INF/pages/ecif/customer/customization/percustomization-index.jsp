...
<script type="text/javascript">
	...
	$(function() {
		...
		$.ajax({
			cache : false,
			async : false,
			url : "${ctx}/ecif/perctzt/getColumns.json",
			type : "post",
			beforeSend : function() {
				BIONE.loading = true;
		...
		$("#search").ligerForm({
			...
			fields : [ {
				...
				options : {
					valueFieldID : "id",
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD010016",
					ajaxType : "post"
				...
				options : {
					valueFieldID : "id",
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD000002",
					ajaxType : "post"
				},
				attr : {
					op : "=",
					field : "prrcitizenship"
				}
			}, {
				display : "民族",
				name : "prrnationality",
				...
				options : {
					valueFieldID : "id",
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD010017",
					ajaxType : "post"
				},
				attr : {
					op : "=",
					field : "prrnationality"
				}
			} , {
				display : "婚姻状况",
				name : "prrmarriage",
				...
				options : {
					valueFieldID : 'id',
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD010019",
					ajaxType : "post"
				},
				attr : {
					op : "=",
				...
				display : "最高学历",
				name : "prrhighestschooling",
				newline : false,
				type : "select",
				cssClass : "field",
				options : {
					valueFieldID : "id",
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD010024",
					ajaxType : "post"
				},
				attr : {
					op : "=",
					field : "prrhighestschooling"
				}
			} , {
				display : "生命周期状态类型",
				...
				options : {
					valueFieldID : "id",
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD010151",
					ajaxType : "post"
				},
				attr : {
					op : "=",
				...
				options : {
					valueFieldID : "id",
					url : "${ctx}/ecif/perctzt/getComBoBox.json?codeType="+"XD010131",
					ajaxType : "post"
				},
				attr : {
					op : "=",
		...
		$.ajax({
			...
			async : false,
			type : 'post',
			url : "${ctx}/ecif/perctzt/getTableColumn.json",
			beforeSend : function() {
				BIONE.loading = true;
				BIONE.showLoading("正在加载数据中...");
			},
		...
		if(columns != null){
			...
			for(var j = 0; j < tables.length; j ++){
				var tableWidth = 3*tables[j].tbCnnm + "%";
				var tableStr = "{display: '"+tables[j].tbCnnm+"', width: '"+tableWidth+"', columns: [";
			...
			gridJsonStr = "{width : '100%'," + columnsJsonStr + "checkbox : false, delayLoad :false, usePage : true, isScroll : false, rownumbers : true, alternatingRow : true, colDraggable : true, dataAction : 'server', method : 'post', url : '${ctx}/ecif/perctzt/list.json', sortName : 'CST.CUST_NO', sortOrder : 'asc', toolbar : {} }";
	...
	function doDownload(file) {
		...
		var form = $('<form/>').attr({
			target: '',
			method: 'post',
			action: '${ctx}/ecif/perctzt/export.xls'
		}).css('display', 'none');
	...
	function file_down() {
		...
		$.ajax({
			cache : false,
			async : true,
			url: '${ctx}/ecif/perctzt/getExportFile.json?' + timestamp,
			type: 'post',
			data:{
				'rule':ruleJson
			},
    ...
    function setupTree(){
		...
		$.ajax({
			...
			url : "${ctx}/ecif/perctzt/setupColumns.json",
			type : "post",	
			beforeSend : function() {
				BIONE.loading = true;
    ...
    function setcolumn(){
		...
		if(columns != null){
			...
			columnsJsonStr = columnsJsonStr + cstidStr + tablesStr + "],";
			gridJsonStr = "{width : '100%'," + columnsJsonStr + "checkbox : false, delayLoad :true, usePage : true, isScroll : false, rownumbers : true, alternatingRow : true, colDraggable : true, dataAction : 'server', method : 'post', url : '${ctx}/ecif/perctzt/list.json', sortName : 'CST.CUST_NO', sortOrder : 'asc', toolbar : {} }";
			gridJson = eval("("+gridJsonStr+")"); 
			grid = $("#maingrid").ligerGrid(gridJson);
		}
    }
