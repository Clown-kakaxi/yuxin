package com.yuchengtech.emp.bione.dao;
...
public class JPABaseDAO<T, PK extends Serializable> extends SimpleJPADAO<T, PK> {
	...
	public <X> SearchResult<X> findPageWithNameParamByNativeSQL(final int firstResult,
		...
		List objectList = createNativeQueryWithNameParam(sql, values)
				.setFirstResult(firstResult).setMaxResults(maxResult)
				.getResultList();

		// 查询记录总数
		String countSql = SQLUtils.buildCountSQL(sql);
		Long totalCount = (Long) createNativeQueryWithNameParam(countSql, values)
				.getSingleResult();
