...
/**
 * @describtion: 对公非授信一次提交一次复核处理类
 *
 * @author : lhqheli (email: lhqheli@gmail.com)
 * @date : 2014年12月3日 上午10:50:39
 */
public class OrgFsx extends EChainCallbackCommon{
	...
	public void endY(EVO vo){
		try {
			...
			for (SortedMap<?, ?> item : result.getRows()) {
				...
				switch(pageItem){
					case 0:
						log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<对公非授信第一屏调用开始--"+updateFlag);
						new ComFsxFir().endY(vo,custId,updateFlag,auth);
						log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<对公非授信第一屏调用结束--"+updateFlag);
						break;
					case 1:
					case 2:
					case 3:
					case 5:
						log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<对公非授信第二屏调用开始--"+updateFlag);
						new OrgFsxSecDz().endY(vo,custId,updateFlag,auth,pageItem,modifyFlag);
						log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<对公非授信第二屏调用结束--"+updateFlag);
						break;
					case 4:
						log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<对公非授信第三屏调用开始--"+updateFlag);
						new OrgFsxTrd().endY(vo,custId,updateFlag,auth);
						log.info("<<<<<<<<<<<<<<<<<<<<<<<<<<对公非授信第三屏调用结束--"+updateFlag);
						break;
					default:
						break;
				}
