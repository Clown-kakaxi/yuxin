...
//          //system.out.printlnln(auth2.getPid());
    ...
    /**
     * 导出错误信息之前执行的方法，主要用于拼接SQL
     */
    public void prepareFalse(){}
	/**
	 * 导出错误信息
	 */
	public String exportFalse() {
		try {
			ActionContext ctx = ActionContext.getContext();
			request = (HttpServletRequest) ctx.get(ServletActionContext.HTTP_REQUEST);
			this.setJson(request.getParameter("condition"));
			...
			//processSQL();
			// 添加导出列字典映射字段
			Map<String, String> translateMap = (Map<String, String>) JSONUtil.deserialize(request.getParameter("translateMap"));
			for (String key : translateMap.keySet()) {
				if (null != translateMap.get(key) && !"".equals(translateMap.get(key))) {
					this.addOracleLookup(key, translateMap.get(key).toString());
				}
