...
@Service
public class OcrmFCiSmeApprovlEService extends CommonService {

	public OcrmFCiSmeApprovlEService(){
		JPABaseDAO<OcrmFCiMktApprovlC,Long> baseDao = new JPABaseDAO<OcrmFCiMktApprovlC,Long>();
...
//		this.em.createQuery("update OcrmFCiMktCheckC t set t.ifFourthStep = '6' where t.id = '"+scId+"'").executeUpdate();
		...
		if(oldCheck != null){
			...
			newCheck.setCustType(oldCheck.getCustType());
			newCheck.setCaFinishDate(oldCheck.getCaFinishDate());
			newCheck.setToCoDate(oldCheck.getToCoDate());
			newCheck.setVisitFactoryDate(oldCheck.getVisitFactoryDate());
			newCheck.setRequireCaseDate(oldCheck.getRequireCaseDate());
			newCheck.setRefuseDate(oldCheck.getRefuseDate());
			newCheck.setDocuCheck(oldCheck.getDocuCheck());
			this.save(newCheck);
		}
	}
