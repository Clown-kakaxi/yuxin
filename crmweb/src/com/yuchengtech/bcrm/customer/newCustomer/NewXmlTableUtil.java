...
public class NewXmlTableUtil extends CommonService{
	 ...
	 * @throws Exception
	 */
	public String reqBodyXml(List<OcrmFCiCustinfoUphi> list) throws Exception{
		...
		for(int i=0;i<list.size();i++){
			...
			Object tableName = list.get(i).getUpdateTable();
//			Object tableName = map.get("UPDATE_TABLE");
			if(tableName!=null && !tableName.equals("")){
				if(checkTable(tableName.toString())){
					if(parmMap.containsKey(tableName)){
						columnMap.put(list.get(i).getUpdateItemEn(), list.get(i).getUpdateAfCont());	
						lists.add(columnMap);
...
//				      "OCRM_F_CI_CUST_SCIENCE",
				      ...
				      ,
//                      "ACRM_F_CI_AGENTINFO",
                      "OCRM_F_CI_BELONG_CUSTMGR"
				      ,"OCRM_F_CI_BELONG_ORG"
				      ,"ACRM_F_CI_SPECIALLIST","ACRM_F_CI_PER_KEYFLAG"
                      };
		for(String strs:str){
			if(tableName!=null && strs.equals(tableName.trim())){
				return true;
...
//			tableName="perKeyFlag";
	...
	public String getColunmName(String update_item_en) throws Exception{
		try{
			@SuppressWarnings("unchecked")
			List<ReviewMapping> list1 = this.em.createQuery(
					"select origin_column from review_mapping where page_column='"+update_item_en+"'")
					.getResultList();
...
//			}
			...
			if(list1!=null && list1.size()>0){
		    	ReviewMapping map = list1.get(0);
		    	//String colunmName = (String) map.get("origin_column");
		       	String colunmName = (String) map.getOriginColumn();
		    	return colunmName;
			}
		}catch(Exception e){
			e.printStackTrace();
