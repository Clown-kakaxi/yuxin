 ...
 */
@Service
public class AcrmFCrRelatePrivyInfoTempService extends CommonService {
    public AcrmFCrRelatePrivyInfoTempService(){
    ...
    public Object save(Object obj) {
        ...
        if(relateid == null){//新增
        	...
        	acrmfcrrelateprivyinfo.setEffectDate(currDate);//设置生效日期
        	//设置申报状态
...
//    	tempRelatePartyInfo.setLastUpdateUser(auth.getUsername());
    	if (tempRelatePartyInfo != null) {
    		...
    		tempRelatePartyInfo.setRemark(acrmfcrReatepartyinfo.getRemark());
    		obj = super.save(tempRelatePartyInfo);
    	}else
    	{
			if(acrmfcrReatepartyinfo.getMainId().longValue()==0){
    			return null;
			}else{
    		AcrmFCrRelatePrivyInfo temprelateprivyinfo1 = em.find(AcrmFCrRelatePrivyInfo.class,acrmfcrReatepartyinfo.getRelateId()); //从数据库中获取关联方信息
    		acrmfcrReatepartyinfo.setCancelState(temprelateprivyinfo1.getCancelState());//设置注销状态
    		acrmfcrReatepartyinfo.setMainId(temprelateprivyinfo1.getMainId());
    		acrmfcrReatepartyinfo.setEffectDate(currDate);//设置生效日期 
    		obj = super.save(acrmfcrReatepartyinfo);
			}
    	}
		return obj ;
	}
