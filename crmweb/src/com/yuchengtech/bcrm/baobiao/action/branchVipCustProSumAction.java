...
@Action(value="/branchVipCustProSum", results={
    @Result(name="success", type="json"),
})
public class branchVipCustProSumAction extends CommonAction{
	@Autowired
	@Qualifier("dsOracle")	
	private DataSource dsOracle;   
 	public void prepare() {
		// TODO Auto-generated method stub
        StringBuilder sb = new StringBuilder("SELECT A.MONTH,A.ORG_NAME2,A.ORG_ID2," +
        		...
        		" FROM ACRM_M_VIP_PROD_HAVING_RATE A WHERE A.ORG_ID4 IS NULL"  );
        for(String key:this.getJson().keySet()){
            if(null!=this.getJson().get(key)&&!this.getJson().get(key).equals("")){
                if(key.equals("MONTH"))
                    sb.append(" and A.MONTH = '"+this.getJson().get(key)+"'");
                else if(key.equals("instncode"))
                    sb.append(" and A.ORG_ID2 = '"+this.getJson().get(key)+"'");
