...
@Action(value = "/ocrmFWpScheduleM", results = { @Result(name = "success", type = "json")})
public class OcrmFWpScheduleMonthAction extends CommonAction {
	...
	private DataSource ds;
	 ...
	 @Autowired
	 private  OcrmFWpScheduleMonthService  service;
	 @Autowired
	 private  OcrmFWpScheduleService  sservice;
	 AuthUser auth=(AuthUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal(); 
	 @Autowired
		public void init(){
		  	model = new OcrmFWpScheduleMonth(); 
	...
	public void prepare() {
    	 ...
    	 StringBuilder sb = new StringBuilder(" select v.*,s.user_id as VISITOR,a.user_name as arange_name" +
    	 		" from OCRM_F_WP_SCHEDULE_MONTH v,admin_auth_account a,OCRM_F_WP_SCHEDULE s where s.sch_id = v.sch_id and v.ARANGE_ID = a.account_name and v.SCH_ID='"+schId+"' ");
     	...
     	addOracleLookup("STAT", "STAT_PLAN");
		SQL=sb.toString();
		datasource = ds;
	}
	//保存
	public void save(){
    	 ...
    	 if(((OcrmFWpScheduleMonth)model).getWId() == null ){//新增的时候
    		 ((OcrmFWpScheduleMonth)model).setSchId(BigDecimal.valueOf(Long.parseLong(schId)));
        	 if(userId.equals(auth.getUserId())){//自己创建任务
        		 ((OcrmFWpScheduleMonth)model).setStat("ap");
        	 }else{//为他人创建任务
        		 ((OcrmFWpScheduleMonth)model).setStat("xf");
        	 }
        	 service.save(model);
        	 //修改日程主表的数目值
        	 OcrmFWpSchedule sch = (OcrmFWpSchedule)sservice.find(Long.parseLong(schId));
        	 sch.setMonthCount(sch.getMonthCount().add(BigDecimal.ONE));
        	 sservice.save(sch);
