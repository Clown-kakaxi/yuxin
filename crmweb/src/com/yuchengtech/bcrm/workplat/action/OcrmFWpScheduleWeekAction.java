...
@Action(value = "/ocrmFWpScheduleW", results = { @Result(name = "success", type = "json")})
public class OcrmFWpScheduleWeekAction extends CommonAction {
	...
	private DataSource ds;
	 ...
	 @Autowired
	 private  OcrmFWpScheduleWeekService  service;
	 @Autowired
	 private  OcrmFWpScheduleService  sservice;
	 AuthUser auth=(AuthUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal(); 
	 @Autowired
		public void init(){
		  	model = new OcrmFWpScheduleWeek(); 
	...
	public void prepare() {
    	 ...
    	 StringBuilder sb = new StringBuilder(" select v.*,s.user_id as VISITOR,a.user_name as arange_name" +
    	 		" from OCRM_F_WP_SCHEDULE_WEEK v,admin_auth_account a,OCRM_F_WP_SCHEDULE s where s.sch_id = v.sch_id and v.ARANGE_ID = a.account_name and v.SCH_ID='"+schId+"'");
	...
	public void save(){
    	 ...
    	 if(((OcrmFWpScheduleWeek)model).getWId() == null ){//新增的时候
    		 ((OcrmFWpScheduleWeek)model).setSchId(BigDecimal.valueOf(Long.parseLong(schId)));
        	 if(userId.equals(auth.getUserId())){//自己创建任务
        		 ((OcrmFWpScheduleWeek)model).setStat("ap");
        	 }else{//为他人创建任务
        		 ((OcrmFWpScheduleWeek)model).setStat("xf");
        	 }
        	 service.save(model);
        	 //修改日程主表的数目值
        	 OcrmFWpSchedule sch = (OcrmFWpSchedule)sservice.find(Long.parseLong(schId));
        	 sch.setWeekCount(sch.getWeekCount().add(BigDecimal.ONE));
        	 sservice.save(sch);
