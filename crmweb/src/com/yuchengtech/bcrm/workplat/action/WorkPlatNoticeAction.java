...
public class WorkPlatNoticeAction extends CommonAction{
...
//    }
    ...
    public DefaultHttpHeaders create() {
    	ActionContext ctx = ActionContext.getContext();
  		...
  		wn.setReceiveOrg(request.getParameter("receiveOrgName"));
       ...
       if(null!=wnc&&wnc.size()>0){
        	...
        	if(methodNs!=null){
        		if(methodNs.toLowerCase().equals("delete")){
        		  String[] strings = request.getParameter("noticeId").split(",");
        		  for(String s : strings){
        			  long noticeId = Long.parseLong(s);
        			  ns.remove(noticeId);
        		  }
        		}else if(methodNs.toLowerCase().equals("publish")){
        			   ns.save(wn,"publish");
