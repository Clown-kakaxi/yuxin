...
@Action(value = "/ocrmFWpScheduleL", results = { @Result(name = "success", type = "json")})
public class OcrmFWpScheduleLoanCheckAction extends CommonAction {
	...
	private DataSource ds;
	 ...
	 @Autowired
	 private  OcrmFWpScheduleLoanCheckService  service;
	 @Autowired
	 private  OcrmFWpScheduleService  sservice;
	 AuthUser auth=(AuthUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal(); 
	 @Autowired
		public void init(){
		  	model = new OcrmFWpScheduleLoanCheck(); 
	...
	public void prepare() {
    	 ...
    	 StringBuilder sb = new StringBuilder(" select v.*,s.user_id as VISITOR,a.user_name as arange_name" +
    	 		" from OCRM_F_WP_SCHEDULE_LOAN_CHECK v,admin_auth_account a,OCRM_F_WP_SCHEDULE s where s.sch_id = v.sch_id and v.ARANGE_ID = a.account_name and v.SCH_ID='"+schId+"' ");
	...
	public void save(){
    	 ...
    	 if(((OcrmFWpScheduleLoanCheck)model).getLId() == null ){//新增的时候
    		 ((OcrmFWpScheduleLoanCheck)model).setSchId(BigDecimal.valueOf(Long.parseLong(schId)));
        	 if(userId.equals(auth.getUserId())){//自己创建任务
        		 ((OcrmFWpScheduleLoanCheck)model).setStat("ap");
        	 }else{//为他人创建任务
        		 ((OcrmFWpScheduleLoanCheck)model).setStat("xf");
        	 }
        	 service.save(model);
        	 //修改日程主表的数目值
        	 OcrmFWpSchedule sch = (OcrmFWpSchedule)sservice.find(Long.parseLong(schId));
        	 sch.setLoanCheckCount(sch.getLoanCheckCount().add(BigDecimal.ONE));
        	 sservice.save(sch);
