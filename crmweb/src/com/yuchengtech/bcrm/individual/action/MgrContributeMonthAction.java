...
/**
 * @describtion: 客户经理贡献度连续12个月趋势
 *
 * @author : lhqheli (email: lhqheli@gmail.com)
 * @date : 2014年7月24日 上午10:54:48
 */
@Action("/mgrContributeMonth")
public class MgrContributeMonthAction extends CommonAction{
	...
	public void prepare(){
		...
		SQL ="SELECT T.COUNT_YEAR,T.COUNT_MONTH, ROUND(COALESCE(SUM(T.CONTRIBUTE),0)/10000,2) AS CONTRIBUTE FROM ACRM_F_CI_MGR_CONTRIBUTE_MONTH T "
				+ " WHERE MGR_ID = '"+auth.getUserId()+ "' GROUP BY COUNT_YEAR, COUNT_MONTH " ;
		...
		try {
			...
			rs = statement.executeQuery(" SELECT t.count_year|| '.' || t.count_month from ACRM_F_CI_MGR_CONTRIBUTE_MONTH t where MGR_ID ='"+auth.getUserId()+"' GROUP BY COUNT_YEAR, COUNT_MONTH order by COUNT_YEAR, COUNT_MONTH");
			while(rs.next()){
