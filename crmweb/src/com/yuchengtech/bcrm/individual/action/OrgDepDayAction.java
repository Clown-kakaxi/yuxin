...
/**
 * @describtion: 存款余额趋势图(连续30天)
 *
 * @author : lhqheli (email: lhqheli@gmail.com)
 * @date : 2014年7月21日 上午10:56:59
 */
@Action("/orgDepDay")
public class OrgDepDayAction extends CommonAction{
	...
	public void prepare(){
		...
		SQL = "select ROUND(COALESCE(SUM(t.DEP_BAL),0)/10000,2) as DEP_BAL,ROUND(COALESCE(SUM(t.DEP_YEAR_AVG),0)/10000,2) as DEP_YEAR_AVG,t.ODS_DATE  from ACRM_F_CI_ORG_DEP_DAY t"
		...
		try {
			...
			rs = statement.executeQuery(" SELECT TO_CHAR(T.ODS_DATE,'YYYY-MM-DD') from ACRM_F_CI_ORG_DEP_DAY t where t.ORG_ID in (SELECT SYSUNIT.UNITID FROM SYS_UNITS SYSUNIT WHERE SYSUNIT.UNITSEQ LIKE '"+auth.getUnitInfo().get("UNITSEQ")+"%') group by ODS_DATE order by ODS_DATE");
			while(rs.next()){
