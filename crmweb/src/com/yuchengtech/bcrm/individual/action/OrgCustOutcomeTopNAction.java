...
/**
 * @describtion: 机构TOP10资金流出客户
 *
 * @author : lhqheli (email: lhqheli@gmail.com)
 * @date : 2014年7月18日 上午11:37:13
 */
@Action("/orgCustOutcomeTopN")
public class OrgCustOutcomeTopNAction extends CommonAction{
	...
	public void prepare() {
		...
		SQL = "SELECT * FROM (SELECT T.CUST_ID, T.CUST_NAME, T.ODS_DATE, SUM(T.BAL) AS BAL FROM ACRM_F_CI_OUTCOME_TOP T "
			+"WHERE T.ORG_ID IN (SELECT SYSUNIT.UNITID FROM SYS_UNITS SYSUNIT WHERE SYSUNIT.UNITSEQ LIKE '"+auth.getUnitInfo().get("UNITSEQ")+"%') "
			+"GROUP BY T.CUST_ID, T.CUST_NAME, T.ODS_DATE ORDER BY SUM(T.BAL) DESC) " + rownumcondition;
		datasource = ds;
	}
}
