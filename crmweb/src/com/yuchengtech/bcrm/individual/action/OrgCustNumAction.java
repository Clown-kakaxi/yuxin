 ...
 */
@Action("/orgCustNum")
public class OrgCustNumAction extends CommonAction{
	...
	public void prepare(){
		...
		SQL =/*"SELECT '客户总数' AS CUST_TYPE,COUNT(1) AS NUM FROM ACRM_F_CI_CUSTOMER C1 "
				...
				+ " AND EXISTS (SELECT 1 FROM OCRM_F_CI_BELONG_ORG ORG WHERE ORG.CUST_ID = C1.CUST_ID AND ORG.INSTITUTION_CODE IN (SELECT SYSUNIT.UNITID FROM SYS_UNITS SYSUNIT WHERE SYSUNIT.UNITSEQ LIKE '"+auth.getUnitInfo().get("UNITSEQ")+"%') )"
				+ " UNION "
				...
				+ " AND EXISTS (SELECT 1 FROM OCRM_F_CI_BELONG_ORG ORG WHERE ORG.CUST_ID = C2.CUST_ID AND ORG.INSTITUTION_CODE IN (SELECT SYSUNIT.UNITID FROM SYS_UNITS SYSUNIT WHERE SYSUNIT.UNITSEQ LIKE '"+auth.getUnitInfo().get("UNITSEQ")+"%') )"
