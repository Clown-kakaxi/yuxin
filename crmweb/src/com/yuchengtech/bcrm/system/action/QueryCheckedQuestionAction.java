...
@Action(value = "/QueryCheckedQuestion", results = { @Result(name = "success", type = "json") })
@SuppressWarnings("unchecked")

/***
 * 
 * @description 根据问卷编码查询该问卷下已选择的试题
 * @author  chenlin
 *
 * @date 2014-10-22
 */
public class QueryCheckedQuestionAction extends CommonAction {
	@Autowired
	...
	public void prepare() {
		...
		sb.append("select f.* ,(case  when f.checked is null then  0 else  1 end) is_checked"+
		  ...
		  " where t2.paper_id ="+paperId+") e on e.question_id = t1.title_id) f where f.checked is not null order by is_checked desc,question_order ");
