...
Wlj.view.CustView = Ext.extend(Ext.Panel,{
	...
	initComponent : function(){
		...
		var formtbar = [{
			...
			handler:function(){
				Ext.Ajax.request({
					url : basepath + '/mktCustManegerback!getTodayCallCount.json',
					method :'GET',
					params : {
						'custId': _this.custId
					},
					success:function(res){
						var resJson1 = Ext.decode(res.responseText);
						if(resJson1.count == 0){
							Ext.Msg.alert('提示','请先填写callreport信息！');
							return false;
						}else{
							Ext.Msg.confirm('提示', '你确定接收该客户么？',function(btn){
								if(btn == 'yes'){
									Ext.Ajax.request({
										url : basepath+ '/mktCustManegerback!acceptCus.json',
										method : 'GET',
										params : {
											 'custId' : _this.custId,//客户编号
											 'coreNo' : _this.coreNo//核心客户号
										},
										success : function(result) {
											var resJson = Ext.util.JSON.decode(result.responseText);
											if (resJson.success) {
												Ext.Msg.alert('提示',resJson.msg);
												if(_this.coreNo){
													Ext.getCmp("acceptCusBtn").hide();
													Ext.getCmp("rejectCusBtn").hide();
												}else{
													Ext.getCmp("acceptCusBtn").hide();
												}
											} else {
				...
				Ext.Ajax.request({
					...
					success:function(res){
						...
						if(resJson1.count == 0){
							...
							Ext.Msg.confirm('提示', '你确定拒绝接收该客户么？',function(btn){
							if(btn == 'yes'){
								//将OCRM_F_MK_MKT_MY_ACTI的活动执行人删除
								Ext.Ajax.request({
									url : basepath + '/mktCustManegerback!rejectCus.json',
									method : 'GET',
									params : {
										'custId' : _this.custId//客户编号
									},
									success : function(result){
										...
										if(resJson){
											if(resJson.rejectOk == 'success'){
												Ext.Msg.alert('提示','你已拒绝接受该客户，该客户将被回收！');
												Ext.getCmp("acceptCusBtn").hide();
												Ext.getCmp("rejectCusBtn").hide();
											}else{
												Ext.Msg.alert('提示',resJson.msg);
...
//					Ext.getCmp("remindType").setValue(aa.remindType);
	...
	queryViewTree : function(){
		var _this = this;
		Ext.Ajax.request({
			url : basepath + '/mktCustManegerback!queryCustViewTree.json?custId='+_this.custId,
			method:'GET',
			success:function(response){
				var nodeArra = Ext.decode(response.responseText).data;
	...
	queryMyViewTree : function(){
		var _this = this;
		Ext.Ajax.request({
			url : basepath + '/mktCustManegerback!queryMyViewTree.json?custId='+_this.custId,
			method:'GET',
			success:function(response){
	...
	queryCustBase : function(){
		var _this = this;
		Ext.Ajax.request({
			url : basepath + '/mktCustManegerback!queryCustBase.json?custId='+_this.custId,
			method:'GET',
			success:function(response){
				var data = Ext.decode(response.responseText);
				if(data){
					_this.custName = data.custName;
					_this.custType = data.custType;
					_this.coreNo = data.coreNo;
				}else{
