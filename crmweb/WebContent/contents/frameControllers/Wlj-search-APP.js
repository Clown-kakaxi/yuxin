...
/**
 * 初始化菜单信息,根据用户权限查询用户所拥有的所有菜单信息,并存在_APP对象的menus属性上
 * 同时初始化首页显示
 */
...
Wlj.search.App.prototype.bootViews = function(){
	...
	this.viewShowAdjust();
	//added by liuyx 20180208 获取保存在全局对象_APP上的菜单对应的_menuItem对象，主要是为了解决点击非菜单区域让菜单自动收起
    var mainMenuWithin = Ext.DomQuery.selectNode("div[class='main_menu_div']");
	Ext.getBody().on("click", function(e,target) {
		if(!mainMenuWithin) {
			mainMenuWithin = Ext.DomQuery.selectNode("div[class='main_menu_div']");
		}
		if(_APP._menuItem) {
			if(!e.within(mainMenuWithin)){
    			_APP._menuItem.hide();
			}
		}
	});
...
Wlj.search.App.prototype.bootHeader = function(){
	...
	//如果图标类型为普通类型，则使用Wlj-search-APP-cfg.js里配置的信息作为一级图标,metro风格的首页默认使用的是该模式,所有的一级菜单在菜单图标下面
	...
	}else if(_this.headerType === 'menu'){//图标类型为菜单,则一级图标区域显示的是一级菜单，这里另外一种展现形式
		//查询一级菜单信息
		var mainMenuCfgs = _this.getMenuDataByProperty('PARENT_ID',_this.headerRoot);
		var mainFunctions = [];
		//如果需要返回和主页这两个一级图标,则加入进来
...
//					iconcls : 'icon'+Math.ceil(Math.random()*7),
	...
	//导航区域，包括了左侧的图标和右侧的用户信息展示区域
	this.headerBar = new Wlj.widgets.search.header.HeadBar({
		y : this.headerTopMargin,//距离顶部的距离，Wlj-search-APP-cfg.js里配置的
		height : this.headerHeight,//首页主功能导航区高度，Wlj-search-APP-cfg.js里配置的
 ...
 * 生成一级菜单信息
 * @param {} id 父菜单id
 * @return {} false/Array 一级菜单信息,如果没有一级菜单信息，则返回false
 */
