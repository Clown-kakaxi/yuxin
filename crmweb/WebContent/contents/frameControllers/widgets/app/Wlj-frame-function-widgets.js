...
Wlj.frame.functions.app.widgets.SearchContainer = Ext.extend(Ext.Panel,{
	...
	onRender : function(ct, position){
		var vs =  this.vs;
		var _this = this;
		//根据页面大小计算一行放置几个控件，如果大于1024则4个，否则3个
		var columnCount = 0;
		if(vs.width > 1024){
			columnCount = 4;
		} else {
...
Wlj.frame.functions.app.widgets.ResultContainer = Ext.extend(Ext.Panel, {
	...
	validateFieldsCfg : function(){
		var dfs = this.dataFields;//创建对象之前，将配置信息fields拷贝之后赋值给dataFields的
		var targetFs = [];
	...
	/**
	 * 构建store的fields,根据fields里配置的dataType转换type
	 * @return {}
	 */
...
Ext.extend(Wlj.frame.functions.app.widgets.TitleTile, Ext.util.Observable, {
	...
	defaultDataLineRender : function(){
		...
		store.data.each(function(item, index, length){
			var oddc =_this.pipelineColor(index,item);
	...
	pipelineColor:function(index,item){
		var oddc = index % 2 ===0 ? "ygc-row-odd" : "";
		var fieldPara="";
		item.fields.each(function(tf){
			if(Ext.isFunction(tf.comFn)){
				 fieldPara=tf.comFn();
				 var fieldParaV=item.get(fieldPara);
				 if(fieldParaV=='1'){
					 oddc=  "pipeline-color-1" ;
				 }else if(fieldParaV=='2'){
					 oddc= "pipeline-color-2";
				 }else if(fieldParaV=='3'){
					 oddc=  "pipeline-color-3" ;
				 }else if(fieldParaV=='4'){
					 oddc=  "pipeline-color-4";
				 }else if(fieldParaV=='5'){
					 oddc= "pipeline-color-5";
				 }else if(fieldParaV=='6'){
					 oddc=  "pipeline-color-6";
				 }else if(fieldParaV=='7'){
					 oddc= "pipeline-color-7";
				 }
			}
...
//			if(Ext.isFunction(tf.comFn)){
//				 fieldPara=tf.comFn();
//				 var fieldParaV=record.get(fieldPara);
//				 if(fieldParaV=='1'){
//					 fData="<h1 onclick='' style='color:red' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }else if(fieldParaV=='2'){
//					 fData="<h1 onclick='' style='color:orange' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }else if(fieldParaV=='3'){
//					 fData="<h1 onclick='' style='color:yellow' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }else if(fieldParaV=='4'){
//					 fData="<h1 onclick='' style='color:green' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }else if(fieldParaV=='5'){
//					 fData="<h1 onclick='' style='color:cyan' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }else if(fieldParaV=='6'){
//					 fData="<h1 onclick='' style='color:blue' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }else if(fieldParaV=='7'){
//					 fData="<h1 onclick='' style='color:violet' >"+_this.translateFieldData(tf, record.get(tf.name))+"</h1>"
//				 }
//				
//			}else{
				fData = _this.formatFieldData(tf,_this.translateFieldData(tf, record.get(tf.name)));
//			}
