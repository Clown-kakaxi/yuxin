...
Ext.apply(Ext.form.VTypes, {
     ...
     */
	"money" : function(_v) {
		return /^([1-9][0-9]{0,9}|0)(\.[0-9]{0,2})?$/.test(_v);  
	},   
	'moneyText' : '金额类型错！！格式如10.0或0.11',  
	'moneyMask' : /[0-9\.]/i,
     ...
     */
	"dueTime" : function(_v) {
		return /^([1-9][0-9]{0,2})?$/.test(_v);  
	},   
	'dueTimeText' : '格式类型错！！只能输入三位正整数！',  
	'dueTimeMask' : /[0-9]/i,
     ...
     */
	"percent" : function(_v) {
		return /^(([1-9][0-9]{0,1}|0)(\.[0-9]{1,2})?|100)$/.test(_v);  
	},   
	'percentText' : '金额比率类型错！！格式如100或1.00,99.99',  
	'percentMask' : /[0-9\.]/i,
	
    /**
     * 担保率
     */
	"dbPercent" : function(_v) {
		return /(^([1-9][0-9]{0,2}|0)(\.[0-9]{1,2})?$)/.test(_v);  //
	},   
	'dbPercentText' : '担保率类型错！！格式如999或1.00',  
	'dbPercentMask' : /[0-9\.]/i,
    ...
    "telephone" : function(_v) {   
      ...
      * 手机或者固话
      */    
     "phoneNumber" : function(_v) {   
        return /(^\d{3}\-\d{7,8}$)|(^\d{4}\-\d{7,8}$)|(^\d{3}\d{7,8}$)|(^\d{4}\d{7,8}$)|(^\d{7,8}$)|(^1[3458][0-9]\d{8}$)/.test(_v);   
     },   
     "phoneNumberText" : "该输入项目必须是电话号码格式，例如：13485135075,0513-89500414,051389500414,89500414",   
     "phoneNumberMask" : /[0-9\-]/i,  
     ...
     'trimText' : '输入项项首项尾有空格',
     'length'  : function(v,field){
        if(!field.length){
           field.length = 20;
        }
        this.lengthText = String.format(this.lengthText,field.length);
        if(!v||v.length!=field.length){
            return false;
        }else{
            return true;
        }
    },
    'lengthText' : '格式错误，必须是{0}位字符',  
    'lengthMask' : /[a-zA-Z0-9]/i
});
