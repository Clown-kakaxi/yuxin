...
Ext.onReady(function() {
	...
	function changePage(btn) {
		...
		// mktAssuResloveInfoPanel.layout.activeItem.id//获取当前步骤对象的ID，如info1
		var index = Number(mktAssuResloveInfoPanel.layout.activeItem.id
				.substring(4));// 获取步骤序号
		if (btn.text == '上一步') {
			// 点击“上一步”按钮的处理
			index -= 1;
		...
		if (index == 1) {
			mktAssuResloveInfoPanel.buttons[0].setDisabled(true);
		} else {
			mktAssuResloveInfoPanel.buttons[0].setDisabled(false);
		}
		// 如果是第四步，那么“下一步”按钮不可用；否则，“下一步”按钮可用
		if (index == 4) {
			mktAssuResloveInfoPanel.buttons[1].setDisabled(true);
		} else {
			mktAssuResloveInfoPanel.buttons[1].setDisabled(false);
		}
		// 设置当前要显示的步骤对应的页面
		mktAssuResloveInfoPanel.layout.setActiveItem('info' + index);
	};
	...
	// 营销任务新增、修改窗口展示的from
	var mktAssuResloveInfoPanel = new Ext.Panel({
		layout : 'card',
		activeItem : 0,
		autoScroll : true,
		buttonAlign : "center",
	...
	function resolveInit() {
		...
		if (selectLength < 1) {
			...
			mktAssuInfoForm
					.getForm()
					.findField("taskParentId")
					.setValue(listPanel.getSelectionModel().selections.items[0].data.taskId);
			// 设置任务ID
			mktAssuInfoForm.getForm().findField("taskId").setValue('');
		...
		mktAssuResloveInfoPanel.buttons[1].setDisabled(false);
		// 展示第一个tab页
		mktAssuResloveInfoPanel.layout.setActiveItem('info1');
	}
	...
	function editInit() {
		...
		mktAssuInfoWindow.show();
		mktAssuResloveInfoPanel.buttons[0].setDisabled(true);// “上一步”按钮不可用
		mktAssuResloveInfoPanel.buttons[1].setDisabled(false);// “下一步”按钮可用（修改操作，下一步到最后一步的界面时，“下一步”按钮被disabled，所以此处要重新启用）
		mktAssuResloveInfoPanel.layout.setActiveItem('info1');
	};
