...
var addrTypeStore =  new Ext.data.Store({
	...
	proxy : new Ext.data.HttpProxy( {
		url : basepath + '/lookup.json?name=XD000192_CALLCENTER'
	}),
	reader : new Ext.data.JsonReader( {
		root : 'JSON'
	},['key','value'])
});
var alladdrTypeStore =  new Ext.data.Store({
	restful : true,
	autoLoad : true,
	sortInfo : {
        field:'key',
...
var addrCustInfoCm = new Ext.grid.ColumnModel([
    ...
    {dataIndex:'ADDR_TYPE_ORA',header:'地址类型',width : 150,sortable : true},
...
var addrCustInfoStore = new Ext.data.Store({
    restful:true,
    proxy: new Ext.data.HttpProxy({
        url: basepath + '/dealWithFsx!queryAddr.json?flag=callcenter',
        method:'GET'
    }),
    reader: new Ext.data.JsonReader({
        root:'json.data',
...
var addrCustInfoTbar = new Ext.Toolbar({
    items: [{
        ...
        handler: function(){
            ...
            maintainForm.getForm().findField('ADDR_TYPE').bindStore(addrTypeStore);
            maintainWindow.show();
        }
    },{
        text: '修改',
...
var contmethTypesStore =  new Ext.data.Store({
	...
	proxy : new Ext.data.HttpProxy({
		url : basepath + '/lookup.json?name=XD000193_CALLCENTER'
	}),
	reader : new Ext.data.JsonReader({
		root : 'JSON'
	},['key','value'])
});
var allcontmethTypesStore =  new Ext.data.Store({
	restful : true,
	autoLoad : true,
	sortInfo : {
        field:'key',
...
var contactInfoCm = new Ext.grid.ColumnModel([
    ...
    {dataIndex:'CONTMETH_INFO',header : '联系方式内容',width : 350,sortable : true}, 
...
var contactInfoStore = new Ext.data.Store({
    restful:true,
    proxy: new Ext.data.HttpProxy({
        url: basepath + '/acrmFCiContmethInfo.json?check=1&&flag=callcenter',
        method:'GET'
    }),
    reader: new Ext.data.JsonReader({
        root:'json.data',
...
var contactInfoTbar = new Ext.Toolbar({
    items: [{
        ...
        handler: function(){
            ...
            contactInfoForm.getForm().findField('CONTMETH_TYPE').bindStore(contmethTypesStore);
