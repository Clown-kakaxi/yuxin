 ...
 * @since 2014-07-18
 * 个金、法金的授信与非授信信息的修改功能
 * 目前授信按钮已经被屏蔽了，暂时只有非授信信息维护
 * 个金与法金使用了不同的面板，修改信息时需要注意
 */
imports([
...
var roleCond = JsContext.checkGrant('all_query')?'1':'2'; 	//个金RM授权 选择2,否则选择1（个金=2;法金=1）(1、4选1  2、4选3)
...
//数据字典定义
var lookupTypes = [{
	TYPE : 'SPECIAL',//自定义数据权限选项数据字典
	url : '/specialSearchAction!searchSpecial.json',
	key : 'KEY',
	value : 'VALUE',
	root : 'json.data'
}];
//代理人证件类型
var store_dlrIdent=new Ext.data.Store( {
	restful : true,
	autoLoad : true,
	sortInfo : {
            field:'key',
...
var indentStore = new Ext.data.Store({
	...
	proxy : new Ext.data.HttpProxy( {
		url : basepath + '/customerQueryAllNew!getIdentType.json'
	}),
	reader : new Ext.data.JsonReader( {
		root : 'JSON'
	},['key','value'])
 ...
 * 证件类型2
 */
var indent2Store = new Ext.data.Store({
	restful : true,
	autoLoad : true,
	sortInfo : {
            field:'key',
	...
	proxy : new Ext.data.HttpProxy( {
		url : basepath + '/customerQueryAllNew!getIdentType2.json'
	}),
	reader : new Ext.data.JsonReader( {
		root : 'JSON'
	},['key','value'])
...
var searchPanel = new Ext.form.FormPanel({
	...
	items : [
		...
		{xtype : 'combo',name:'IDENT_TYPE',hiddenName : 'IDENT_TYPE',fieldLabel : '证件类型',triggerAction : 'all',
			store : indent00Store,displayField : 'value',valueField : 'key',mode : 'local',resizable : true,anchor : '90%'},
 ...
 */
var beforeSetAllCustIsUpdate = function(custType,belongOrg,belongMgr,accountOrg){
	Ext.Ajax.request({
        ...
        success : function(response){
        	...
        	setAllCustIsUpdate(custType,belongOrg,belongMgr,isLock,accountOrg);
        },
 ...
 */
var setAllCustIsUpdate = function(custType,belongOrg,belongMgr,isLock,accountOrg){
	...
	// OP可以修改非授信界面，且法金客户需要判断是否OP所属分支行（根据客户归属分支行或账号分支行）
	if(updateFsxFlag && custType == '2'){
		tempFsxFlag = true;
	}else if(updateFsxFlag && custType == '1' && belongOrg.indexOf(JsContext._orgId)> -1){
		tempFsxFlag = true;
	}else if(updateFsxFlag && custType == '1' && accountOrg.indexOf(JsContext._orgId)> -1){
		tempFsxFlag = true;
	}else if(updateFsxFlag && custType == '1' && belongOrg.indexOf('500')> -1){
		tempFsxFlag = true;
	}else if(updateFsxFlag && custType == '1' && JsContext._orgId.indexOf('500')> -1){
		tempFsxFlag = true;
	}
	...
	Ext.getCmp('pub_fsx_2-41').setVisible(tempFsxFlag);//对公非授信第二屏
	Ext.getCmp('pub_fsx_2-42').setVisible(tempFsxFlag);//对公非授信第二屏
	Ext.getCmp('pub_fsx_2-43').setVisible(tempFsxFlag);//对公非授信第二屏
	...
	Ext.getCmp('per_fsx_2-41').setVisible(tempFsxFlag);//对私非授信第二屏
	Ext.getCmp('per_fsx_2-42').setVisible(tempFsxFlag);//对私非授信第二屏
	Ext.getCmp('per_fsx_2-43').setVisible(tempFsxFlag);//对私非授信第二屏
};
...
//			if(isSxCust != '1'){
//				Ext.Msg.alert('提示','该客户不是信贷客户无授信信息！');
//                return false;
//			}
...
//			if(custType == '2'){
//				searchSxPerFn();
//				infoPanel.layout.setActiveItem(3);
//			}else{
//				searchSxComFn();
//				infoPanel.layout.setActiveItem(4);
//			}
//        }
//    },'-',
    	{
        ...
        handler: function(){
            ...
            indent2Store.load({
	   			params	: {
	   				name : selectRecord.get('IDENT_TYPE')
	   			}
	   		});
	   		var fsx_LONGTERM = fsxbaseInfo.getForm().findField('LONGTERM');
	   		var fsx_LONGTERM1 = fsxbaseInfo.getForm().findField('LONGTERM1');
	   		var fsxCom_LONGTERM = fsxCombaseInfo.getForm().findField('LONGTERM');
	   		var fsxCom_LONGTERM1 = fsxCombaseInfo.getForm().findField('LONGTERM1');
	   		fsx_LONGTERM.setValue(false);
	   		fsx_LONGTERM1.setValue(false);
	   		fsxCom_LONGTERM.setValue(false);
	   		fsxCom_LONGTERM1.setValue(false);
            custId = selectRecord.data.CUST_ID;
			custName = selectRecord.data.CUST_NAME;
			custType = selectRecord.data.CUST_TYPE;
			isSxCust = selectRecord.data.IS_SX_CUST;
			...
			var tempActOrg = selectRecord.data.ACT_ORG_IDS;//账号所属机构
			var tempMgr = selectRecord.data.MGR_ID;//所属客户经理
			var tempAccountOrg=selectRecord.data.ACCOUNT_ORG;
			beforeSetAllCustIsUpdate(custType,tempCustOrg+','+tempActOrg,tempMgr,tempAccountOrg);
			//个金，XD000080
            if(custType == '2'){
...
var allCustCm = new Ext.grid.ColumnModel([
	...
	{dataIndex:'ACCOUNT_ORG',header:'开户行',width: 100,hidden:true}
]);
...
var fsxStore = new Ext.data.Store({
	...
	reader: new Ext.data.JsonReader({
		...
		'NATIONALITY','HIGHEST_SCHOOLING','HOME_TEL','UNIT_TEL','UNIT_NAME','UNIT_ADDR',
		'ANNUAL_INCOME','ANNUAL_INCOME_SCOPE','MARRIAGE','SPOUSE_NAME','SPOUSE_PHONE','SPOUSE_MOBILE','SPOUSE_ID','SPOUSE_CORE_ID',
		'SPECIAL_LIST_FLAG','ENTER_REASON','START_DATE',
		'STAFFIN','JOINT_CUST_TYPE','SWIFT','REMARK',
		'RECOMMENDER','RECOMMENDER_NAME','ORG_SUB_TYPE','IF_ORG_SUB_TYPE','EN_NAME','INOUT_FLAG','AR_CUST_FLAG','RISK_NATION_CODE','PER_CUST_TYPE','GENDER','BIRTHDAY','CITIZENSHIP','AREA_CODE','USA_TAX_IDEN_NO','BIRTHLOCALE','EMAIL','VIP_FLAG',
	...
	fsxStore.load({
		...
		callback:function(){
			if(fsxStore.getCount()!=0){
				var _store = findLookupByType('SPECIAL');
	        	_store.load({
	                params : {
	                    'custType' :custType
	                }
	            });
	        	fsxbaseInfo.getForm().findField('ORG_SUB_TYPE').bindStore(_store);
...
var fsxbaseInfo = new Ext.form.FormPanel({
	 ...
	 items:[{
	    ...
	    items:[{
				...
				layout : 'form',
				//op(全行客户查询)页面修改，新增字段
				//在客户信息管理页面增加客户信息维护的字段，包括：【民族】【学历】【家庭电话】【工作单位电话】【工作单位名称】【工作单位地址】【个人月收入】
		      	//【家庭月收入】【婚姻情况】【配偶姓名】【配偶联系电话】【配偶移动电话】【配偶对应客户号】【配偶对应核心客户号】【上本行黑名单标志】
		      	//【上黑名单原因】【上本行黑名单日期】【是否农户】
				items :[
					...
					{xtype : 'combo',name : 'IDENT_TYPE',hiddenName : 'IDENT_TYPE',fieldLabel : '<font color="red">*</font>证件类型1',store : indentStore,resizable : true,valueField : 'key',displayField : 'value',
		            ...
		            {xtype : 'combo',id : 'IDENT_TYPE1',name : 'IDENT_TYPE1',hiddenName : 'IDENT_TYPE1',fieldLabel : '证件类型2',store : indent2Store,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'
			             ...
			             layout : 'column',
		   				 items:[{
								 ...
								 layout : 'form',
								 items :[
					             		{xtype : 'datefield', name : 'IDENT_EXPIRED_DATE',fieldLabel :'<font color = "red">*</font>证件1失效日期',format:'Y-m-d',anchor : '95%',width:250,
					             		 listeners : {
					            			'change': function(obj,oldValue,newValue){
					            					var fsx_LONGTERM = fsxbaseInfo.getForm().findField('LONGTERM');
					            					if(this.value == '9999-12-31'){
					            							  fsx_LONGTERM.setValue(true);
					            						}else{
															  fsx_LONGTERM.setValue(false);
														};
					            					}
									 ...
									 items :[{
											   ...
											   listeners:{
													'check':function(obj,ischeck){
														var IDENT_EXPIRED_DATE = fsxbaseInfo.getForm().findField('IDENT_EXPIRED_DATE');
															if (ischeck == true){
		            ...
		            {xtype : 'combo',name : 'USA_TAX_FLAG',hiddenName : 'USA_TAX_FLAG',fieldLabel : '<font color="red"></font>是否美国纳税人',store : usaTaxStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',allowBlank:true},
					...
					{xtype : 'combo',name : 'AGENT_NATION_CODE',hiddenName : 'AGENT_NATION_CODE',fieldLabel : '代理人国籍',store : conStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					{xtype : 'combo',name : 'AGENT_IDENT_TYPE',hiddenName : 'AGENT_IDENT_TYPE',fieldLabel : '代理人证件类型',store : store_dlrIdent,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					...
					{xtype : 'textfield',fieldLabel : '工作单位名称',readOnly:true,cls:'x-readOnly',name : 'UNIT_NAME',anchor : '90%',maxLength:100,allowBlank:false},
					{xtype : 'textfield',fieldLabel : '工作单位地址',readOnly:true,cls:'x-readOnly',name : 'UNIT_ADDR',anchor : '90%',maxLength:200,allowBlank:false},
			    	{xtype : 'textfield',fieldLabel : '单位电话',readOnly:true,cls:'x-readOnly',name : 'UNIT_TEL',anchor : '90%',maxLength:20,allowBlank:false},
					{xtype : 'textfield',fieldLabel : '个人月收入',readOnly:true,cls:'x-readOnly',name : 'ANNUAL_INCOME',anchor : '90%',maxLength:100,allowBlank:false},
					{xtype : 'textfield',fieldLabel : '家庭月收入',readOnly:true,cls:'x-readOnly',name : 'ANNUAL_INCOME_SCOPE',anchor : '90%',maxLength:100,allowBlank:false},
					{xtype:'combo',anchor:'90%',fieldLabel:'婚姻状态',readOnly:true,cls:'x-readOnly',name:'MARRIAGE',store:marriageStore,valueField : 'key',displayField : 'value',mode : 'local',
			              typeAhead : true,forceSelection : true,triggerAction : 'all',emptyText : '未知',labelStyle:'text-align:right;',selectOnFocus : true,allowBlank:false},
					{xtype : 'customerquery',name:'SPOUSE_NAME',hiddenName : 'SPOUSE_NAME',fieldLabel : '配偶姓名',searchType:'ALLORG',singleSelect:true,anchor : '90%',maxLength:20},
				//	{xtype : 'combo',name : 'USA_TAX_FLAG',hiddenName : 'USA_TAX_FLAG',fieldLabel : '<font color="red"></font>是否农户',store : usaTaxStore,resizable : true,valueField : 'key',displayField : 'value',
					//		mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',allowBlank:true},
					{xtype : 'combo',name : 'SPECIAL_LIST_FLAG',hiddenName : 'SPECIAL_LIST_FLAG',fieldLabel : '上本行黑名单标志',readOnly:true,cls:'x-readOnly',resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					{xtype : 'textarea',fieldLabel : '上黑名单原因',readOnly:true,cls:'x-readOnly',name : 'ENTER_REASON',maxLength: 100,anchor : '95%'}
					]
	      	},{
	      	//op(全行客户查询)页面修改，新增字段
			//在客户信息管理页面增加客户信息维护的字段，包括：【民族】【学历】【家庭电话】【工作单位电话】【工作单位名称】【工作单位地址】【个人月收入】
	      	...
	      	//【上黑名单原因】【上本行黑名单日期】【是否农户】
			    ...
			    items :[
			    	...
			    	{xtype : 'combo',name : 'GENDER',hiddenName : 'GENDER',fieldLabel : '<font color="red">*</font>性别',store : sexStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',allowBlank:false},
				    {xtype:'combo',anchor:'90%',fieldLabel:'民族',readOnly:true,cls:'x-readOnly',name:'NATIONALITY',store:nationStore,valueField : 'key',displayField : 'value',mode : 'local',
			              	typeAhead : true,forceSelection : true,triggerAction : 'all',emptyText : '请选择',labelStyle:'text-align:right;',selectOnFocus : true,allowBlank:false}, 
			        {xtype:'combo',anchor:'90%',fieldLabel:'最高学历',readOnly:true,cls:'x-readOnly',name:'HIGHEST_SCHOOLING',store:educationStore,valueField : 'key',displayField : 'value',mode : 'local',
				            typeAhead : true,forceSelection : true,triggerAction : 'all',emptyText : '未知',labelStyle:'text-align:right;',selectOnFocus : true,allowBlank:false}	,
			    	...
			    	{xtype : 'textfield',fieldLabel : '证件号码2',name : 'IDENT_NO1',anchor : '90%',maxLength:100},
			    	{
			             layout : 'column',
		   				 items:[{
				        		 columnWidth : .8,
								 layout : 'form',
								 items :[{xtype : 'datefield',name : 'IDENT_EXPIRED_DATE1',fieldLabel : '证件2失效日期',format:'Y-m-d',anchor : '95%',width:250,
					             		 listeners : {
					            			'change': function(obj,oldValue,newValue){
				            					var fsx_LONGTERM1 = fsxbaseInfo.getForm().findField('LONGTERM1');
				            					if(this.value == '9999-12-31'){
				            							  fsx_LONGTERM1.setValue(true);
				            						}else{
														  fsx_LONGTERM1.setValue(false);
													};
				            					}
									 ...
									 items :[{
											   ...
											   listeners:{
													'check':function(obj,ischeck){
														var IDENT_EXPIRED_DATE1 = fsxbaseInfo.getForm().findField('IDENT_EXPIRED_DATE1');
															if (ischeck == true){
																IDENT_EXPIRED_DATE1.setValue("9999-12-31");  
																IDENT_EXPIRED_DATE1.setReadOnly(true);
															}else{
																IDENT_EXPIRED_DATE1.setValue("");  
																IDENT_EXPIRED_DATE1.setReadOnly(false);
															 }
																				}
											  			 }
						    	               }
					...
					{xtype : 'combo',name : 'AR_CUST_FLAG',hiddenName : 'AR_CUST_FLAG',fieldLabel : '是否AR客户标志',store : arCustFlagStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					{xtype : 'combo',name : 'IF_ORG_SUB_TYPE',hiddenName : 'IF_ORG_SUB_TYPE',fieldLabel : '是否自贸区客户',store : arCustFlagStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					{xtype : 'combo',name : 'ORG_SUB_TYPE',hiddenName : 'ORG_SUB_TYPE',fieldLabel : '<font color=red>*</font>特殊监管区',store : orgSubTypeStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',allowBlank:false,listeners:{
		                    select: function() {
		                        ...
		                        if(value=='K'  ){
		                        	fsxbaseInfo.getForm().findField('IF_ORG_SUB_TYPE').setValue('1');
		                        }else if(value=='99'){
		                        	fsxbaseInfo.getForm().findField('IF_ORG_SUB_TYPE').setValue('');
		                        }else if(value=='J'){
		                        	fsxbaseInfo.getForm().findField('IF_ORG_SUB_TYPE').setValue('2');
		                        }else {
		                        	Ext.Msg.alert('提示','请选择个人户自贸区');
		                        	this.setValue('');
		                        	return false;
		                        }
		                     }
					...
					{xtype : 'textfield',fieldLabel : '业务邮箱',name : 'EMAIL',vtype:'email',anchor : '90%',maxLength:40},
			    	...
			    	{xtype : 'textfield',fieldLabel : '住宅电话',readOnly:true,cls:'x-readOnly',name : 'HOME_TEL',anchor : '90%',maxLength:20,allowBlank:false},
					{xtype : 'textfield',fieldLabel : '配偶移动电话',name : 'SPOUSE_MOBILE',anchor : '90%',maxLength:20,allowBlank:false},
					{xtype : 'textfield',fieldLabel : '配偶联系电话',name : 'SPOUSE_PHONE',anchor : '90%',maxLength:20,allowBlank:false},
					{xtype : 'textfield',name : 'SPOUSE_ID', fieldLabel : '配偶客户号',anchor : '90%'},
					{xtype : 'textfield',name : 'SPOUSE_CORE_ID', fieldLabel : '配偶核心客户号',readOnly:true,cls:'x-readOnly',anchor : '90%'},
					{xtype : 'datefield',name : 'START_DATE',fieldLabel : '上本行黑名单日期',readOnly:true,cls:'x-readOnly',format:'Y-m-d',anchor : '90%'}
...
//	}
	...
	//2.2、证件2类型：证件1类型为“台湾居民身份证”时，证件2类型只能为“台湾同胞来往内地通行证”、“临时台胞证”、“旅行证件”
	if(tempJson2.IDENT_TYPE == 'X2' && (tempJson2.IDENT_TYPE1 != '6' && tempJson2.IDENT_TYPE1 != 'X24'&& tempJson2.IDENT_TYPE1 != 'X3')){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“台湾居民身份证”时，证件2类型只能为“台湾同胞来往内地通行证”、“临时台胞证”、“旅行证件”！');
		return false;
	}
	//2.2.1、证件2类型为“台湾同胞来往内地通行证”、“临时台胞证”、“旅行证件”时，证件1类型只能为“台湾居民身份证”！'
	if((tempJson2.IDENT_TYPE1 == '6' || tempJson2.IDENT_TYPE1 == 'X24'|| tempJson2.IDENT_TYPE1 == 'X3')&& tempJson2.IDENT_TYPE != 'X2' ){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件2类型为“台湾同胞来往内地通行证”、“临时台胞证”、“旅行证件”时，证件1类型只能为“台湾居民身份证”！');
		return false;
	}
	//2.2.2、证件2类型为“港澳居民来往内地通行证”时，证件1类型只能为“港澳居民身份证”！'
	if(tempJson2.IDENT_TYPE1 == '5'&& tempJson2.IDENT_TYPE != 'X1' ){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件2类型为“港澳居民来往内地通行证”时，证件1类型只能为“港澳居民身份证”！');
		return false;
	}
	//2.3、证件2类型：证件1类型为“港澳居民身份证”时，证件2类型只能为“港澳居民来往内地通行证”
	if(tempJson2.IDENT_TYPE == 'X1' && (tempJson2.IDENT_TYPE1 != '5' )){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“港澳居民身份证”时，证件2类型只能为“港澳居民来往内地通行证”！');
		return false;
	}
	...
	if(tempJson2.IF_ORG_SUB_TYPE == '1' && (tempJson2.ORG_SUB_TYPE=='' || (tempJson2.ORG_SUB_TYPE!='K' && tempJson2.ORG_SUB_TYPE!='I' && tempJson2.ORG_SUB_TYPE!='M'))){
		Ext.Msg.alert('提示','若是自贸区客户，请在特殊监管区中选择自贸区！');
		return false;
	}
	return true;
};
...
//	}
	if(keyflag){
		addFieldFn(perModel,tempCustId,fsxbaseInfo.getForm().findField('KEYFLAG_CUST_ID').getValue(),tempCustId
			,'KEYFLAG_CUST_ID','1','1');
	}
	if(serviceflag){
		addFieldFn(perModel,tempCustId,tempCustId
...
var fsxperLists = new Ext.Panel({
	...
	buttons:[{
		...
		handler:function(){
		    ...
		    var identType = fsxbaseInfo.getForm().findField("IDENT_TYPE").getValue();
		    var identType1 = fsxbaseInfo.getForm().findField("IDENT_TYPE1").getValue();
		    
			if(identType == "X2" ){
				if(identType1 == ''){
				Ext.Msg.alert('提示','校验失败，【证件类型1】选择台湾身份证时,【证件类型2】必填！');
				return false;
				}
			}else if(identType == "X1"){
				if(identType1 == ''){
				Ext.Msg.alert('提示','校验失败，【证件类型1】选择港澳身份证时,【证件类型2】必填!');
				return false;
				}
			}
		    if(!validFsxPerFn()){
	 	...
	 	items : [{
		    ...
		    items:[{
					...
					items :[
			            ...
			            {xtype : 'combo',name : 'IDENT_TYPE1',hiddenName : 'IDENT_TYPE1',fieldLabel : '证件类型2',store : indent2Store,resizable : true,valueField : 'key',displayField : 'value',
							mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
		    ...
		    items:[{
				...
				items :[
					{xtype : 'combo',name : 'PO_MARRIAGE',hiddenName : 'PO_MARRIAGE',fieldLabel : '<font color=red>*</font>婚姻状况',store : marriageStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',allowBlank:false},
					{xtype : 'combo',name : 'PO_IDENT_TYPE',hiddenName : 'PO_IDENT_TYPE',fieldLabel : '配偶证件类型',store : indent00Store,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
...
var fsxComStore = new Ext.data.Store({
	...
	reader: new Ext.data.JsonReader({
		...
		'IDENT_ID','IDENT_TYPE','IDENT_NO','IDENT_EXPIRED_DATE',
		'IDENT_ID1','IDENT_TYPE1','IDENT_NO1','IDENT_EXPIRED_DATE1',
		'IDENT_ID2','IDENT_TYPE2','IDENT_NO2','IDENT_EXPIRED_DATE2','BUSI_LIC_NO',
		'RECOMMENDER','RECOMMENDER_NAME','EN_NAME','INOUT_FLAG','AR_CUST_FLAG','RISK_NATION_CODE','BUILD_DATE','ORG_TYPE','USA_TAX_FLAG','HQ_NATION_CODE','STAFFIN','SWIFT','JOINT_CUST_TYPE','ORG_SUB_TYPE','IF_ORG_SUB_TYPE',
	...
	fsxComStore.load({
		...
		callback:function(){
			if(fsxComStore.getCount()!=0){
	        	...
	        	fsxCombaseInfo.getForm().findField('ORG_SUB_TYPE').bindStore(_store);
				fsxCombaseInfo.getForm().loadRecord(fsxComStore.getAt(0));	
				//判断【是否台资企业】栏位是否已op复核，liuming 20170612
				Ext.Ajax.request({
			    	url : basepath + '/dealWithFsx!isOp.json',
			        method : 'GET',
			        params : {custId: custId},
			        success : function(response){
			        	var ret = Ext.decode(response.responseText);
			        	var isOp = false;
			        	if(ret.json.data.length>0){
			        		isOp = true;
			        	}
			        	if((fsxCombaseInfo.getForm().findField('IS_TAIWAN_CORP').getValue() == 'Y' 
							|| fsxCombaseInfo.getForm().findField('IS_TAIWAN_CORP').getValue() == 'N') && isOp){
							fsxCombaseInfo.getForm().findField('IS_TAIWAN_CORP').setReadOnly(true);
							fsxCombaseInfo.getForm().findField('IS_TAIWAN_CORP').addClass('x-readOnly');
						}else{
							fsxCombaseInfo.getForm().findField('IS_TAIWAN_CORP').setReadOnly(false);
							fsxCombaseInfo.getForm().findField('IS_TAIWAN_CORP').removeClass('x-readOnly');
						}
			        },
			        failure : function(response){
			        }
	    ...
	    items:[{
				...
				items :[
					...
					{xtype : 'textfield',fieldLabel : '<font color="red">*</font>中文名',name : 'CUST_NAME',anchor : '90%',allowBlank:false,maxLength:80},
					{
						xtype : 'combo',
						name : 'IDENT_TYPE',
						hiddenName : 'IDENT_TYPE',
						fieldLabel : '<font color="red">*</font>证件类型1',
						store : indentStore,
						resizable : true,
						valueField : 'key',
						displayField : 'value',
						...
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
		            {
			             layout : 'column',
		   				 items:[{
								 ...
								 layout : 'form',
								 items :[
					             		{xtype : 'datefield',name : 'IDENT_EXPIRED_DATE',fieldLabel :'<font color = "red">*</font>证件1失效日期',format:'Y-m-d',anchor : '95%',allowBlank:false,width:250,
					             		listeners : {
					            			'change': function(obj,oldValue,newValue){
					            					var fsxCom_LONGTERM = fsxCombaseInfo.getForm().findField('LONGTERM');
					            					if(this.value == '9999-12-31'){
					            							  fsxCom_LONGTERM.setValue(true);
					            						}else{
															  fsxCom_LONGTERM.setValue(false);
														};
					            					}
									 ...
									 items :[{
											   ...
											   listeners:{
													'check':function(obj,ischeck){
															var IDENT_EXPIRED_DATE = fsxCombaseInfo.getForm().findField('IDENT_EXPIRED_DATE');
															if (ischeck == true){
																IDENT_EXPIRED_DATE.setValue("9999-12-31"); 
																IDENT_EXPIRED_DATE.setReadOnly(true);
															}else{
					...
					{xtype : 'combo',name : 'IF_ORG_SUB_TYPE',hiddenName : 'IF_ORG_SUB_TYPE',fieldLabel : '是否自贸区客户',store : arCustFlagStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					{xtype : 'combo',name : 'ORG_SUB_TYPE',hiddenName : 'ORG_SUB_TYPE',fieldLabel : '特殊监管区',width: 200,valueField : 'key',displayField : 'value',
							store : orgSubTypeStore,
							mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',listeners:{
		                    select: function() {
		                    	var value=this.value;
		                        if(value=='E' || value=='F' || value=='G' || value=='H' || value=='I'  || value=='M'){
		                        	fsxCombaseInfo.getForm().findField('IF_ORG_SUB_TYPE').setValue('1');
		                        }else if(value=='99'){
		                        	fsxCombaseInfo.getForm().findField('IF_ORG_SUB_TYPE').setValue('');
		                        }else if (value=='J' || value=='K'){
		                        	Ext.Msg.alert('提示','请选择企业监管区');
		                        	this.setValue('');
		                        	return false;
		                        }else{
		                        	fsxCombaseInfo.getForm().findField('IF_ORG_SUB_TYPE').setValue('2');
		                        }
		                     }
		                 }},
					...
					{xtype : 'textfield',fieldLabel : '机构信用代码',name : 'CREDIT_CODE',anchor : '90%',vtype:'length',length:18},
					{xtype : 'textfield',fieldLabel : 'Email',name : 'ORG_EMAIL',anchor : '90%',maxLength:40},
					{xtype : 'textfield',fieldLabel : '代理人户名',name : 'AGENT_NAME',anchor : '90%',maxLength:40},
		            {xtype : 'combo',name : 'AGENT_IDENT_TYPE',hiddenName : 'AGENT_IDENT_TYPE',fieldLabel : '代理人证件类型',store : store_dlrIdent,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%'},
					{xtype : 'textfield',fieldLabel : '代理人证件号码',name : 'AGENT_IDENT_NO',anchor : '90%',maxLength:40},
					{xtype : 'textfield',fieldLabel : '统一社会信用代码',name : 'BUSI_LIC_NO',anchor : '90%',maxLength:40},
			    	...
			    	{xtype : 'textfield',fieldLabel : '证件号码2',name : 'IDENT_NO1',anchor : '90%',maxLength:50},
			    	{
			             layout : 'column',
		   				 items:[{
								 ...
								 layout : 'form',
								 items :[
					             		{xtype : 'datefield',name : 'IDENT_EXPIRED_DATE1',fieldLabel : '证件2失效日期',format:'Y-m-d',anchor : '95%',width:250,
					             		 listeners : {
					            			'change': function(obj,oldValue,newValue){
					            					var fsxCom_LONGTERM1 = fsxCombaseInfo.getForm().findField('LONGTERM1');
					            					if(this.value == '9999-12-31'){
					            							  fsxCom_LONGTERM1.setValue(true);
					            						}else{
															  fsxCom_LONGTERM1.setValue(false);
														};
					            					}
									 ...
									 items :[{
											   ...
											   listeners:{
													'check':function(obj,ischeck){
															var IDENT_EXPIRED_DATE1 = fsxCombaseInfo.getForm().findField('IDENT_EXPIRED_DATE1');
															if (ischeck == true){
																IDENT_EXPIRED_DATE1.setValue("9999-12-31");  
																IDENT_EXPIRED_DATE1.setReadOnly(true);
															}else{
...
var fsxComThreePanel = new Ext.form.FormPanel({
	 ...
	 items:[{
	    ...
	    items:[{
				...
				items :[
					{xtype: 'textfield',name : 'CUST_ID', fieldLabel : '客户编号',readOnly:true,cls:'x-readOnly',anchor : '90%'},
					{xtype : 'combo',name : 'IDENT_TYPE',hiddenName : 'IDENT_TYPE',fieldLabel : '证件类型1',store : indentStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',readOnly:true,cls:'x-readOnly'},
		            {xtype : 'combo',name : 'IDENT_TYPE1',hiddenName : 'IDENT_TYPE1',fieldLabel : '证件类型2',store : indent2Store,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',readOnly:true,cls:'x-readOnly'},
			    	...
			    	{xtype : 'combo',name : 'LEGAL_REPR_IDENT_TYPE',hiddenName : 'LEGAL_REPR_IDENT_TYPE',fieldLabel : '法人证件类型',store : indent00Store,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',disabled:true,cls:'x-readOnly'},
		            ...
		            {xtype : 'combo',name : 'ENT_SCALE_CK',hiddenName : 'ENT_SCALE_CK',fieldLabel : '公司规模',store : entScaleRhStore,resizable : true,valueField : 'key',displayField : 'value',
						mode : 'local',forceSelection : true,triggerAction : 'all',anchor : '90%',editable:false,allowBlank:false}
		       ]
	      	},{
	       		columnWidth : .5,
			    layout : 'form',
			    items :[
			    	...
			    	//modify by liuming 20170828 OP不需要维护员工人数,设置为只读
			    	{xtype : 'numberfield',fieldLabel : '员工人数',name : 'EMPLOYEE_SCALE',minValue:'0',anchor : '90%',maxValue:99999999999,readOnly:true,cls:'x-readOnly'},
			    	{
			             layout : 'column',
		   				 items:[{
				        		 columnWidth : .8,
								 layout : 'form',
								 items :[
					             		{xtype : 'datefield',name : 'TAX_IDENT_EXPIRED_DATE',fieldLabel : '税务登记证失效日期',format:'Y-m-d',anchor : '95%',width:250,
					             		 listeners : {
					            			'change': function(obj,oldValue,newValue){
					            					var fsxThree_LONGTERM = fsxComThreePanel.getForm().findField('LONGTERM3');
					            					if(this.value == '9999-12-31'){
					            							  fsxThree_LONGTERM.setValue(true);
					            						}else{
															  fsxThree_LONGTERM.setValue(false);
														};
					            					}
					            			}
									 ...
									 items :[{
											   ...
											   listeners:{
													'check':function(obj,ischeck){
														var IDENT_EXPIRED_DATE3 = fsxComThreePanel.getForm().findField('TAX_IDENT_EXPIRED_DATE');
															if (ischeck == true){
																IDENT_EXPIRED_DATE3.setValue("9999-12-31");  
																IDENT_EXPIRED_DATE3.setReadOnly(true);
															}else{
																IDENT_EXPIRED_DATE3.setValue("");  
																IDENT_EXPIRED_DATE3.setReadOnly(false);
															 }
																				}
											  			 }
						    	               }
	...
	{xtype : 'textfield',fieldLabel : '经营ID',name : 'BUSI_CUST_ID',hidden:true}
	 ...
	 buttons:[{
		...
		hidden:true,
		handler:function(){
			var identType = fsxCombaseInfo.getForm().findField("IDENT_TYPE").getValue();
			var identType1 = fsxCombaseInfo.getForm().findField("IDENT_TYPE1").getValue();
			if(identType == "X2" ){
				if(identType1 == ''){
				Ext.Msg.alert('提示','校验失败，【证件类型1】选择台湾身份证时,【证件类型2】必填！');
				return false;
			...
			if (!fsxComThreePanel.getForm().isValid()) {
		         Ext.MessageBox.alert('提示','校验失败，非授信第三屏输入有误或存在漏输入项,请检查输入项');
		         return false;
		    }
			...
			var fsxSec_4 = window.getFsxSecComModel_4();//股东信息
			var fsxThird = getFsxThirdComModel();
			if(fsxFirst.length <1 && fsxSec_1.length < 1 && fsxSec_2.length < 1 && fsxSec_3.length < 1 && fsxSec_4.length < 1 && fsxThird.length < 1){
				Ext.Msg.alert('提示', '信息未作任何修改,不允许提交!');
				return false;
			}
			debugger;
...
var validFsxComFirstFn = function(){
	...
	if(tempJson2.BUSI_LIC_NO!=null && tempJson2.BUSI_LIC_NO!=''){
	   ...
	   if(tempJson2.BUSI_LIC_NO.length!=18){
		   Ext.Msg.alert('提示','统一社会信用码必须为18位字符');
		   return false;
	   }else if(!validateUnicode(tempJson2.BUSI_LIC_NO)){
		   Ext.Msg.alert('提示','统一社会信用码不符合校验规则');
		   return false;
	   }else if(tempJson2.BUSI_LIC_NO.length==18){
		   if(tempJson2.IDENT_TYPE=='20'){
			   var IDENT_NO=tempJson2.IDENT_NO.replace(/-/g,'');
			...
			if(tempJson2.IDENT_TYPE1=='20'){
				var IDENT_NO1=tempJson2.IDENT_NO1.replace(/-/g,'');
				if(IDENT_NO1!=tempJson2.BUSI_LIC_NO.substr(8,9)){
					Ext.MessageBox.confirm(   
			                "提示"  
			               ,"境内组织机构代码与社会统一信用代码9-17位不匹配，是否将其自动更改？"  
			               ,function( button ){  
			                   if( button == 'yes'){  
			                	   var IDENT_NO1_temp=tempJson2.BUSI_LIC_NO.substr(8,8)+'-'+tempJson2.BUSI_LIC_NO.substr(16,1);
			                	   fsxCombaseInfo.getForm().findField('IDENT_NO1').setValue(IDENT_NO1_temp);
			                   } else if(button == 'no'){
			                	   return false;
			                   } else{
			                	   return false;
			...
			if((tempJson2.IDENT_TYPE!='20') && (tempJson2.IDENT_TYPE1=='' || tempJson2.IDENT_TYPE1==null) ){
				fsxCombaseInfo.getForm().findField('IDENT_TYPE1').setValue('20');
				var IDENT_NO1_temp=tempJson2.BUSI_LIC_NO.substr(8,8)+'-'+tempJson2.BUSI_LIC_NO.substr(16,1);
				fsxCombaseInfo.getForm().findField('IDENT_NO1').setValue(IDENT_NO1_temp);
			}
	   }
	...
	//modified by ganyu 20171102 下面的写法类型错乱了，不正确
	//2.2、证件2类型：证件1类型为“台湾居民身份证”时，证件2类型只能为“台湾同胞来往内地通行证”、“临时台胞证”
	/*if(tempJson2.IDENT_TYPE == 'X1' && (tempJson2.IDENT_TYPE1 != '6' && tempJson2.IDENT_TYPE1 != 'X24' && tempJson2.IDENT_TYPE1 != 'X3')){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“台湾居民身份证”时，证件2类型只能为“台湾同胞来往内地通行证”、“临时台胞证”、“旅行证件”！');
		return false;
	}*/
	//2.3、证件2类型：证件1类型为“港澳居民身份证”时，证件2类型只能为“港澳居民来往内地通行证”、“旅行证件”
	/*if(tempJson2.IDENT_TYPE == 'X2' && (tempJson2.IDENT_TYPE1 != '5')){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“港澳居民身份证”时，证件2类型只能为“港澳居民来往内地通行证”！');
		return false;
	}*/
	...
	//2.5、证件1类型：证件1类型为“台湾同胞来往内地通行证”、“临时台胞证”、“港澳居民来往内地通行证”时，证件2失效日期必须输入
	if((tempJson2.IDENT_TYPE != 'X2' && tempJson2.IDENT_TYPE != 'X1') && tempJson2.IDENT_EXPIRED_DATE == ''){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型不为“台湾居民身份证”、“港澳居民身份证”时，证件1失效日期必须输入！');
		return false;
	}
	...
	//7.2境内/外标志--:证件1类型为“境外登记证件代码(赋码)”、“台湾居民身份证”、“港澳居民身份证”、“境外居民护照”时，境内/外标志只能选择境外
	if((tempJson2.IDENT_TYPE == '2X' || tempJson2.IDENT_TYPE == 'X2' || tempJson2.IDENT_TYPE == 'X1'
		|| tempJson2.IDENT_TYPE == '2' || tempJson2.IDENT_TYPE == 'X3') && tempJson2.INOUT_FLAG != 'F'){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“境外登记证件代码(赋码)”、“台湾居民身份证”、“港澳居民身份证”、“境外居民护照”、“旅行证件”时，境内/外标志只能选择境外！');
		return false;
	...
	/*//add by liuming 20170828 境内/外标志为“境内”时，机构信用代码必填
    if(tempJson2.INOUT_FLAG == 'D' && (tempJson2.CREDIT_CODE).trim() ==''){
    	Ext.Msg.alert('提示','非授信第一屏校验失败，境内/外标志为“境内”时,机构信用代码必须输入！');
		return false;
    }*/
	...
	//针对OP使用的全行客户查询中， 证件类型为‘境外登记证件代码’，且境内外标志位为‘境外’的企业客户，
    //所对应证件号码必须输为9位数字， 如其同时填写了obu code栏位， 则证件号码栏位必须与obu code栏位保持一致。
	//2015-04-21
	if(tempJson2.IDENT_NO2!=null && tempJson2.IDENT_NO2!=''){
	   if(tempJson2.IDENT_NO2.length!=9){
	   	Ext.Msg.alert('提示','obu code必须为9位字符');
		return false;
	   }
	}
	if(tempJson2.IDENT_TYPE == '2X' && tempJson2.INOUT_FLAG == 'F' && tempJson2.IDENT_NO.length != 9){
		Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“境外登记证件代码”境内外标志位“境外”时，对应证件号码必须输为9位字符');
		return false;
	}else if(tempJson2.IDENT_TYPE == '2X' && tempJson2.INOUT_FLAG == 'F' && tempJson2.IDENT_NO.length == 9 
				&& (tempJson2.IDENT_NO2 != '' && tempJson2.IDENT_NO2 != null)){
		if(tempJson2.IDENT_NO != tempJson2.IDENT_NO2){
			Ext.Msg.alert('提示','非授信第一屏校验失败，证件1类型为“境外登记证件代码”境内外标志位“境外”时，对应证件号码必须输为9位数字；若同时填写了obu code栏位， 则证件号码栏位必须与obu code栏位保持一致');
			return false;
		}
	}
...
var getFsxFirstComModel = function(){
	...
	var keyflagArr = ['KEYFLAG_CUST_ID','IS_SEND_ECOMSTAT_FLAG','IS_FAX_TRANS_CUST','IS_TAIWAN_CORP'];//增加是否台资企业栏位
...
var infoPanel = new Ext.Panel({
	layout : 'card',
	activeItem : 0,
	//items:[全行查询表格,个金非授信信息面板2屏,法金非授信信息面板3屏,个金授信信息面板,法金授信信息面板]
	items:[allCustGrid,fsxPerInfo,fsxComInfo,sxPerInfo,sxComInfo]
});
