...
var viewshow = function(theview){
	if(theview._defaultTitle == '菜单功能栏'){
		Ext.Ajax.request({//重新加载菜单树
			...
			success : function(response) {
				...
				menuTree.resloader.nodeArray = nodeArra;//重新获取后台数据
				menuTree.resloader.refreshCache(); // 刷新缓存
				var children = menuTree.resloader.loadAll(); //得到相应的树数据
				menuTree.root.removeAll(true); // 清掉以前的数据
				menuTree.appendChild(children);// 把数据重新填充
				menuTree.resloader.refreshCache(); // 刷新缓存
