...
var tempSearch = null;
var url = basepath + '/custTrans.json?type=mgr';
 ...
 * 移交类型
 * @type String
 * 1：对私区域内移交，2：对私跨区域移交，3：对公支行移交，4：对公区域内移交
 * 5：对公跨区域移交，6：对私主管直接移交，7：对公主管直接移交（主办），8：对公主管直接移交（协办）
 */
var applyType = '';
/**
 * 远程数据字典类型;
 * APP对象会在页面渲染之前自动请求所有数据字典项,并纳入数据字典管理器;
 * @type array[string]
 */
var lookupTypes = [
	'MAINTAIN_TYPE',//主协办类型
...
var fields = [
	...
	{name:'CUST_TYPE',text:'客户类型',translateType:'LOCAL_CUST_TYPE',searchField:true,cAllowBlank:false,readOnly:true,value:'2'},
 ...
 */
var extraParams = {
	impFlag	: "0"//导入查询标志，1：导入查询；0：普通查询
};
/**
 * 默认情况下有查询、重置、清除，此配置可以添加额外的按钮
 * @type object
 */
...
WLJUTIL.conditionButtons.search.fn = function() {
	// Ext.getCmp("CUST_TYPE").setValue('1');
	extraParams.impFlag = "0";//普通查询
	this.searchHandler();//调用内部函数触发查询
};
//为新添加的按钮注册按钮类型
...
var beforeinit = function(app){
    ...
    //为原来的onBeforeLoad、onExceptionLoad、onDataLoad创建拦截函数，以便根据数据加载是否完成控制查询按钮是否可用
...
var importSearchManager = new Com.yucheng.crm.common.ImportSearchManager({
	templateFile: 'gejinImport.xlsx',
    tradeCode   : 'importCustTransByManager',
    ...
    listeners   : {
        beforeSearch : function(){
            ...
            Ext.Ajax.request({
                url : basepath + '/custTrans!searchCustId.json',
...
var tbar = [{
	...
	handler:function(){
		if(getSelectedData() == false){
			...
			return false;
		}if(getAllSelects().length>=200){
			Ext.Msg.alert('提示','请选择记录条数少于200条！');
			return false;
		}
		applyType = '1';//对私区域内移交
		json={'transedData':[]};
	...
	text : '模板下载', 
	hidden:JsContext.checkGrant('custManagerTrans_download'),
	handler : function(){
		importSearchManager.downloadTemplate();
	}
},{
	text : '导入查询',
	hidden:JsContext.checkGrant('custManagerTrans_import'),
	handler : function(){
...
var downloadWindow = new Ext.Window({
	...
	buttons:[{
		...
		handler : function(){
			...
			var fileName = 'dskhyjImportGroup.xlsx';
		...
		handler : function(){
			var winPara = 'height=100, width=200, top=300, left=500, toolbar=no,menubar=no,'
							+ ' scrollbars=no, resizable=no,location=no, status=no';
			var fileName = 'khyjImportGroup.xlsx';
			var uploadUrl = basepath + '/TempDownload?filename=' + fileName;
			window.open(uploadUrl, '', winPara);
		}
	}]
...
var pTrans = new Ext.Panel({
	...
	buttons:[{
		...
		handler:function(){
			...
			Ext.Ajax.request({
	             url : basepath + '/custTrans!MgrTrans.json',
	             method : 'GET',
	...
	items : [ {
		...
		items : [{
			...
			items : [{xtype:'textfield',
             ...
             fieldLabel: '<font color="red">*</font>工作移交类别',
             name : 'handKind',
             hiddenName:'handKind',
             forceSelection : true,
             xtype:'combo',
			...
			items : [ {
			    name : 'handOverReason',
			    xtype : 'textarea',
			    fieldLabel : '<font color="red">*</font>工作移交原因',
			    labelStyle : 'text-align:right;',
			    allowBlank:false,
			    anchor : '97.5%'
			}]
...
var afterinit = function(app){
	...
	Ext.getCmp('trans3').hide();
	Ext.getCmp('trans4').hide();
	Ext.getCmp('trans5').hide();
...
var setsearchparams = function(){
	...
	if(type == '2'){
		Ext.getCmp('trans1').show();
		Ext.getCmp('trans2').show();
		Ext.getCmp('trans3').hide();
		Ext.getCmp('trans4').hide();
		Ext.getCmp('trans5').hide();
	}else{
