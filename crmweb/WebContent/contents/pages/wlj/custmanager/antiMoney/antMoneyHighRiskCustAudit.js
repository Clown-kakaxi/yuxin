...
var url=basepath+"/customerAntMoneyAuditQuery.json?riskLevel=H";
...
//];   
//  
//var riskgradeStore1 = new Ext.data.Store({   
//	proxy: new Ext.data.MemoryProxy(data),   
//    reader: new Ext.data.ArrayReader({}, [])   
//});   
//  
//riskgradeStore1.load();  
...
var riskgradeStore1 =  new Ext.data.Store({
	...
	proxy : new Ext.data.MemoryProxy(data101),
	reader : new Ext.data.ArrayReader({}, [{
		name : '中' // Json中的属性Key值
	}, {
		name : '高'
...
var beforeconditioninit = function(panel, app){
    ...
    /*if(JsContext.checkGrant("_fxqdqsh_h")){
       app.autoLoadGrid = false;
       app.url = app.url + "&permission=false"
...
function perFXQLevel(n){
	...
	var c=0;
					...
					var fxqHmd= fxqLevelAuditPanel.form.findField("DQSH024").getValue();
					...
					if(fxqHmd=="1"||dqsh007=="1"||fxq009=="1"){
...
function orgFXQLevel(n){
	...
	var c=0;
				...
				fxqORGLevelAuditPanel.form.findField("CUST_GRADE_CHECK").setValue("");
					//a
					var fxqHmd= fxqORGLevelAuditPanel.form.findField("DQSH024").getValue();
...
//fxqLevelAuditPanel.form.findField("CUST_GRADE_CHECK").store=riskgradeStoreH;
	

//fxqLevelAuditPanel.form.findField("CUST_GRADE_CHECK").store = riskgradeStoreHM;
//	fxqLevelAuditPanel.form.findField("CUST_GRADE_CHECK").store = riskgradeStoreH;
...
var tbar =[{
    text:'反洗钱风险等级审核',
    hidden:JsContext.checkGrant('_fxqdqsh_h'),
    handler:function(){
    	/**
    	 * 定期审核限制
    	 */
