...
<head>
	...
	<script type="text/javascript">
		...
		var roleCodes='<%=request.getParameter("roleCodes")%>';
		 ...
		 var IDENT_TYPE1='<%=request.getParameter("IDENT_TYPE1")==null?"":request.getParameter("IDENT_TYPE1")%>';	//'&IDENT_TYPE1='+cond.IDENT_TYPE1+	//证件类型1
		 ...
		 var IDENT_EXPIRED_DATE2='<%=request.getParameter("IDENT_EXPIRED_DATE2")==null?"":request.getParameter("IDENT_EXPIRED_DATE2")%>';	//'&IDENT_EXPIRED_DATE2='+cond.CUST_ID+	//证件2到期日
		 ...
		 var AGENT_NATION_CODE='<%=request.getParameter("AGENT_NATION_CODE")==null?"":request.getParameter("AGENT_NATION_CODE")%>';	//'&AGENT_NATION_CODE='+cond.CUST_ID+	//代理人国籍
		 ...
		 var TEL='<%=request.getParameter("TEL")==null?"":request.getParameter("TEL")%>';	//'&TEL='+cond.TEL+	//代理人联系方式
		 var CUST_TYPE='<%=request.getParameter("CUST_TYPE")==null?"":request.getParameter("CUST_TYPE")%>';	//'&CUST_TYPE='+cond.CUST_TYPE+	//客户类型
		 var FXQ008='<%=request.getParameter("FXQ008")==null?"":request.getParameter("FXQ008")%>';	//'&FXQ008='+cond.CUST_ID+	//客户是否涉及风险提示信息或权威媒体报道信息
     	
		<%--  var DQSH024='<%=request.getParameter("DQSH024")==null?"":request.getParameter("DQSH024")%>';	//'&DQSH024='+cond.DQSH024+	//客户是否涉及反洗钱黑名单
		 --%> 
		var CUST_GRADE='<%=request.getParameter("CUST_GRADE")==null?"":request.getParameter("CUST_GRADE")%>';	//'&CUST_GRADE='+cond.CUST_GRADE+	//系统预评
		 ...
		 var IF_ORG_SUB_TYPE_PER='<%=request.getParameter("IF_ORG_SUB_TYPE_PER")==null?"":request.getParameter("IF_ORG_SUB_TYPE_PER")%>';	//'&IF_ORG_SUB_TYPE_PER='+cond.IF_ORG_SUB_TYPE_PER+	//客户是否为自贸区客户
     	
     
		 var CUST_GRADE_FP='<%=request.getParameter("CUST_GRADE_FP")==null?"":request.getParameter("CUST_GRADE_FP")%>';	//'&CUST_GRADE_FP='+cond.IF_ORG_SUB_TYPE_PER+	//复评等级
		 
	</script>
	<version:frameScript type="text/javascript" src="/contents/pages/wlj/custmanager/antiMoney/printManager/printAntMoneyReview.js"/>
