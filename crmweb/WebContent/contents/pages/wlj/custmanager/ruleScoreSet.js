...
imports([
    ...
    '/contents/pages/common/Com.yucheng.crm.common.IndexSearchField.js'
]);

var ifIndex = "";//if表达式的index  格式为1&2#5&9  #为表达式之间的间隔，&为起始的间隔
var url = basepath + '/ocrmFSysRuleScoreSet.json';

var createView = false;
var editView = false;
var detailView = false;

var lookupTypes = ['IF_FLAG','TYPE_RANGE','COUNT_WAY','COUNT_FREQUENCE'];

var fields = [
	{name:'ID',text:''},
	{name:'USE_WAY',text:'用途',hidden:true},
	{name:'SCORE_NAME',text:'折算规则名称',dataType:'string',searchField:true},
	{name:'INDEX_CODE',text:'指标号',dataType:'string'},
	{name:'INDEX_NAME',text:'指标名称',dataType:'string',searchField:true},
	{name:'CUST_TYPE',text:'客户类型',dataType:'string',translateType:'TYPE_RANGE',resutlWidth:70},
	{name:'STATUS',text:'是否启用',translateType:'IF_FLAG',searchField:true,resutlWidth:70},
	{name:'COMPUTE_TYPE',text:'计算方式',translateType:'COUNT_WAY',resutlWidth:70},
	{name:'CONVERT_RATE',text:'折算率',dataType:'number',decimalPrecision : 6,resutlWidth:70},
	{name:'FREQUENCE',text:'计算频率',dataType:'string',translateType:'COUNT_FREQUENCE',resutlWidth:70},
	{name:'FORMULA',text:'公式',dataType:'string',hidden:true},
	{name:'FORMULA_MEAN',text:'公式解释',dataType:'string',hidden:true},
	{name:'USER_ID',text:''},
	{name:'USER_NAME',text:'设置人',dataType:'string'},
	{name:'SET_DATE',text:'设置日期',dataType:'date'}
];
...
var tbar = [{
	...
	handler:function(){
		if(getSelectedData() == false){
			...
			for (var i=0;i<getAllSelects().length;i++){
				if(getAllSelects()[i].data.STATUS != '0' ){
					Ext.Msg.alert('提示','只能选择未启用的记录！');
					return false;
				}
				json.id.push(getAllSelects()[i].data.ID);
			}
	...
	handler:function(){
		if(getSelectedData() == false){
			...
			var ID = '';
			for (var i=0;i<getAllSelects().length;i++){
				if(getAllSelects()[i].data.STATUS != '1' ){
	...
	text :'新增',
	handler :function(){
		if(ruleSetForm.getForm().getEl()){
			ruleSetForm.getForm().getEl().dom.reset();
		}
  ...
  text :'修改',
	handler :function(){
		...
		var a = {'data':data};
		ruleSetForm.getForm().loadRecord(a);
		var type =getSelectedData().data.COMPUTE_TYPE;
		if (type == "1") {//直接折算
			...
			ruleSetForm.getForm().findField("convertRate").hide();
			ruleSetForm.getForm().findField("convertRate").setDisabled(true);
			getIfIndex(getSelectedData().data.FORMULA);
		}
		setFormFieldDisabled(false);
		showCustomerViewByIndex(0);
		ruleSetForm.buttons[0].show();
...
var setFormFieldDisabled = function(disable){
	ruleSetForm.getForm().findField("scoreName").setDisabled(disable);
	ruleSetForm.getForm().findField("useWay").setDisabled(disable);
	ruleSetForm.getForm().findField("indexName").setDisabled(disable);
	ruleSetForm.getForm().findField("custType").setDisabled(disable);
	ruleSetForm.getForm().findField("frequence").setDisabled(disable);
	ruleSetForm.getForm().findField("status").setDisabled(disable);
	ruleSetForm.getForm().findField("computeType").setDisabled(disable);
	ruleSetForm.getForm().findField("formula").setDisabled(disable);
	ruleSetForm.getForm().findField("formulaMean").setDisabled(disable);
	ruleSetForm.getForm().findField("convertRate").setDisabled(disable);
	
	if(disable){
		ruleSetForm.getForm().findField("scoreName").addClass('x-readOnly');
		ruleSetForm.getForm().findField("useWay").addClass('x-readOnly');
		ruleSetForm.getForm().findField("indexName").addClass('x-readOnly');
		ruleSetForm.getForm().findField("custType").addClass('x-readOnly');
		ruleSetForm.getForm().findField("frequence").addClass('x-readOnly');
		ruleSetForm.getForm().findField("status").addClass('x-readOnly');
		ruleSetForm.getForm().findField("computeType").addClass('x-readOnly');
		ruleSetForm.getForm().findField("formula").addClass('x-readOnly');
		ruleSetForm.getForm().findField("formulaMean").addClass('x-readOnly');
		ruleSetForm.getForm().findField("convertRate").addClass('x-readOnly');
	}else{
		ruleSetForm.getForm().findField("scoreName").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("useWay").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("indexName").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("custType").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("frequence").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("status").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("computeType").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("formula").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("formulaMean").removeClass('x-readOnly');
		ruleSetForm.getForm().findField("convertRate").removeClass('x-readOnly');
	}
...
items:[{
    ...
    items:[
            ...
            items :[{
    			...
    			xtype:'indexchoose',
    			fieldLabel : '<span style="color:red">*</span>指标', 
    			labelStyle: 'text-align:right;',
    			name : 'indexName',
    			hiddenName:'indexCode',
    			searchType:'INDEXTREE',
    			singleSelect:true,
    			allowBlank:false,
    			anchor : '90%',
    			callback: function() {
					...
					listeners : {
						"select" : function(combo,record) {
							if(record.data.key == '1'){
								ruleSetForm.getForm().findField("custType").setValue('1');
							}
						}
					},
					anchor : '90%'
...
buttons : [{ 
    ...
    handler :function(){
      ...
      //先判断是否重复问题（同一指标，同一种客户类型只能配置一条规则）
        ...
        function checkResult(response) {
			...
			if (resultArray == 200 ||resultArray == 201) {
				...
				if(exit=='no'){
					...
					if(custType=='1')
						 type2='对公';
					Ext.Msg.alert("提示", "已有针对"+type2+"客户的指标（"+ruleSetForm.form.findField("indexName").getValue()+"）的规则配置。");
				}
			}else {
				Ext.Msg.alert('提示', '失败' );
			}
...
//触发公司生成
ruleSetForm.form.findField('convertRate').on("blur",function(){
	getFormula();
});
/*********************规则编辑页面from**********************/
...
/*******************************打开公司编辑器页面******************************/
ruleSetForm.form.findField('formula').on("focus",function(){
