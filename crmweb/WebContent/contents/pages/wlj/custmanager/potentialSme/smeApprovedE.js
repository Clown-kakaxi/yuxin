...
var db_amt_selected;
var url = basepath + '/smeApprovedE.json';

var lookupTypes = ['IF_FLAG','CHECK_STAT','CUST_TYPE_SME','CASE_TYPE_SME','GRADE_PERSECT','ACC0600002','CURRENCY','SP_LEVEL',
	'UNISSUE_REASON_APPROVED_C','UNRECEPT_REASON_APPROVED_SME',
	{TYPE : 'AREA',//区域中心数据字典
...
var fields = [{name:'CUST_NAME',text:'客户名称',xtype:'customerquery',hiddenName:'CUST_ID',resutlWidth:150,allowBlank: false,singleSelect:false,searchField:true},
              ...
              {name:'RM',text:'RM',xtype:'userchoose',hiddenName:'RM_ID',resutlWidth:150,allowBlank: false,singleSelect: false,searchField: true},
			  ...
			  {name:'CASE_TYPE',text:'案件类型',translateType:'CASE_TYPE_SME',allowBlank:false,searchField: true,resizable:true,editable:true},
			  {name:'CURRENCY',text:'申请币别(原币別)',translateType:'CURRENCY',allowBlank:false,searchField: true,resizable:true,editable:true},
			  {name:'APPLY_AMT',text:'申请额度(折人民币/千元)',allowBlank:false,dataType:'money',maxLength:24},
			  ...
			  {name:'ADD_AMT',text:'增贷金额(折人民币/千元)',gridField:true,allowBlank: false,dataType:'money',maxLength:24 },
...
//							getCurrentView().contentPanel.form.findField("INSURE_MONEY").hide();
              ...
              {name:'UNRECEPT_REASON',text:'客户不接受核准额度原因',translateType:'UNRECEPT_REASON_APPROVED_SME' ,editable: true,listeners:{
				select:function(){
...
var tbar = [{
	text:'删除',
	hidden:JsContext.checkGrant('smeApprovedEDelet'),
	handler : function(){
		...
		Ext.MessageBox.confirm('提示','确定删除吗?',function(buttonId){
		    ...
		    Ext.Ajax.request({
                url: basepath+'/smeApprovedE!batchDel.json?idStr='+ID,                                
                success : function(){
	...
	new Com.yucheng.crm.common.NewExpButton({
        ...
        url :  basepath + '/smeApprovedE.json'
    })];
...
var newPanelStroe_1 = new Ext.data.Store({
	...
	proxy : new Ext.data.HttpProxy(
			{
				url : basepath + '/smeApprovedE!queryDB.json' ,
				method:'get'
			}),
    ...
    tbar : [{
    	...
    	handler :function(){
    		...
    		Ext.MessageBox.confirm('提示','你确定删除吗!',function(buttonId){
    			...
    			Ext.Ajax.request({
    				url : basepath + '/smeApprovedE!batchDelDB.json',
    				method : 'GET',
    				params:{
    					'idStr':ids 
    				},
...
var customerView = [{
		    ...
		    var data = formPanel.getForm().getFieldValues();
		      ...
		      newPanelStroe_1.reload({params:{'pipelineId':data.PIPELINE_ID},
  				callback : function() {
					...
					Ext.Ajax.request({
							url : basepath + '/smeApprovedE!addDB.json',
							method : 'POST',
		    ...
		    var data = formPanel.getForm().getFieldValues();
		            ...
		            newPanelStroe_1.reload({params:{'pipelineId':data.PIPELINE_ID},
						callback : function() {
							...
							Ext.Msg.wait('正在处理，请稍后......','系统提示');
							Ext.Ajax.request({
									url : basepath + '/smeApprovedE!updateDB.json',
									method : 'POST',
									params : {'DB_AMT':data.DB_AMT,
											  'PIPELINE_ID':data.PIPELINE_ID,
											  'DB_DATE':data.DB_DATE,
	...
	title:'修改',
	hideTitle:JsContext.checkGrant('smeApprovedEEdit'),
	type:'form',
	...
	groups : [{
		...
		fields :[
		         ...
		         {name:'RM',text:'RM',resutlWidth:150},
		          'CASE_TYPE','CUST_TYPE','INSURE_MONEY','IF_ADD','INSURE_CURRENCY','ADD_AMT','INSURE_AMT','CURRENCY','XD_CHECK_DATE','APPLY_AMT',		         
		          'DB_AMTS','IF_ACCEPT','USE_DATE_P',
				  'UNRECEPT_REASON','CTR_S_DATE','MORTGAGE_DATE','FILE_UP_DATE','SX_CTR_DATE','PROBLEM_DATE','AMT_USE_DATE',
				  'UNISSUE_REASON','CTR_C_DATE','ACCOUNT_DATE','PAY_DATE',
				  'USER_ID','ID','CUST_ID','RM_ID','DEPT_ID','AREA_ID','HP_ID','PIPELINE_ID','RECORD_DATE','LINE'],												
			fn:function(CUST_NAME,AREA_NAME,DEPT_NAME,RM,
						 CASE_TYPE,CUST_TYPE,INSURE_MONEY,IF_ADD,INSURE_CURRENCY,ADD_AMT,INSURE_AMT,CURRENCY,XD_CHECK_DATE,APPLY_AMT,						
						 DB_AMTS,IF_ACCEPT,USE_DATE_P,UNRECEPT_REASON,CTR_S_DATE,MORTGAGE_DATE,FILE_UP_DATE,
						 SX_CTR_DATE,PROBLEM_DATE,AMT_USE_DATE,UNISSUE_REASON,CTR_C_DATE,ACCOUNT_DATE,PAY_DATE,
						 USER_ID,ID,CUST_ID,RM_ID,DEPT_ID,AREA_ID,HP_ID,PIPELINE_ID,RECORD_DATE,LINE){
...
//				XD_CHECK_DATE.cls = 'x-readOnly';
		...
		labelWidth:150,  
		columnCount: 1,
		  fields : ['CTR_PROBLEM','REASON_REMARK1', 'REASON_REMARK2'],
		...
		formButtons : [{
			...
			fn : function(formPanel,basicForm){
		         ...
		         var data = formPanel.getForm().getFieldValues();
					...
					Ext.Msg.wait('正在处理，请稍后......','系统提示');
					Ext.Ajax.request({
							url : basepath + '/smeApprovedE!save.json',
							method : 'POST',
							params : commintData,
							success : function(response) {
									Ext.MessageBox.alert('提示','数据保存成功!');
	...
	title:'详情',
	hideTitle:JsContext.checkGrant('smeApprovedEDetail'),
	type:'form',
	autoLoadSeleted : true,
	groups : [{
		labelWidth:150,
	...
	if(view._defaultTitle == '修改'||view._defaultTitle == '详情'){
		   newPanelStroe_1.reload(
				{params:{
			     ...
			     if(getSelectedData().data.LINE=='3' || getSelectedData().data.LINE=='5'){
			     	newPanel_1.show();
			     }else{
			     	newPanel_1.hide();
			     }
...
var viewshow = function(view){
	...
	if(view.contentPanel.form.findField("IF_ACCEPT").getValue() == ''){//客户是否接受核准额度 默认为 是
		view.contentPanel.form.findField("IF_ACCEPT").setValue('1');
	}
	
	if(view.contentPanel.form.findField("CUST_TYPE").getValue() == '2'){
