/**
 * @description 个金客户营销流程 - 个金修正产品准备书信息页面
 * @author luyy
 * @since 2014-07-24
 */
...
var url = basepath + '/mktChangeP.json';
...
var fields = [{name:'ID',text:'ID',gridField:false},
			  ...
			  {name:"IF_FOURTH_STEP",text:"是否进入第四阶段",translateType:"IF_FLAG",allowBlank:false},
              ...
              {name:'CHANGE_INFO',text:'建议书修改内容',xtype:'textarea',gridField:false,allowBlank:false}
			  ];
...
var tbar = [{
	...
	handler : function(){
		...
		Ext.MessageBox.confirm('提示','确定删除吗?',function(buttonId){
		    ...
		    Ext.Ajax.request({
                url: basepath+'/mktChangeP!batchDel.json?idStr='+ID,                                
                success : function(){
...
var customerView = [{
	...
	groups : [{
		...
		fields :['ID','CUST_NAME','AREA_NAME','DEPT_NAME','RM',
			...
			,'PRODUCT_CATL_NAME','SALE_AMT','RISK_LEVEL','IF_FOURTH_STEP','PRODUCT_CATL'],
			fn:function(ID,CUST_NAME,AREA_NAME,DEPT_NAME,RM,PRODUCT_NAME,PRODUCT_CATL_NAME,SALE_AMT,RISK_LEVEL,IF_FOURTH_STEP,PRODUCT_CATL){
				ID.hidden = true;
				...
				return [CUST_NAME,AREA_NAME,DEPT_NAME,RM,PRODUCT_NAME,PRODUCT_CATL_NAME,SALE_AMT,RISK_LEVEL,IF_FOURTH_STEP,ID,PRODUCT_CATL];
			}
	},{
		   columnCount : 1,
		   fields:['CHANGE_INFO'],
		   fn:function(CHANGE_INFO){
			   return [CHANGE_INFO];
		   }
		}],
		formButtons : [{
			...
			fn : function(formPanel,basicForm){
		         ...
		         var data = formPanel.getForm().getFieldValues();
					...
					Ext.Ajax.request({
							url : basepath + '/mktChangeP!save.json',
							method : 'GET',
							params : commintData,
	...
	groups : [{
		...
		fields :['CUST_NAME','AREA_NAME','DEPT_NAME','RM','PRODUCT_NAME'
			,'PRODUCT_CATL_NAME','SALE_AMT','RISK_LEVEL','IF_FOURTH_STEP'],
			fn:function(CUST_NAME,AREA_NAME,DEPT_NAME,RM,PRODUCT_NAME,PRODUCT_CATL_NAME,SALE_AMT,RISK_LEVEL,IF_FOURTH_STEP){
				CUST_NAME.readOnly = true;
				CUST_NAME.cls = 'x-readOnly';
				AREA_NAME.readOnly = true;
				AREA_NAME.cls = 'x-readOnly';
				...
				return [CUST_NAME,AREA_NAME,DEPT_NAME,RM,PRODUCT_NAME,PRODUCT_CATL_NAME,SALE_AMT,RISK_LEVEL,IF_FOURTH_STEP];
			}
	},{
		   columnCount : 1,
		   fields:['CHANGE_INFO'],
