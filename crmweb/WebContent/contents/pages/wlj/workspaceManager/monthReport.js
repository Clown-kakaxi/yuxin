...
var url = basepath+'/reportManger.json?type=month';
...
var customerView = [{
		...
		fn:function(formPanel,basicForm){
			Ext.Ajax.request({
				...
				params : {
					...
					reportType:'monthReportP',
					name:name,
					cycle:cycle
				},
				success : function() {
					top.window.open( basepath+'/TempDownload?filename=monthReportP'+formPanel.getForm().getFieldValues().ID+'.doc','', 
	...
	groups:[{
		...
		fn : function(ID,REPORT_SUB1,REPORT_SUB2,REPORT_SUB3,REPORT_SUB4,REPORT_SUB5,REPORT_SUB6,REPORT_SUB7,REPORT_SUB8,REPORT_SUB9,
			...
			ID.hidden = true;
			return [ID,REPORT_SUB1,REPORT_SUB2,REPORT_SUB3,REPORT_SUB4,REPORT_SUB5,REPORT_SUB10,REPORT_SUB7,REPORT_SUB8,REPORT_SUB9,
			        REPORT_SUB6,REPORT_SUB11,REPORT_SUB12,REPORT_SUB13,REPORT_SUB14,REPORT_SUB15,REPORT_SUB16,REPORT_SUB17,REPORT_SUB18];
		}
	}],
	formButtons:[{
		text:'导出报告',
		fn:function(formPanel,basicForm){
			Ext.Ajax.request({
				...
				params : {
					ID:formPanel.getForm().getFieldValues().ID,
					reportType:'monthReportC',
					name:name,
					cycle:cycle
				},
				success : function() {
					top.window.open( basepath+'/TempDownload?filename=monthReportC'+formPanel.getForm().getFieldValues().ID+'.doc','', 
							'height=100, width=200, top=300, left=500, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
				},	
				failure : function() {
					Ext.Msg.alert('提示', '生成报告失败');	
