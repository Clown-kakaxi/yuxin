...
*@author:luyy
*@since:2014-06-25
* add export word report 
*/

var name = '';//报告人
var cycle = '';//周期

var url = basepath+'/reportManger.json?type=day';
...
var tbar = [{
	...
	text:'提交审批',
	hidden : JsContext.checkGrant('_dayReportCommit'),
	handler:function(){
		if(getSelectedData() == false){
			Ext.Msg.alert('提示','请选择一条数据');
			return false;
		}
		var id = getSelectedData().data.REPORT_ID;
		if(getSelectedData().data.REPORT_STAT != '01' && getSelectedData().data.REPORT_STAT != undefined 
			&& getSelectedData().data.REPORT_STAT != '' && getSelectedData().data.REPORT_STAT != '04'){
			Ext.Msg.alert('提示','请选择一条报告状态为已下达的记录');
			return false;	
		}
		Ext.Ajax.request({
            url: basepath + '/reportManger!commitApprReport.json',
            method: 'GET',
            params: {
                id: id
            },
            success: function(response) {
				 ...
				 selectUserList(instanceid,"67_a3","67_a4");//选择下一步办理人
//				 reloadCurrentData();
//				 hideCurrentView();
//            	 selectUserList("REP_"+id,"67_a3","67_a4");//选择下一步办理人
            },
...
var customerView = [{
	...
	groups : [{
		...
		fields : [
	        ...
	        {name:'REPORT_SUB14',text:'电访',readOnly:true,cls:'x-readOnly'},																			
	        {name:'REPORT_SUB15',text:'亲访',readOnly:true,cls:'x-readOnly'},																			
	        {name:'REPORT_SUB16',text:'来行约谈',readOnly:true,cls:'x-readOnly'},																			
	        {name:'REPORT_SUB17',text:'客户活动',readOnly:true,cls:'x-readOnly'},
	        {name:'ID',text:'ID',hidden:true}
		],
		fn : function(REPORT_SUB1,REPORT_SUB2,REPORT_SUB3,REPORT_SUB4,REPORT_SUB5,REPORT_SUB6,REPORT_SUB7
		        ,REPORT_SUB8,REPORT_SUB9,REPORT_SUB10,REPORT_SUB11,REPORT_SUB12,REPORT_SUB13,REPORT_SUB14,REPORT_SUB15,REPORT_SUB16,REPORT_SUB17,ID){
		    ID.hidden = true;
		...
		fn:function(formPanel,basicForm){
			Ext.Ajax.request({
				...
				params : {
					...
					reportType:'dayReportP',
					name:name,
					cycle:cycle
				},
				success : function() {
					top.window.open( basepath+'/TempDownload?filename=dayReportP'+formPanel.getForm().getFieldValues().ID+'.doc','', 
	...
	groups:[{
		...
		fn : function(ID,REPORT_SUB1,REPORT_SUB2,REPORT_SUB3,REPORT_SUB4,REPORT_SUB5,REPORT_SUB6,REPORT_SUB7,REPORT_SUB8,REPORT_SUB9,
			...
			ID.hidden = true;
			return [ID,REPORT_SUB1,REPORT_SUB2,REPORT_SUB3,REPORT_SUB4,REPORT_SUB9,REPORT_SUB6,REPORT_SUB7,REPORT_SUB8,REPORT_SUB5,
			        REPORT_SUB10,REPORT_SUB11,REPORT_SUB12,REPORT_SUB13,REPORT_SUB14,REPORT_SUB15,REPORT_SUB16,REPORT_SUB17,REPORT_SUB18];
		}
	}],
	formButtons:[{
		...
		fn:function(formPanel,basicForm){
			Ext.Ajax.request({
				...
				params : {
					ID:formPanel.getForm().getFieldValues().ID,
					reportType:'dayReportC',
					name:name,
					cycle:cycle
				},
				success : function() {
					top.window.open( basepath+'/TempDownload?filename=dayReportC'+formPanel.getForm().getFieldValues().ID+'.doc','', 
					'height=100, width=200, top=300, left=500, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
				},	
				failure : function() {
					Ext.Msg.alert('提示', '生成报告失败');	
