 ...
 */
Com.yucheng.bcrm.common.CustomerQueryFieldDK = Ext.extend(Ext.form.TwinTriggerField, {
    ...
    onRender : function(ct, position){
		Com.yucheng.bcrm.common.CustomerQueryFieldDK.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
...
//    					_this.oCustomerQueryForm.form.findField('CUST_TYPE').setReadOnly(true);
		...
		if(this.searchParams&&Ext.isArray(this.searchParams)){
			var column_1_panel = _this.oCustomerQueryForm.find("itemId","column_1")[0];
            Ext.each(this.searchParams,function(searchParams){
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    ...
    	    {header : '证件类型',dataIndex : 'IDENT_TYPE_ORA',width : 150,sortable : true,hidden:true},
    	    {header : '证件号码',dataIndex : 'IDENT_NO',width : 150,sortable : true,hidden:true},
    	    ...
    	    {header : '职业类别',dataIndex : 'JOB_TYPE_ORA',width : 80,sortable : true,hidden:true},
    	    ...
    	    {header : '客户类型',dataIndex : 'CUST_TYPE_ORA',width : 80,sortable : true,hidden:true},
    	    {header : '是否目标行业',dataIndex : 'IF_TARGETBUSI',width : 80,sortable : true,hidden:true},
    	    {header : '是否目标行业',dataIndex : 'IF_TARGETBUSI_ORA',width : 80,sortable : true,hidden:true},
    	    {header : '客户级别',dataIndex : 'CUST_LEVEL_ORA',width : 80,sortable : true,hidden:true},
    	    ...
    	    {header : '客户类别id',dataIndex : 'ORG_BIZ_CUST_TYPE',width:150,sortable: true,hidden:true},
    	    {header : '客户类别',dataIndex : 'F_VALUE',width:150,sortable: true},
    	    {header : '上级机构ID',dataIndex : 'UP_ORG_ID',width:150,sortable: true,hidden : true},
    	    {header : '上级机构',dataIndex : 'UP_ORG_NAME',width:150,sortable: true},
    	    {header : '是否台商',dataIndex : 'IS_TAIWAN_CORP',width:150,sortable: true}
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/customerBaseInformationDkAction.json?isNew='+oThisSearchField.isNew}),
    		reader: new Ext.data.JsonReader({
...
//    						checkedNodes = _this.oCustomerQueryGrid.getSelectionModel().selections.items;
    						...
    						for(var i=0;i<checkedNodes.length;i++){
    							if(i==0){
    								sName=checkedNodes[i].data.CUST_NAME;//客户名称
    								sOrgName=checkedNodes[i].data.ORG_NAME;//机构名称
    								oThisSearchField.hiddenField.setValue(checkedNodes[i].data.CUST_ID);//客户号
    							}else{
    								sName=sName+','+checkedNodes[i].data.CUST_NAME;
    								sOrgName=checkedNodes[i].data.ORG_NAME;
    						...
    						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
    							...
    							oThisSearchField.orgId=checkedNodes[0].data.ORG_ID;//分支行
    							...
    							oThisSearchField.custType1=checkedNodes[0].data.ORG_BIZ_CUST_TYPE;//客户类别
    							if(checkedNodes[0].data.ORG_ID=='500'){
    								oThisSearchField.region=checkedNodes[0].data.ORG_ID;
    							}else{
    								oThisSearchField.region=checkedNodes[0].data.UP_ORG_ID;//区域
    							}
    							oThisSearchField.potentialFlag=checkedNodes[0].data.POTENTIAL_FLAG;//是否潜在客户0-既有客户
    							oThisSearchField.isTaiwanCorp=checkedNodes[0].data.IS_TAIWAN_CORP;//是否台商
    						}
    					}
    					/**
...
Ext.reg('customerquerydk',Com.yucheng.bcrm.common.CustomerQueryFieldDK);
