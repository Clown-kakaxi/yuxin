...
//	    	},
    	...
    	_this.cm = new Ext.grid.ColumnModel([_this.rownum,_this.sm, 
    	    ...
    	    {header : '沟通话术',dataIndex : 'RECORD_SESSION',width : 150,sortable : true,hidden : true}
    	]);
    	/**
    	 * 数据存储
    	 */
    	_this.oCustomerQueryStore = new Ext.data.Store({
    		restful:true,	
    		proxy : new Ext.data.HttpProxy({url:basepath+'/customerBaseInformation.json?isNew='+oThisSearchField.isNew}),
    		reader: new Ext.data.JsonReader({
...
//    						checkedNodes = _this.oCustomerQueryGrid.getSelectionModel().selections.items;
						...
						if(checkedNodes.length==1){//如果单选，则设置该客户相应的附属属性
							...
							oThisSearchField.recordSession=checkedNodes[0].data.RECORD_SESSION;
						}
					}
    					/**
