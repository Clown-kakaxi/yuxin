 ...
 * @since:2017.05.11
 */
Com.yucheng.bcrm.common.BusinessType = Ext.extend(Ext.form.TwinTriggerField, {
	
	initComponent : function(){
		Com.yucheng.bcrm.common.BusinessType.superclass.initComponent.call(this);
	},
	onRender : function(ct, position){
		Com.yucheng.bcrm.common.BusinessType.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
	...
	onTrigger2Click : function() {
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				_this.busiTreeForShow.appendChild(children);
			},
		...
		_this.busiTreeForShow = new Com.yucheng.bcrm.TreePanel({
...
//								if (b.getKey() == 13) {
//									_this.busiTreeForShow.filterTree(_this.searchPanel.getForm().findField('orgfield'));
//								}
//							}
//						}
//					}]
...
//			}]
//		});
		_this.busiWindow = new Ext.Window({
			title : '所属行业',
			resizable : true,
			width : 300,
			height : 420,
				...
				handler : function() {
					...
					if(_this.hiddenName){
						if(!_this.checkBox){//单选状态
							if(!_this.busiTreeForShow.getSelectionModel().selNode.isLeaf()){
								Ext.Msg.alert('提示','请选择子行业');
								return false;
							}
							if(!(_this.busiTreeForShow.getSelectionModel().selNode==null)){
								var checkedNodes = _this.busiTreeForShow.getSelectionModel().selNode;
								if(!_this.busiTreeForShow.getSelectionModel().selNode.isRoot){
									json.push(_this.busiTreeForShow.getSelectionModel().selNode.attributes.id);
								    var node=_this.busiTreeForShow.getSelectionModel().selNode;
								  
								    var ids = new Array();//存储id的数组
								    var names=new Array();//存储name的数组
								    if(node.id >='A' && node.id<='Z'){
								    	for(var i=0; node.id >='A'; i++){ 
									        ids.push(node .id);
								    ...
								    for(var i=names.length-1; i>=0; i--){
								    	if(i==0){ sName=sName+names[i];}
								    	else{sName=sName+names[i]+'>';}
								    }
//									sName = sName + _this.busiTreeForShow.getSelectionModel().selNode.attributes.text;
								...
								qzCombaseInfo.form.findField('IN_CLL_TYPE_ID').setValue(checkedNodes.id);
								qzCombaseInfo.form.findField('IN_CLL_TYPE').removeClass("x-form-invalid");
							}
						}else{//多选状态
							if(!(_this.busiTreeForShow.getSelectionModel().selNode==null)&&(_this.busiTreeForShow.getChecked().length==0)){
								if(_this.busiTreeForShow.getSelectionModel().selNode.isRoot){
									var checkedNodes = _this.busiTreeForShow.getSelectionModel().selNode;
									json.push(checkedNodes.attributes.id);
									sName =  checkedNodes.attributes.text;
							...
							}else if ((_this.busiTreeForShow.getSelectionModel().selNode==null)&&(!(_this.busiTreeForShow.getChecked().length==0))){
								var checkedNodes = _this.busiTreeForShow.getChecked();
								...
								var checkedNodes2 = _this.busiTreeForShow.getChecked();
					...
					_this.busiWindow.hide();
					if (typeof _this.callback == 'function') {
//						_this.callback(checkedNodes);
						_this.callback(sName,checkedNodes);
					}
				}
			}, {
				text : '取消',
		...
		_this.busiWindow.show(this.id);
		return;
	}
});
Ext.reg('businessType',Com.yucheng.bcrm.common.BusinessType);