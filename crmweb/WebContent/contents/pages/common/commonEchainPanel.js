		...
		if(commentContent!=''){
			...
			Ext.Ajax.request( {
				...
				success : function(response) {
					...
					var instanceids = _this.instanceID.split("_");
					var taskNumber = instanceids[1];
					Ext.Ajax.request( {
						url : basepath + '/wfComment!save.json',
						method : 'GET',
						params:{
...
//					}
	...
	wfCompleteJob:function(nexNodeID,nextNodeUser){
		...
		success : function(a) {
			...
			//_this.saveReader();
			var msg = (returnText.nextNodeName && returnText.nextNodeName != '-') ?',下一节点:'+returnText.nextNodeName : '';
			msg += (returnText.nextNodeName && returnText.nextNodeName != '-' 
				&& returnText.nextUserName && returnText.nextUserName !='-')?',下一办理人:'+returnText.nextUserName : '';
			Ext.Msg.alert('系统提示信息',returnText.tip+msg);
