...
//		}
		var searchFunction = function(){
			...
			//查询控制   1 支行内客户经理 2 区域内非本支行的客户经理   3区域内客户经理   4 跨区域客户经理 
			 type = _this.ownerCt.ownerCt.ownerCt.getForm().findField('type')?_this.ownerCt.ownerCt.ownerCt.getForm().findField('type').getValue():'';
		...
		_this.orgUserManageProxy = new Ext.data.HttpProxy({
			baseParams:{
				'type':_this.ownerCt.ownerCt.ownerCt.getForm().findField('type').getValue(),
			},
			url:basepath+'/managerSearchfj.json'
		});
