 ...
 * @class Com.yucheng.crm.common.EchainSelectNextNodeUserPanel
 * @extends Ext.Window
 */
Com.yucheng.crm.common.EchainSelectNextNodeUserPanel = Ext.extend(Ext.Window, {
	wfArr : [],
	instanceId : '',	//流程实例号
	nodeId : '',		//流程当前节点
	nextNodeId : '',	//流程下一节点
    ...
    constrain : true,
    buttonAlign:'center',
    cbCommitFn: Ext.emptyFn(),//提交下一办理人回调方法
    cbCancelFn: Ext.emptyFn(),//撤销办理执行回调方法
		...
		Ext.Ajax.request({
			...
			success : function(b) {
				...
				if(nextUserList.nodeUsers.length>0){
					...
					if (typeof _this.cbCommitFn == 'function') {
						_this.cbCommitFn();
					}
					try{
						reloadCurrentData();
			...
			failure : function() {
				...
				Ext.Msg.alert('提示', '未找到下一办理人，后续可在[待办工作]中手动提交或撤销!' );
			}
		});
		
		Com.yucheng.crm.common.EchainSelectNextNodeUserPanel.superclass.initComponent.call(_this);
	},
	onRender : function(ct, position){
		var _this = this;
		Com.yucheng.crm.common.EchainSelectNextNodeUserPanel.superclass.onRender.call(this, ct, position);
	},
	/**
						...
						for(var i=0;i<_this.wfArr.length;i++){
							Ext.Ajax.request({
								...
								success : function(response) {
									...
									_this.isHideFlag = true;
									_this.hide();
									if (typeof _this.cbCancelFn == 'function') {
										_this.cbCancelFn();
									}
							    },
							    failure : function(){
 ...
 * @param {} cbCommitFn 提交回调方法
 * @param {} cbCancelFn 撤销回调方法
 */
selectUserList = function(instanceId,nodeId,nextNodeId,cbCommitFn,cbCancelFn){
	var selWin = new Com.yucheng.crm.common.EchainSelectNextNodeUserPanel({
		wfArr : [],
		instanceId : instanceId,
		nodeId :　nodeId,
		nextNodeId : nextNodeId,
		cbCommitFn : cbCommitFn,
		cbCancelFn : cbCancelFn
	});
};
