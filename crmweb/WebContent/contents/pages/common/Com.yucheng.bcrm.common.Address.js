 ...
 * @since:2017.05.11
 */
Com.yucheng.bcrm.common.Address = Ext.extend(Ext.form.TwinTriggerField, {
	
	initComponent : function(){
		Com.yucheng.bcrm.common.Address.superclass.initComponent.call(this);
	},
	onRender : function(ct, position){
		Com.yucheng.bcrm.common.Address.superclass.onRender.call(this, ct, position);
		if(this.hiddenName){
			var ownerForm = this;
	...
	onTrigger2Click : function() {
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				_this.addrTreeForShow.appendChild(children);
			},
		...
		_this.addrTreeForShow = new Com.yucheng.bcrm.TreePanel({
...
//								if (b.getKey() == 13) {
//									_this.addrTreeForShow.filterTree(_this.searchPanel.getForm().findField('orgfield'));
//								}
//							}
//						}
//					}]
...
//			}]
//		});
		_this.addrWindow = new Ext.Window({
			title : '所属地址',
			resizable : true,
			width : 300,
			height : 420,
				...
				handler : function() {
					...
					if(_this.hiddenName){
						if(!_this.checkBox){//单选状态
							if(!_this.addrTreeForShow.getSelectionModel().selNode.isLeaf()){
								Ext.Msg.alert('提示','请选择子地址');
								return false;
							}
							if(!(_this.addrTreeForShow.getSelectionModel().selNode==null)){
								if(!_this.addrTreeForShow.getSelectionModel().selNode.isRoot){
									json.push(_this.addrTreeForShow.getSelectionModel().selNode.attributes.id);
								    var node=_this.addrTreeForShow.getSelectionModel().selNode;
...
//									sName = sName + _this.addrTreeForShow.getSelectionModel().selNode.attributes.text;
									_this.hiddenField.setValue(json[0]);
								}else{
									var checkedNodes = _this.addrTreeForShow.getSelectionModel().selNode;
								...
								qzCombaseInfo.form.findField('REGISTER_AREA').removeClass("x-form-invalid");
							}
						}else{//多选状态
							if(!(_this.addrTreeForShow.getSelectionModel().selNode==null)&&(_this.addrTreeForShow.getChecked().length==0)){
								if(_this.addrTreeForShow.getSelectionModel().selNode.isRoot){
									var checkedNodes = _this.addrTreeForShow.getSelectionModel().selNode;
									json.push(checkedNodes.attributes.id);
									sName =  checkedNodes.attributes.text;
							...
							}else if ((_this.addrTreeForShow.getSelectionModel().selNode==null)&&(!(_this.addrTreeForShow.getChecked().length==0))){
								var checkedNodes = _this.addrTreeForShow.getChecked();
								...
								var checkedNodes2 = _this.addrTreeForShow.getChecked();
...
//						_this.callback(checkedNodes);
		...
		_this.addrWindow.show(this.id);
		return;
	}
});
Ext.reg('address',Com.yucheng.bcrm.common.Address);