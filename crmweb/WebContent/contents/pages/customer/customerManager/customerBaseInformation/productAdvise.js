...
//		tbar:tbar3,
	...
	var panel = new Ext.form.FormPanel({
		...
		items:[{xtype:'textfield', fieldLabel : '客户编号', name : 'custId', labelStyle : 'text-align:right;', anchor : '100%',allowBlank : false},
			...
			{xtype:'textfield', fieldLabel : '产品id', name : 'prodId', labelStyle : 'text-align:right;', anchor : '100%',allowBlank : false,hidden:true},
			{xtype:'textfield', fieldLabel : '产品名称', name : 'prodName', labelStyle : 'text-align:right;', anchor : '100%',allowBlank : false},
			{fieldLabel : '短信模板<font color="red">*</font>',hiddenName : 'modelId',name:'modelId',xtype : 'combo',labelStyle: 'text-align:right;',triggerAction : 'all',
...
//	        			 Ext.Msg.wait('正在生成短信内容......','系统提示');
	        	 ...
	        	 {xtype:'textarea', fieldLabel : '短信内容', name : 'messageRemark', labelStyle : 'text-align:right;', anchor : '100%',allowBlank : false,disabled:false}
	        	 ]
	});
	//短信窗口
		...
		buttons : [ {text:'确定',
			handler:function(){
				if(!panel.getForm().isValid()){
					...
					else if(messageRemark!= panel.form.findField('messageRemark').getValue()){//做了修改
						needApprove = true;
					}else{
						needApprove = false;
					}
	...
	var tbar = new Ext.Toolbar({
		items : [
				...
				handler : function() {
					...
					iconCls: 'editIconCss',
					handler:function(){
						var selectLength = productGrid.getSelectionModel().getSelections().length;
						if (selectLength > 1) {
						...
						if (selectLength > 1) {
							...
							if (infoRecord == null || infoRecord == '') {
								...
								if (data.telephoneNum == '') {// 没有电话信息
									...
									panel.getForm().findField('cellNumber').setValue(data.telephoneNum.substring(data.telephoneNum.length-11,data.telephoneNum.length));
									panel.getForm().findField('prodId').setValue(data.prodId);
									panel.getForm().findField('prodName').setValue(data.prodName);
									...
									modelstore.load({
										...
										callback:function(){
											if(modelstore.getCount()!=0){
								        		...
								        		Ext.Msg.alert("提示", "本产品没有相关的短信模板,您需要自行编辑内容!");
								        		messageRemark = '';
								        		panel.form.findField('modelId').setVisible(false);
								        	}
					...
					handler : function() {
						var checkedNodes = productGrid.getSelectionModel().selections.items;
						 ...
						 ifProd = true;
						    	...
						    	addBusiOpporForm.form.findField("custType").setValue(data.custStat);
						    	addBusiOpporForm.form.findField("custCategory").setValue(data.custTyp);
						    	addBusiOpporForm.form.findField("custName").setValue(data.custZhName);
						    	addBusiOpporForm.form.findField("custContactName").setValue(data.linkUser);
						    	addBusiOpporForm.form.findField("prodId").setValue(data.prodId);
						    	addBusiOpporForm.form.findField("prodName").setValue(data.prodName);
...
//	// 定义自动当前页行号
	var rownum = new Ext.grid.RowNumberer({
	   ...
	   var store = new Ext.data.Store({
			...
			{name:'linkUser',mapping:'LINK_USER'},
             {name:'FR_ID_ORA'},
             {name:'PROD_FR_ID_ORA'},
