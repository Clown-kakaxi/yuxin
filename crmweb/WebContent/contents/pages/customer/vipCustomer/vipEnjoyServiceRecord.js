 ...
 */
	...
	var url=basepath+'/vipEnjoyServiceRecordQueryAction.json';
	
	var fields=[
	            {name:'ID',text:'编号',hidden:true},
	            {name:'CUST_ID',text:'客户编号',searchField:true},
	            {name:'CUST_NAME',text:'客户名称',searchField:true},
	            {name:'ALIANCE_PROGRAM_ID',text:'联盟商代码',xtype:'textfield'},
	            {name:'ALIANCE_PROGRAM_NAME',text:'联盟商名称',searchField:true},
	            {name: 'ADD_SERVICE_IDENTIFY',text:'增值服务标识',searchField:true,xtype: 'combo',width: 180,translateType:'SERVICE_IDTYPE',mode : 'local',store: new Ext.data.Store(),triggerAction : 'all',displayField:'value',valueField:'key',editable: false,allowBlank:false},
	            {name:'SERVICE_CONTENT',text:'增值服务内容',resutlWidth:200},
	            {name:'ENJOY_SERVICE_DATE',text:'享受服务日期',xtype:'datefield',format:'Y-m-d'},
	            {name:'ENJOY_SERVICE_DATE1',mapping:'ENJOY_SERVICE_DATE',text:'享受服务日期从',gridField:false,xtype:'datefield',format:'Y-m-d',searchField:true},
	            {name:'ENJOY_SERVICE_DATE2',mapping:'ENJOY_SERVICE_DATE',text:'到',gridField:false,xtype:'datefield',format:'Y-m-d',searchField:true}
	            ];
	...
	var tbar=[{
		...
		handler : function() {
			...
			var fileName = 'importVipEnjoyServiceRecord.xlsx';// 模板名称
		...
		handler : function() {
			importForm.tradecode = 'importVipEnjoyServiceRecord';
			importWindow.show();
		}}];
	/*************导入窗口定义模块****************/
