...
//购房规划创建面板内容 第一步
var addHousePanel = new Ext.FormPanel({
    labelAlign:'right',
    labelWidth:150,
    buttonAlign:'center',
    ...
    items:[{
        ...
        items:[
            ...
            {items: [{xtype:'datefield',fieldLabel: '购房日期',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '房屋单价',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '房屋面积',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'combo',fieldLabel: '房屋用途',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '契税率',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '印花税税率',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '公共维修基金税率',name: 'xming',anchor:'90%'}]},
            {columnWidth:1.05,items: [{xtype:'checkbox',fieldLabel: '是否录入购房费用',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '置业装修费用',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '物业费用(年)',name: 'xming',anchor:'90%'}]},
            {columnWidth:1.05,items: [{xtype:'textarea',height:100,enableAlignments:false,enableLists:false,fieldLabel: '备注',name: 'xming',anchor:'90%'}]}
         ]
    },{
        xtype: 'fieldset',
        title: '费用总计',
        ...
        border: true,
        items:[
          {layout: "form", border: false, items: { xtype: "label", fieldLabel: "购房总费用" ,anchor:'90%'}, columnWidth: .25},
          {layout: "form", border: false, items: { xtype: "label", fieldLabel: "877654.00",anchor:'90%'}, columnWidth: .25,labelAlign:'left'},
          {layout: "form", border: false, items: { xtype: "label", fieldLabel: "购房后总费用" ,anchor:'90%'}, columnWidth: .25},
          {layout: "form", border: false, items: { xtype: "label", fieldLabel: "877654.00" ,anchor:'90%'}, columnWidth: .25,labelAlign:'left'}
        ]
    }],
    buttons:[{
	    		text:'计算',
	    		handler:function() {Ext.MessageBox.alert('提示消息', '计算完成！', ''); }
    		},{
    			text:'下一步',
		    	handler:function(){
		    		houseWin1.hide();//hide()隐藏窗口，close()关闭窗口
		    		houseWin2.show();
		    	}
    }]
});
...
//第二步
var addHousePanel2 = new Ext.FormPanel({
    labelAlign:'right',
    labelWidth:180,
    buttonAlign:'center',
    bodyStyle:'padding:5px;',
    ...
    items:[{
        ...
        defaults:{layout: 'form',border:false,columnWidth:.5},
        items:[
            {items: [{xtype:'textfield',fieldLabel: '期望投资的收入',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '现在为购房已经准备的投资金额',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '将来每年为购房继续投资的金额',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '购房时获得的资助金额',name: 'xming',anchor:'90%'}]},          
            {columnWidth:1.05,items: [{xtype:'textarea',height:100,enableAlignments:false,enableLists:false,fieldLabel: '备注',name: 'xming',anchor:'90%'}]}
         ]
    }],
    buttons:[{
    		text:'上一步',handler:function(){houseWin2.hide();houseWin1.show();}
    	 },{text:'保存&下一步',handler:function(){houseWin2.hide();houseWin3.show();}
    }]
});


//创建购房规划面板  第二步
var houseWin2 = new Ext.Window( {
	resizable : false,
	collapsible : false,
	draggable : true,
	closeAction : 'hide',
	...
	buttonAlign : "center",
	title : '创建购房规划 第二步',
	items : [ addHousePanel2 ]
});


//购房规划创建面板内容 第三步
var addHousePanel3 = new Ext.FormPanel({
    labelAlign:'right',
    labelWidth:180,
    buttonAlign:'center',
    bodyStyle:'padding:5px;',
    ...
    items:[{
        ...
        items:[
            ...
            {items: [{xtype:'textfield',fieldLabel: '规划名称',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '购房总费用',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '购房后总费用',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'datefield',fieldLabel: '购房日期',name: 'xming',anchor:'90%'}]},            
            {items: [{xtype:'textfield',fieldLabel: '期望投资的收益率',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '现在为购房已经准备的投资金额',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '将来每年为购房继续投资的金额',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '购房时获得的资助金额',name: 'xming',anchor:'90%'}]},
            {items: [{xtype:'textfield',fieldLabel: '通过投资可获得的购房资金',name: 'xming',anchor:'90%'}]}
         ]
    },{
        xtype: 'fieldset',
        title: '贷款计算器',
        ...
        defaults:{layout: 'form',border:false,columnWidth:.5},
        items:[         
              {columnWidth:1.05,items: [{xtype:'textarea',height:60,enableAlignments:false,enableLists:false,fieldLabel: '备注',name: 'xming',anchor:'90%'}]}    
    ...
    buttons:[{
    		text:'上一步',
    		handler:function(){
        		houseWin3.hide();//hide()隐藏窗口，close()关闭窗口
        		houseWin2.show();
    		}
    	},{
    		text:'计算',handler:function() {Ext.MessageBox.alert('提示消息', '计算完成！', ''); }
    	},{
    		text:'保存&下一步',
    		handler:function(){
    		houseWin3.hide();//hide()隐藏窗口，close()关闭窗口
...
var columns2 = new Ext.grid.ColumnModel([num2, sm2,{
	dataIndex : 'a0',header : '产品编号',sortable : true,width : 80}, {
	dataIndex : 'a1',header : '产品名称',sortable : true,width : 140},{
	dataIndex : 'a2',header : '<center>起购金额</center>',sortable : true,width : 140,align:'right'},{
	dataIndex : 'a3',header : '推荐原因',sortable : true,width : 140}
	]);
...
//第四步
var addHousePanel4 = new Ext.FormPanel({
    labelAlign:'right',
    labelWidth:150,
    buttonAlign:'center',
    bodyStyle:'padding:5px;',
    ...
    items:[{
        ...
        items:[
			...
			{items: [{xtype:'textfield',fieldLabel: '客户姓名',name: 'xming',anchor:'90%',value:'张三购房规划'}]},
    ...
    buttons:[{
    			...
    			text:'保存',
    			handler:function(){
    				Ext.Msg.alert("系统提示信息", "保存成功！",function(){houseWin4.hide();});	
    			}
    		},{
    			text:'生成报告',
    			handler:function(){
    				window.open( basepath+'/TempDownload?filename=personReport.doc','', 'height=100, width=200, top=300, left=500, toolbar=no,menubar=no, scrollbars=no, resizable=no,location=no, status=no');
    			}
    }]
});
