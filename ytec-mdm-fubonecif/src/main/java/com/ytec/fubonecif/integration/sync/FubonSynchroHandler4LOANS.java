...
@Component
@Scope("prototype")
public class FubonSynchroHandler4LOANS extends SynchroExecuteHandler{
	private static Logger log = LoggerFactory.getLogger(FubonSynchroHandler4LOANS.class);
	...
	public String syncPackageReqXml(String custId) throws Exception{
		...
		requestHeader.addElement("DestSysCd").setText("LOANS");// 目标系统代号
		...
		int length=reqxml.getBytes("GBK").length;
		temp.append(String.format("%08d", length+4));//这里添加了4位
		temp.append(reqxml);
		String strRequest = new String(HSDES.genMAC(temp.toString().getBytes("GBK")));
		buf.append(strRequest.substring(0,4));
		return buf.toString();
	}
		...
		if (this.synchroResponseMsg != null) {
			...
			try {
				...
				if (txStatCodeNode == null || txStatDescNode == null || txStatDetailNode == null) {
					String msg = String.format("{},收到外围系统[%s]响应报文必要节点为空[需要节点：%s,%s,%s]", ErrorCode.ERR_SYNCHRO_RESP_BODY_ERROR.getChDesc(), BusinessCfg.getString("plCd"), txStatCodeXpath,
				...
				String txStatDetail = "外围系统[" + BusinessCfg.getString("plCd") + "]响应:txStatDesc:{" + txStatDesc + "},txStatDetail:{" + txStatDetailNode.getText() + "}";
