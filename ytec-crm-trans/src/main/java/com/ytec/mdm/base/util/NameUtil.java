...
public class NameUtil {
	...
	/**
	 * @函数名称:GetProcedureName
	 * @函数描述：getProcedureName
	 * 深藍:
	 * @参数与返回说明:
	 * 		@param name
	 * 		@return
	 * @算法描述:
	 */
	public String  GetProcedureName() throws Exception {	
		 String ProcedureName="";
		   try { 
			   	String filepath = Class.class.getClass().getResource("/").getPath()+"/serverConfig.xml" ;                       
			   	File f = new File(filepath); 
			   	DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();  
			   	DocumentBuilder builder = factory.newDocumentBuilder();       
			   	Document doc = builder.parse(f);       
			   	NodeList nl = doc.getElementsByTagName("Procedure");      
			   	for (int i = 0; i < nl.getLength(); i++) {  
			   	   ProcedureName=doc.getElementsByTagName("ProcedureName").item(i).getFirstChild().getNodeValue();    
		   		}      
		   	} catch (Exception e) { 
		   	   e.printStackTrace();     
		   	}
