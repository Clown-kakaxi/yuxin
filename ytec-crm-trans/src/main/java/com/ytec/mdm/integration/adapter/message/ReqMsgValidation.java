...
public class ReqMsgValidation extends TransVerif {
	...
	public boolean validateAndConvertReqMsg() {
		try {
			...
			List<TxMsgNode> tsMsgNodeList = null;
	...
	private void validateOneNode(Element context, String xpath, TxMsgNode currentxMsgNode, Map<Long, List<TxMsgNode>> txMsgNodeMap) {
		...
		List<TxMsgNodeAttr> txMsgNodeAttrList = null;
		...
		if (!"".equals(xpath) && !"0".equals(currentxMsgNode.getNodeDisplay())) {// ------------------------------
			...
			// TODO
			// if (nodeNum > 1 && !MdmConstants.NODE_GROUP_M.equals(currentxMsgNode.getNodeGroup()) && "1".equals(this.txModel.getTxDef().getTxCheckXsd())) {
		...
		// TODO
		// if (nodeNum == 0 && !MdmConstants.NODE_GROUP_M.equals(currentxMsgNode.getNodeGroup()) && "1".equals(this.txModel.getTxDef().getTxCheckXsd())) {
		if (nodeNum == 0 && !MdmConstants.NODE_GROUP_M.equals(currentxMsgNode.getNodeGroup())) {
			ecifData.setStatus(ErrorCode.ERR_XML_FORMAT_INVALID);
			log.warn("{}:{}不存在", ErrorCode.ERR_XML_FORMAT_INVALID.getChDesc(), nodeCode);
			return;
		}
		for (TxMsgNodeAttr txMsgNodeAttr : txMsgNodeAttrList) {
			...
			if (attrNum != nodeNum) {
				if (StringUtil.isEmpty(txMsgNodeAttr.getDefaultVal())) {
					ecifData.setStatus(ErrorCode.ERR_XML_FORMAT_INVALID);
					log.warn("{}:{}不符合接口配置", ErrorCode.ERR_XML_FORMAT_INVALID.getChDesc(), attrCode);
					return;
				}
			}
			if (elementList != null) {
				for (Element element : elementList) {
					...
					if (isFormat) {
						/** 数值日期型格式化 ***/
						value = formatValue(decimalFormat, dateFormat, value);
					}
			...
			for (TxMsgNode txMsgNode : txMsgNodeList) {
				...
				if (MdmConstants.NODE_GROUP_M.equals(txMsgNode.getNodeGroup())) {
					if ("0".equals(txMsgNode.getNodeDisplay())) {
						...
						log.warn("{}:{}节点组,不可配置为影式标签", ErrorCode.ERR_XML_FORMAT_INVALID.getChDesc(), txMsgNode.getNodeCode());
						return;
					}
					if (MdmConstants.NODE_GROUP_NO_LABEL.equals(txMsgNode.getNodeLabel())) {
						...
						// ecifData.setStatus(ErrorCode.ERR_XML_FORMAT_INVALID);
						// log.warn("{}:{}不存在",ErrorCode.ERR_XML_FORMAT_INVALID.getChDesc(),txMsgNode.getNodeCode()+ MdmConstants.NODE_GROUP_SUFFIX);
						// return;
						// }
						newXpath += txMsgNode.getNodeCode() + MdmConstants.NODE_GROUP_SUFFIX + "/" + txMsgNode.getNodeCode();
					}
				} else {// 非节点组或者没有父节点//------------------------------
					if (!"0".equals(txMsgNode.getNodeDisplay())) {
						newXpath += txMsgNode.getNodeCode();
	 ...
	 * @算法描述:
	 */
	private String formatValue(DecimalFormat decimalFormat, SimpleDateFormat dateFormat, String value) {
		if (!StringUtils.isEmpty(value)) {
			if (decimalFormat != null) {
				try {
					...
					ecifData.setStatus(ErrorCode.ERR_RULE_INVALID_LONG.getCode(), "%s:数值类型格式不正确", value);
					log.warn("{}:数值类型格式不正确", value);
					return null;
				}
				...
				if (dateval != null) {
					...
					ecifData.setStatus(ErrorCode.ERR_RULE_INVALID_DATE_FORMAT.getCode(), "%s:日期时间格式不正确", value);
					log.warn("{}:日期时间格式不正确", value);
					return null;
				}
			}
		}
