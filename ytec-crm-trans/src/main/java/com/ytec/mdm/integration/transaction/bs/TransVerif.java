...
public class TransVerif {
	...
	protected TxModel4CRM txModel;// 交易配置模型对象
	...
	protected String processCTDR(TxMsgNodeAttr txMsgNodeAttr, String value) {
		...
		if (StringUtils.isEmpty(value)) {
			if ("N".equals(txMsgNodeAttr.getNulls())) {
				ecifData.setStatus(ErrorCode.ERR_RULE_VALUE_IS_NULL.getCode(),"%s不可为空",attrName);
				log.warn("{}不可为空",attrName);
				return null;
			} else {
				return value;
			}
		}else{
			if(MdmConstants.NODE_TYPE_VARI_STR.equals(txMsgNodeAttr.getDataType())){/**变长字符串**/
				/***计算字节长度,会牺牲效率****/
				...
				if(txMsgNodeAttr.getDataLen()!=null&&txMsgNodeAttr.getDataLen()>0&&relLength>txMsgNodeAttr.getDataLen()){
					ecifData.setStatus(ErrorCode.ERR_RULE_INVALID_LENGTH.getCode(),"%s长度不可超过%d",attrName,txMsgNodeAttr.getDataLen());
					log.warn("{}({})长度不可超过{}",attrName,txMsgNodeAttr.getAttrCode(),txMsgNodeAttr.getDataLen());
					return null;
				}
			}else if(MdmConstants.NODE_TYPE_FIX_STR.equals(txMsgNodeAttr.getDataType())){/**定长长字符串**/
				int relLength=value.getBytes().length;   //value.length();
				if(txMsgNodeAttr.getDataLen()!=null&&txMsgNodeAttr.getDataLen()>0&&relLength!=txMsgNodeAttr.getDataLen()){
					ecifData.setStatus(ErrorCode.ERR_RULE_INVALID_LENGTH.getCode(),"%s应该为固定长度%d",attrName,txMsgNodeAttr.getDataLen());
...
//		}
	 ...
	 * @算法描述:
	 */
	protected String convertObjectToString(Long nodeCode,
			TxMsgNodeAttr txMsgNodeAttr, Object value) {
		...
		if (MdmConstants.NODE_TYPE_FIX_STR.equals(dataType)
			...
			try {
				...
				String msg = "返回报文节点:" + nodeCode + "的属性:" + attrName
						+ " 格式化失败";
				log.error(msg,e);
