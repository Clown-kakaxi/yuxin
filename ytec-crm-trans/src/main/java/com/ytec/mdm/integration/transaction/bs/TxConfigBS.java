...
public class TxConfigBS {
	...
	public TxModel getTxModel(String txCode) {
		...
		// TODO
		String[] custrizTxCode = new String[] { "openOrgAccount", "openPerAccount", "pushCustInformation" };
		for (String code : custrizTxCode) {
			if (code.equals(txCode)) {
				TxDef def = new TxDef();
				def.setTxCode(code);
				def.setTxDealClass(code);
				def.setTxName(code);
				def.setTxCnName(code);
				def.setTxDealEngine("custrizEcifDealEngine");
				def.setTxCfgTp(MdmConstants.TX_CFG_TP_CUS);
				txModel.setTxDef(def);
				return txModel;
			}
		}
		...
		if (null != reqTxMsg) {
			List<TxMsgNode> reqTxMsgNodeList = txMsgNodeDAO.findByPropertyAndOrder("msgId", txModel.getReqTxMsg().getMsgId(), "nodeSeq", false);
			...
			for (int i = 0; i < reqTxMsgNodeList.size(); i++) {
				...
				findNodeInfo(txMsgNodeAttrMap, txMsgNodeAttrCtMap, txMsgNodeTabMapMap, txMsgNodeTabRelMap, txMsgNodeFilterMap, txMsgNode);
		...
		if (null != resTxMsg) {
			List<TxMsgNode> resTxMsgNodeList = txMsgNodeDAO.findByPropertyAndOrder("msgId", txModel.getResTxMsg().getMsgId(), "nodeSeq", false);
	 ...
	 * @param nodeId
	 */
	private void findNodeInfo(Map<Long, List<TxMsgNodeAttr>> txMsgNodeAttrMap, Map<Long, List<TxMsgNodeAttrCt>> txMsgNodeAttrCtMap, Map<Long, List<TxMsgNodeTabMap>> txMsgNodeTabMapMap,
			Map<Long, List<TxMsgNodeTabsRel>> txMsgNodeTabRelMap, Map<Long, List<TxMsgNodeFilter>> txMsgNodeFilterMap, TxMsgNode txMsgNode) {
		...
		if (txMsgNodeTabRelList != null && txMsgNode.getNodeTp().equals(MdmConstants.NODE_TP_T)) {
			...
			for (TxMsgNodeTabsRel txMsgNodeTabsRel : txMsgNodeTabRelList) {
				...
				if ((temp = colNameMap.get(txMsgNodeTabsRel.getTabId1().toString() + txMsgNodeTabsRel.getColId1().toString())) != null) {
					colName1 = temp.getColName();
				} else {
					colName1 = null;
				}

				if ((temp = colNameMap.get(txMsgNodeTabsRel.getTabId2().toString() + txMsgNodeTabsRel.getColId2().toString())) != null) {
					colName2 = temp.getColName();
				} else {
					colName2 = null;
				}
				if (colName1 == null || colName2 == null) {
					log.error("TxMsgNodeTabsRel,找不到id为:{}或者{} 对应的元数据表字段信息！", txMsgNodeTabsRel.getColId1(), txMsgNodeTabsRel.getColId2());
		...
		if (txMsgNodeFilterList != null) {
			...
			for (TxMsgNodeFilter txMsgNodeFilter : txMsgNodeFilterList) {// ------------------------------
				...
				if ((temp = colNameMap.get(txMsgNodeFilter.getTabId().toString() + txMsgNodeFilter.getColId().toString())) != null) {
					colName = temp.getColName();
				} else {
					colName = null;
				}
				if (colName == null) {
					log.error("TxMsgNodeFilter,找不到表id:{},字段id:{} 对应的元数据表字段信息！", txMsgNodeFilter.getTabId(), txMsgNodeFilter.getColId());
					throw new BizException("查找元数据表字段信息失败");
				}
				...
				if (txMsgNodeFilter.getAttrId() != null && txMsgNodeFilter.getAttrId() != 0) {
					...
					txMsgNodeFilter.setReqAttrCode(txMsgNodeFilter.getTabId().toString() + txMsgNodeFilter.getColId().toString());
		...
		if (txMsgNodeAtrrList != null && txMsgNode.getNodeTp().equals(MdmConstants.NODE_TP_T)) {
			...
			for (TxMsgNodeAttr txMsgNodeAttr : txMsgNodeAtrrList) {
				...
				if (txMsgNodeAttr.getColId() == null || txMsgNodeAttr.getTabId() == null || txMsgNodeAttr.getTabId() == 0) {
					continue;
				}
		...
		// 查询节点属性
		List<TxMsgNodeAttrCt> txMsgNodeAtrrCtList = txMsgNodeAttrCtDAO.findByPropertyAndOrderWithParams("attrId", attrIdList, "ctRule", false);
