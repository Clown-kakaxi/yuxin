...
public class TxModelHolder {
	 ...
	 * @since 1.0.0
	 */
	private static ConcurrentHashMap<String, TxModel4CRM> txModelMap = new ConcurrentHashMap<String, TxModel4CRM>();
	/**
	 * @属性名称:txCheckInfoMap
	 * @属性描述:XSD校验信息
	 * @since 1.0.0
	 ...
	 * @算法描述:
	 */
	public static TxModel4CRM getTxModel(String txCode) {

		TxModel4CRM txModel = txModelMap.get(txCode);
		TxConfigBS4CRM txConfigBS = null;
		if (txModel == null) {
			txConfigBS = (TxConfigBS4CRM) SpringContextUtils.getBean("txConfigBS4CRM");
			txModel = txConfigBS.getTxModel(txCode);
			TxModel4CRM txModelTemp = null;
			/**
			 * 多线程并发控制
			 */
	 ...
	 * @算法描述:
	 */
	public static TxModel4CRM getTxModelFromCache(String txCode) {
	...
	public static String getTxCheckInfo(String txCode) {
		...
		if (checkInfo == null) {
			...
			if (MdmConstants.globalTxDefCheck) {
				TxConfigBS4CRM txConfigBS = (TxConfigBS4CRM) SpringContextUtils.getBean("txConfigBS4CRM");
				TxDef txDef4CRM = txConfigBS.getTxDef(txCode);
				// 检查修改日期是否发生变化
				...
				if (newTimestamp != null && (oldTimestamp == null || newTimestamp.after(oldTimestamp))) {
					// 交易配置修改，重新获取交易模型
					TxModel4CRM txModel = txConfigBS.getTxModel(txCode);
	...
	private static CheckInfo getCheckInfo(String txCode) {
		...
		try {
			...
			List<Object[]> result = baseDAO
					.findWithIndexParam(
							"select checkinfo.checkinfo,def.updateTm,def.txCheckXsd from TxDef def,TxMsg msg,TxMsgCheckinfo checkinfo where def.txId = msg.txId  and msg.msgId = checkinfo.msgId and msg.msgTp = '1'  and def.txCode =? and def.txState ='1'",
							txCode);
