...
public class PersondebtBS extends BaseBS<Object> {


@SuppressWarnings("unchecked")
public SearchResult<Persondebt> getPersondebtList(int firstResult,
		int pageSize, String orderBy, String orderType,
		Map<String, Object> conditionMap,long custId) {
	StringBuffer jql = new StringBuffer("");
	jql.append("select Persondebt from com.yuchengtech.emp.ecif.customer.entity.perfinance.Persondebt Persondebt where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Persondebt.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Persondebt." + orderBy + " " + orderType);
	}
