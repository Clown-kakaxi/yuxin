...
public class PersonsocialinsuranceBS extends BaseBS<Object> {
...
public SearchResult<Personsocialinsurance> getPersonsocialinsuranceList(int firstResult,
	...
	jql.append("select Personsocialinsurance from com.yuchengtech.emp.ecif.customer.entity.perrisk.Personsocialinsurance Personsocialinsurance where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Personsocialinsurance.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Personsocialinsurance." + orderBy + " " + orderType);
	}
