...
public class TxAlarmEmailBS extends BaseBS<TxAlarmEmail> {
	@SuppressWarnings("unchecked")
	public SearchResult<TxAlarmEmail> getTxAlarmEmailList(int firstResult, int pageSize, String orderBy, String orderType,
		...
		jql.append("select TxAlarmEmail from TxAlarmEmail TxAlarmEmail where 1=1 ");

		if (!conditionMap.get("jql").equals("")) {
			jql.append(" and " + conditionMap.get("jql"));
		}
		
		jql.append(" order by alarmId desc ");
		
		SearchResult<TxAlarmEmail> TxAlarmEmailList = this.baseDAO.findPageWithNameParam(firstResult, pageSize,
				jql.toString(), values);
		return TxAlarmEmailList;
	}
	...
	public List<Map<String, String>> getComBoBox() {
		StringBuffer jql = new StringBuffer("select groupId, groupName from TxAlarmEmail  t");
