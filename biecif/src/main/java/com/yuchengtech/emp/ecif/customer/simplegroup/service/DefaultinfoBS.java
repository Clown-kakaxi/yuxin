...
import com.yuchengtech.emp.ecif.customer.entity.orgrisk.Defaultinfo;
@Service
@Transactional(readOnly = true)
public class DefaultinfoBS extends BaseBS<Object> {
...
public SearchResult<Defaultinfo> getDefaultinfoList(int firstResult,
	...
	jql.append("select Defaultinfo from com.yuchengtech.emp.ecif.customer.entity.orgrisk.Defaultinfo Defaultinfo where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Defaultinfo.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Defaultinfo." + orderBy + " " + orderType);
	}
