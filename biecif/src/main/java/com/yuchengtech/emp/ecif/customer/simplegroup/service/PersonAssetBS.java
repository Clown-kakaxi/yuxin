...
public class PersonAssetBS extends BaseBS<Object> {


@SuppressWarnings("unchecked")
public SearchResult<PersonAsset> getPersonAssetList(int firstResult,
		int pageSize, String orderBy, String orderType,
		Map<String, Object> conditionMap,long custId) {
	StringBuffer jql = new StringBuffer("");
	jql.append("select PersonAsset from com.yuchengtech.emp.ecif.customer.entity.perfinance.PersonAsset PersonAsset where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and PersonAsset.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by PersonAsset." + orderBy + " " + orderType);
	}
