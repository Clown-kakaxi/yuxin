...
public class OverinterestBS extends BaseBS<Object> {
...
public SearchResult<Overinterest> getOverinterestList(int firstResult,
	...
	jql.append("select Overinterest from com.yuchengtech.emp.ecif.customer.entity.orgrisk.Overinterest Overinterest where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Overinterest.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Overinterest." + orderBy + " " + orderType);
	}
