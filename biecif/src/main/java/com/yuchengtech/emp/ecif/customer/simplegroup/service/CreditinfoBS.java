...
public class CreditinfoBS extends BaseBS<Object> {
...
public SearchResult<Creditinfo> getCreditinfoList(int firstResult,
	...
	jql.append("select Creditinfo from com.yuchengtech.emp.ecif.customer.entity.orgevaluate.Creditinfo Creditinfo where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Creditinfo.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Creditinfo." + orderBy + " " + orderType);
	}
