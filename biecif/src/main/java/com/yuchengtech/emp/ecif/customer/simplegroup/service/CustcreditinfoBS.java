...
public class CustcreditinfoBS extends BaseBS<Object> {
...
public SearchResult<Custcreditinfo> getCustcreditinfoList(int firstResult,
	...
	jql.append("select Custcreditinfo from com.yuchengtech.emp.ecif.customer.entity.orgrisk.Custcreditinfo Custcreditinfo where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Custcreditinfo.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Custcreditinfo." + orderBy + " " + orderType);
	}
