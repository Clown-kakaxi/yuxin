...
public class OrgDealingBS extends BaseBS<Object> {
	...
	public SearchResult<Orgspecialcust> getOrgspecialcustList(int firstResult, int pageSize,
			String orderBy, String orderType, Map<String, Object> conditionMap,
			long custId) {
		StringBuffer jql = new StringBuffer("");
		jql.append("select obj from Orgspecialcust obj where 1=1");
	...
	// 展示Orgchannelsign的页面
	@SuppressWarnings("unchecked")
	public SearchResult<Orgchannelsign> getOrgchannelsignList(int firstResult,
			int pageSize, String orderBy, String orderType,
			Map<String, Object> conditionMap, long custId) {
		StringBuffer jql = new StringBuffer("");
		jql.append("select rel from Orgchannelsign rel where 1=1");
		if (!conditionMap.get("jql").equals("")) {
			jql.append(" and " + conditionMap.get("jql"));
		}
		if (!"".equals(custId)) {
