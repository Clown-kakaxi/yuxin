...
public class PersonpointBS extends BaseBS<Object> {


@SuppressWarnings("unchecked")
public SearchResult<Personpoint> getPersonpointList(int firstResult,
		int pageSize, String orderBy, String orderType,
		Map<String, Object> conditionMap,long custId) {
	StringBuffer jql = new StringBuffer("");
	jql.append("select Personpoint from com.yuchengtech.emp.ecif.customer.entity.perevaluate.Personpoint Personpoint where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Personpoint.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Personpoint." + orderBy + " " + orderType);
	}
