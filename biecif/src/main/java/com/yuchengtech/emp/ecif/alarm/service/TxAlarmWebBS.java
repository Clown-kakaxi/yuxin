...
public class TxAlarmWebBS extends BaseBS<TxAlarmWeb> {
	@SuppressWarnings("unchecked")
	public SearchResult<TxAlarmWeb> getTxAlarmWebList(int firstResult, int pageSize, String orderBy, String orderType,
		...
		jql.append("select TxAlarmWeb from TxAlarmWeb TxAlarmWeb where 1=1 and userName='"+loginName +"' ");

		if (!conditionMap.get("jql").equals("")) {
			jql.append(" and " + conditionMap.get("jql"));
		}
		
		SearchResult<TxAlarmWeb> TxAlarmWebList = this.baseDAO.findPageWithNameParam(firstResult, pageSize,
				jql.toString(), values);
		return TxAlarmWebList;
	}
	...
	public List<Map<String, String>> getComBoBox() {
		StringBuffer jql = new StringBuffer("select groupId, groupName from TxAlarmWeb  t");
