...
public class RisksignalBS extends BaseBS<Object> {
...
public SearchResult<Risksignal> getRisksignalList(int firstResult,
	...
	jql.append("select Risksignal from com.yuchengtech.emp.ecif.customer.entity.orgrisk.Risksignal Risksignal where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Risksignal.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Risksignal." + orderBy + " " + orderType);
	}
