...
@Service("PerCustomizationBS")
@Transactional(readOnly = true)
public class PerCustomizationBS extends BaseBS<T> {

	protected static Logger log = LoggerFactory
			.getLogger(PerCustomizationBS.class);
	...
	public SearchResult<Map<String, String>> queryCustomzationListMaps(int firstResult,
		...
		if(sqlMap != null && sqlMap.get("oneToOneSql") != null){
			...
			sql.append(" SELECT CST.CUST_ID FROM M_CI_CUSTOMER CST WHERE 1 = 1 AND CST.CUST_TYPE = '"+CtztConstant.ECIF_CTZT_TABLE_TYPE_PER+"' ");
			...
			if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("prrgender"))) 
					|| !ResultUtil.isEmpty(String.valueOf(queryMap.get("prrcitizenship"))) 
					|| !ResultUtil.isEmpty(String.valueOf(queryMap.get("prrnationality")))
					|| !ResultUtil.isEmpty(String.valueOf(queryMap.get("prrmarriage")))
					|| !ResultUtil.isEmpty(String.valueOf(queryMap.get("prrhighestschooling")))){
				sql.append(" AND EXISTS (SELECT 1 FROM M_CI_PERSON PRR WHERE PRR.CUST_ID = CST.CUST_ID ");
				if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("prrgender")))){
					sql.append(" AND PRR.GENDER = '");
					sql.append(String.valueOf(queryMap.get("prrgender")));
					sql.append("' ");
				}
				if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("prrcitizenship")))){
					sql.append(" AND PRR.CITIZENSHIP = '");
					sql.append(String.valueOf(queryMap.get("prrcitizenship")));
					sql.append("' ");
				}
				if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("prrnationality")))){
					sql.append(" AND PRR.NATIONALITY = '");
					sql.append(String.valueOf(queryMap.get("prrnationality")));
					sql.append("' ");
				}
				if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("prrmarriage")))){
					sql.append(" AND PRR.MARRIAGE = '");
					sql.append(String.valueOf(queryMap.get("prrmarriage")));
					sql.append("' ");
				}
				if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("prrhighestschooling")))){
					sql.append(" AND PRR.HIGHEST_SCHOOLING = '");
					sql.append(String.valueOf(queryMap.get("prrhighestschooling")));
					sql.append("' ");
				}
				sql.append(") ");
			}			
			if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("pslstarlevel")))){
				sql.append(" AND EXISTS(SELECT 1 FROM M_CI_PER_GRADE PSL WHERE CST.CUST_ID = PSL.CUST_ID AND PSL.PERSON_GRADE = '");
				sql.append(String.valueOf(queryMap.get("pslstarlevel")));
				sql.append("') ");
			}
			if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("createTellerNo")))){
				sql.append(" AND CST.CREATE_TELLER_NO = '");
				sql.append(String.valueOf(queryMap.get("createTellerNo")));
				sql.append("' ");
			}
			if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("createBranchNo")))){
				sql.append(" AND CST.CREATE_BRANCH_NO = '");
			...
			if(!ResultUtil.isEmpty(String.valueOf(queryMap.get("identtype")))){
				sql.append(" AND EXISTS(SELECT 1 FROM M_CI_PER_IDENTIFIER PIF WHERE PIF.CUST_ID = CST.CUST_ID AND PIF.IDENT_TYPE = '");
...
//			sql.append(" AND EXISTS(SELECT 1 FROM belongbranch bb WHERE bb.CUST_ID = CST.CUST_ID AND bb.belong_type <> '21') ");
	...
	public List<Map<String, String>> queryCustomzationListMaps(String ids){
		...
		if(sqlMap != null && sqlMap.get("oneToOneSql") != null){
			StringBuffer sql = new StringBuffer("");
			sql.append(" AND CST.CUST_ID IN ("+ids+") AND CST.CUST_TYPE = '"+CtztConstant.ECIF_CTZT_TABLE_TYPE_PER+"' ");
...
//							if(mapKeyName.equals("PERSONIDENTIFIER") && cstMap.get("cstcustid").equals("1000224360219")){
...
//							}
	...
	public String export(String rule) {
		...
		try{
			DefinetableviewLookVO[] tables = customizationColumnQueryBS.getIsResultDefinetableviews(CtztConstant.ECIF_CTZT_TABLE_TYPE_PER);
			DefinetableviewLookVO[] columns = customizationColumnQueryBS.getisMustColumn(CtztConstant.ECIF_CTZT_TABLE_TYPE_PER);
			...
			downloadFile.setPromptMsg("对私客户定制查询客户信息");
			downloadFile.setFileName("对私客户定制查询客户信息.xls");
