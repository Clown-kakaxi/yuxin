...
public class BelonglinedeptBS extends BaseBS<Object> {
...
public SearchResult<Belonglinedept> getBelonglinedeptList(int firstResult,
	...
	jql.append("select Belonglinedept from com.yuchengtech.emp.ecif.customer.entity.permanage.Belonglinedept Belonglinedept where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Belonglinedept.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Belonglinedept." + orderBy + " " + orderType);
	}
