...
public class ScoreinfoBS extends BaseBS<Object> {
...
public SearchResult<Scoreinfo> getScoreinfoList(int firstResult,
	...
	jql.append("select Scoreinfo from com.yuchengtech.emp.ecif.customer.entity.orgevaluate.Scoreinfo Scoreinfo where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Scoreinfo.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Scoreinfo." + orderBy + " " + orderType);
	}
