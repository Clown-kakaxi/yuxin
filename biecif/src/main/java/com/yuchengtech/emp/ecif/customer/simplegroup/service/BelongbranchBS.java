...
public class BelongbranchBS extends BaseBS<Object> {


@SuppressWarnings("unchecked")
public SearchResult<Belongbranch> getBelongbranchList(int firstResult,
		int pageSize, String orderBy, String orderType,
		Map<String, Object> conditionMap,long custId) {
	StringBuffer jql = new StringBuffer("");
	jql.append("select Belongbranch from com.yuchengtech.emp.ecif.customer.entity.permanage.Belongbranch Belongbranch where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Belongbranch.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Belongbranch." + orderBy + " " + orderType);
	}
