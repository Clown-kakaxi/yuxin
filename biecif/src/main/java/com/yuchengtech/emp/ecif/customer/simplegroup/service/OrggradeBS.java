...
public class OrggradeBS extends BaseBS<Object> {
...
public SearchResult<Orggrade> getOrggradeList(int firstResult,
	...
	jql.append("select Orggrade from com.yuchengtech.emp.ecif.customer.entity.orgevaluate.Orggrade Orggrade where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Orggrade.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Orggrade." + orderBy + " " + orderType);
	}
