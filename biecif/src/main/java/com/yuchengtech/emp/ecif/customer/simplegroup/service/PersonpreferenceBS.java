...
public class PersonpreferenceBS extends BaseBS<Object> {


@SuppressWarnings("unchecked")
public SearchResult<Personpreference> getPersonpreferenceList(int firstResult,
		int pageSize, String orderBy, String orderType,
		Map<String, Object> conditionMap,long custId) {
	StringBuffer jql = new StringBuffer("");
	jql.append("select Personpreference from com.yuchengtech.emp.ecif.customer.entity.perevaluate.Personpreference Personpreference where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Personpreference.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Personpreference." + orderBy + " " + orderType);
	}
