...
public class BelongManagerBS extends BaseBS<Object> {
...
public SearchResult<BelongManager> getBelongManagerList(int firstResult,
	...
	jql.append("select BelongManager from com.yuchengtech.emp.ecif.customer.entity.permanage.BelongManager BelongManager where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and BelongManager.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by BelongManager." + orderBy + " " + orderType);
	}
