...
import com.yuchengtech.emp.ecif.customer.entity.orgrisk.Rating;
@Service
@Transactional(readOnly = true)
public class RatingBS extends BaseBS<Object> {


@SuppressWarnings("unchecked")
public SearchResult<Rating> getRatingList(int firstResult,
		int pageSize, String orderBy, String orderType,
		Map<String, Object> conditionMap,long custId) {
	StringBuffer jql = new StringBuffer("");
	jql.append("select Rating from com.yuchengtech.emp.ecif.customer.entity.orgrisk.Rating Rating where 1=1");
	if (!conditionMap.get("jql").equals("")) {
		jql.append(" and " + conditionMap.get("jql"));
	}
	if(!"".equals(custId)){
		jql.append(" and Rating.custId = " + custId + "");
	}
	if (!StringUtils.isEmpty(orderBy)) {
		jql.append(" order by Rating." + orderBy + " " + orderType);
	}
